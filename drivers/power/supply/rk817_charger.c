...
static int rk817_charge_usb_get_property(struct power_supply *psy, enum power_supply_property psp, union power_supply_propval *val)
{
    struct rk817_charger *charge = power_supply_get_drvdata(psy);
    int ret = 0;
...
EXPORT_SYMBOL_GPL(rk817_charge_get_charge_state);

void rk817_charge_enable_charge(struct rk817_charger *charge)
{
    rk817_charge_field_write(charge, CHRG_EN, ENABLE);
}
EXPORT_SYMBOL_GPL(rk817_charge_enable_charge);

void rk817_charge_disable_charge(struct rk817_charger *charge)
{
    rk817_charge_field_write(charge, CHRG_EN, DISABLE);
}
EXPORT_SYMBOL_GPL(rk817_charge_disable_charge);

void rk817_charge_usb_to_sys_enable(struct rk817_charger *charge)
{
    rk817_charge_field_write(charge, USB_SYS_EN, ENABLE);
}
EXPORT_SYMBOL_GPL(rk817_charge_usb_to_sys_enable);

void rk817_charge_usb_to_sys_disable(struct rk817_charger *charge)
{
    rk817_charge_field_write(charge, USB_SYS_EN, DISABLE);
}
EXPORT_SYMBOL_GPL(rk817_charge_usb_to_sys_disable);

int rk817_charge_get_usb_to_sys_status(struct rk817_charger *charge)
{
    return rk817_charge_field_read(charge, USB_SYS_EN);
}
EXPORT_SYMBOL_GPL(rk817_charge_get_usb_to_sys_status);

static struct rk817_charger *g_charge = NULL;
static void rk817_charge_set_charger(struct rk817_charger *charge)
{
    g_charge = charge;
}

struct rk817_charger *rk817_charge_get_charger(void)
{
    return g_charge;
}
EXPORT_SYMBOL_GPL(rk817_charge_get_charger);
/* changed end. */
...
//tanlq add 220617 for boe no cc_report line
		...
		}else if(cc_type == 5 || cc_type == 0x500){
			charger = USB_TYPE_AC_CHARGER;
		}else{
			charger = USB_TYPE_AC_OLD_CHARGER;
		}
...
static int rk817_charge_init_irqs(struct rk817_charger *charge)
{
    ...
    ret = devm_request_threaded_irq(charge->dev, plug_in_irq, NULL, rk817_plug_in_isr, IRQF_TRIGGER_RISING | IRQF_ONESHOT, "rk817_plug_in", charge);
    if (ret) {
        dev_err(&pdev->dev, "plug_in_irq request failed!\n");
        return ret;
    }

    ret = devm_request_threaded_irq(charge->dev, plug_out_irq, NULL, rk817_plug_out_isr, IRQF_TRIGGER_RISING | IRQF_ONESHOT, "rk817_plug_out", charge);
    if (ret) {
