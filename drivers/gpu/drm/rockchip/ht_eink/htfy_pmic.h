...
extern int pmic_probe(struct pmic_sess **sess, const char *id, unsigned int dwell_time_ms,
        unsigned int vcomoff_time_ms, int v3p3_time_ms);
...
struct htfy_pwr_ops
{
    ...
    // NOTE: 在 WAKEUP 拉高的时候，我们需要重新初始化 65185（因为内部的寄存器被复位了).
    // 我们增加了这个函数。其实很多设置直接用默认的值也是可以的。
    // NOTE: 增加一个新的参数: epd_type 来做新旧 EPD的兼容。这个参数来源于 ebc-driver,所以
    // 我们在 65185 里面的初始化要延后一点，通过这个 reinit 来进行。
    int (*reinit)(void*, int epd_type);

    // NOTE：如果调用了 sleep函数，唤醒的时候会显示异常。
    //int (*power_hw_sleep)(void*);
    //int (*power_hw_resume)(void*);
};

// NOTE: EINK 温度传感器读取温度操作，温控模块需提供 htfy_register_ebc_temp_ops 供本模块调用.
// 返回值: 0 --操作OK, < 0: 出错。
