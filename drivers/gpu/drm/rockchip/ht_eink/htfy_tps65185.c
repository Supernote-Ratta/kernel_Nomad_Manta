...
struct papyrus_sess {
	...
	// 20211124: 3566上面增加了 int gpio，出错的时候可以通知系统.目前我们用查询的方式，
	// 所以这个 int 实际可以不需要。 --可能这个 int 就是 PX30 上面的 error_pin.
	//int 	int_pin;
	//int 	irq;
...
static int papyrus_hw_reset(struct papyrus_sess *sess)
{
  ...
  //  gpio_set_value(sess->pwr_en_pin, !sess->poweren_active_high);
...
static int papyrus_probe(struct pmic_sess *pmsess,struct i2c_client *client)
{
	...
	if (!gpio_is_valid(sess->vcom_ctl_pin)) {
		...
		// 20211124: 3566上面可以没有VCOM_CTRL.直接有寄存器来控制。
		goto free_sess;
	}
...
static int tps65185_power_check(void *priv, int dump)
{
    ...
    if (sess->error_pin != INVALID_GPIO){
	   ...
	   if(err_pin_value == 0) { // if low, means power-fail.
	        power_good = 0;
	   } else {
	        if(!dump)  return power_good;
	   }
	}
	...
	if(( power_good == -1) || dump) {  // || BGB_PRINT
	    // INT1_STATUS=0X00000000,INT2_STATUS=0X00000000,PG_STATUS=0X000000FA  // ,dump=%d
