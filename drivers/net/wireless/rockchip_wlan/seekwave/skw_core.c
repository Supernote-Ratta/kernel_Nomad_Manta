...
static int skw_core_show(struct seq_file *seq, void *data)
{
	...
	const char bus[][8] = {"SDIO", "USB", "PCIE", "SDIO_V2"};
...
#define FLAG_TEST(n) test_bit(SKW_FLAG_##n, &skw->flags)
	...
	seq_printf(seq, "Calib File:      \t%s\n"
			...
			bus[skw->hw_pdata->bus_type & SKW_BUS_TYPE_MASK],
...
static int skw_core_init(struct skw_core *skw, struct platform_device *pdev)
{
	...
	skw->event_wq = alloc_workqueue("skw_event_wq", WQ_UNBOUND | WQ_MEM_RECLAIM, 1);
	if (!skw->event_wq) {
...
int skw_lmac_unbind_iface(struct skw_core *skw, int lmac_id, int iface_id)
{
	...
	if (skw->hw.bus == SKW_BUS_PCIE) {
		// skw_edma_disable_channel(&lmac->edma_tx_chn);
	}
...
static int skw_drv_probe(struct platform_device *pdev)
{
	...
	skw_add_debugfs("core", 0444, skw, &skw_core_fops);
	skw_add_debugfs("assert", 0444, skw, &skw_assert_fops);
