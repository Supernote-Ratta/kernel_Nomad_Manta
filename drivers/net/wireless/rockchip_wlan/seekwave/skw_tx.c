...
int skw_sdio_cmd_xmit(struct skw_core *skw, void *data, int data_len)
{
	...
	skw_set_extra_hdr(skw, eof_blk, skw->hw.cmd_port, skw->hw.align, 0, 1);
	sg_set_buf(&skw->sg_list[nr++], eof_blk, skw->hw.align);
			...
			int nents, int tx_len)
{
	...
	total_len = sg_copy_to_buffer(sgl, nents, sdma_buff,
				skw->hw_pdata->max_buffer_size);

	skw->hw_pdata->hw_sdma_tx(port, sdma_buff, total_len);
...
static int __skw_tx_init(struct skw_core *skw)
{
	...
	skw->tx_wq = alloc_workqueue("skwtx_workqueue", WQ_UNBOUND |
				WQ_CPU_INTENSIVE | WQ_HIGHPRI | WQ_SYSFS, 0);
	if (!skw->tx_wq) {
...
static int __skw_tx_init(struct skw_core *skw)
{
	skw->tx_thread = kthread_create(skw_tx_thread, skw, "skw_tx");
...
static void skw_register_tx_callback(struct skw_core *skw, void *func, void *data)
{
	int i;
	...
	for (i = 0; i < SKW_NR_LMAC; i++) {
		...
		skw_register_tx_cb(skw, skw->hw.lmac[i].dport, func, data);
	}
}
...
static int skw_hw_xmit_init(struct skw_core *skw, int dma)
{
	...
	case SKW_SYNC_SDMA_TX:
		...
		sdma_buff = SKW_ALLOC(skw->hw_pdata->max_buffer_size, GFP_KERNEL);
		if (!sdma_buff)
			ret = -ENOMEM;
	...
	case SKW_ASYNC_ADMA_TX:
		...
		skw_register_tx_callback(skw, skw_async_adma_tx_free, skw);
		break;
	...
	case SKW_ASYNC_SDMA_TX:
		...
		skw_register_tx_callback(skw, skw_async_sdma_tx_free, skw);
		break;
...
int skw_tx_init(struct skw_core *skw)
{
	...
	if (skw->hw.bus == SKW_BUS_SDIO) {
		eof_blk = SKW_ALLOC(skw->hw.align, GFP_KERNEL);
		if (!eof_blk)
			return -ENOMEM;
