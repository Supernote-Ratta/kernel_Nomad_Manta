...
static int g_dfs_testmode = 0;
struct skw_dfs_pool g_pri_pool = {0};
				  ...
				  u64 ts, u32 min_count)
{
	...
	LIST_FOR_EACH_ENTRY(p, &pd->pulses, struct skw_pulse_element, head) {
		...
		min_factor = 0;
		LIST_FOR_EACH_ENTRY_CONTINUE(p2, &pd->pulses,
						struct skw_pulse_element, head) {
			u32 factor;
...
static struct skw_pri_sequence *
etsi_add_pulse(struct skw_pri_detector *pd, struct pulse_info *pulse)
{
	u32 max_updated_seq;
	struct skw_pri_sequence *ps;
	u64 ts = pulse->ts;
...
static const struct radar_params *
get_dfs_domain_radar_params(enum nl80211_dfs_regions region)
{
	u32 i;
...
static struct skw_pri_detector *
skw_pri_detector_init(const struct radar_pattern_param *rpp)
{
...
static struct skw_dfs_detector_ctxt *
skw_dfs_detector_init(enum nl80211_dfs_regions region, u8 init_pool)
{
...
static int skw_cmd_dfs_chan_switch(struct wiphy *wiphy, struct net_device *ndev,
			    struct element *csa_ie, struct element *ecsa_ie,
			    struct element *csw_ie)
{
	int ret;
	u8 buf[256 * 3 + 6];
	u16 idx;
				...
				u8 *data, u8 data_len)
{
	...
	for (i = 0; i < pulse_num; i++) {
		...
		if (skw_add_pulse(detector, &info)) {
			...
			if (g_dfs_testmode == 0) {
				skw_del_timer_work((void *)skw_dfs_cac_timeout);
			...
			u8 *data, u8 data_len)
{
	...
	skw_add_timer_work("dfs_cac_timeout", skw_dfs_cac_timeout, iface,
...
void skw_dfs_stop_cac_event(struct wiphy *wiphy, struct skw_iface *iface)
{
	...
	cfg80211_cac_event(iface->ndev, &ctxt->chan_def,
...
{
	int ret;
	struct element *csa_ie, *ecsa_ie, *csw_ie;
	struct skw_iface *iface = netdev_priv(ndev);

	csa_ie = (struct element *)cfg80211_find_ie(WLAN_EID_CHANNEL_SWITCH, ies, ies_len);
	ecsa_ie = (struct element *)cfg80211_find_ie(WLAN_EID_EXT_CHANSWITCH_ANN, ies, ies_len);
	csw_ie = (struct element *)cfg80211_find_ie(WLAN_EID_CHANNEL_SWITCH_WRAPPER, ies, ies_len);
	...
	skw_log(SKW_ERROR, "[SKWIFI DFS]: skw_dfs_trig_chan_switch.\n");
	ret = skw_cmd_dfs_chan_switch(wiphy, iface->ndev,
						csa_ie, ecsa_ie, csw_ie);
	if (ret)
		return -EBUSY;
