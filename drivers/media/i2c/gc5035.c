...
static int gc5035_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
    ...
    dev_err(&client->dev, "gc5035 write reg(0x%x val:0x%x) failed !\n", reg, val);

    return ret;
}

static int gc5035_write_array(struct i2c_client *client, const struct regval *regs)
{
    u32 i = 0;
    int ret = 0;
...
static int gc5035_get_reso_dist(const struct gc5035_mode *mode, struct v4l2_mbus_framefmt *framefmt)
{
    return abs(mode->width - framefmt->width) + abs(mode->height - framefmt->height);
}

static const struct gc5035_mode *gc5035_find_best_fit(struct gc5035 *gc5035, struct v4l2_subdev_format *fmt)
{
...
static int gc5035_set_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_format *fmt)
{
    ...
    if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
        ...
        *v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
        ...
        __v4l2_ctrl_modify_range(gc5035->hblank, h_blank, h_blank, 1, h_blank);
        vblank_def = mode->vts_def - mode->height;
        __v4l2_ctrl_modify_range(gc5035->vblank, vblank_def, GC5035_VTS_MAX - mode->height, 1, vblank_def);
    }
...
static int gc5035_get_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_format *fmt)
{
    struct gc5035 *gc5035 = to_gc5035(sd);
...
static int gc5035_enum_mbus_code(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_mbus_code_enum *code)
{
    if (code->index != 0) {
        return -EINVAL;
    }
...
static int gc5035_enum_frame_sizes(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_frame_size_enum *fse)
{
    struct gc5035 *gc5035 = to_gc5035(sd);
...
static int gc5035_g_frame_interval(struct v4l2_subdev *sd, struct v4l2_subdev_frame_interval *fi)
{
    struct gc5035 *gc5035 = to_gc5035(sd);
    const struct gc5035_mode *mode = gc5035->cur_mode;
...
static void gc5035_get_module_inf(struct gc5035 *gc5035, struct rkmodule_inf *inf)
{
    strlcpy(inf->base.sensor, GC5035_NAME, sizeof(inf->base.sensor));
    strlcpy(inf->base.module, gc5035->module_name, sizeof(inf->base.module));
    strlcpy(inf->base.lens, gc5035->len_name, sizeof(inf->base.lens));
}

static void gc5035_set_module_inf(struct gc5035 *gc5035, struct rkmodule_awb_cfg *cfg)
{
    mutex_lock(&gc5035->mutex);
...
static long gc5035_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
    ...
    switch (cmd) {
        ...
        case RKMODULE_SET_QUICK_STREAM:
            stream = *((u32 *)arg);
            if (stream) {
                ret = gc5035_write_reg(gc5035->client, GC5035_REG_SET_PAGE, GC5035_SET_PAGE_ONE);
                ret |= gc5035_write_reg(gc5035->client, GC5035_REG_CTRL_MODE, GC5035_MODE_STREAMING);
            } else {
                ret = gc5035_write_reg(gc5035->client, GC5035_REG_SET_PAGE, GC5035_SET_PAGE_ONE);
                ret |= gc5035_write_reg(gc5035->client, GC5035_REG_CTRL_MODE, GC5035_MODE_SW_STANDBY);
            }
            break;
        default:
            ret = -ENOTTY;
...
#ifdef CONFIG_COMPAT
static long gc5035_compat_ioctl32(struct v4l2_subdev *sd, unsigned int cmd, unsigned long arg)
{
...
static int __gc5035_power_on(struct gc5035 *gc5035)
{
    ...
    if (!IS_ERR_OR_NULL(gc5035->pins_default)) {
        ret = pinctrl_select_state(gc5035->pinctrl, gc5035->pins_default);
        if (ret < 0) {
    ...
    if (!IS_ERR(gc5035->pwren_gpio)) {
        gpiod_set_value_cansleep(gc5035->pwren_gpio, 1);
    }
...
static void __gc5035_power_off(struct gc5035 *gc5035)
{
    ...
    if (!IS_ERR_OR_NULL(gc5035->pins_sleep)) {
        ret = pinctrl_select_state(gc5035->pinctrl, gc5035->pins_sleep);
        if (ret < 0) {
    ...
    if (!IS_ERR(gc5035->pwren_gpio)) {
        gpiod_set_value_cansleep(gc5035->pwren_gpio, 0);
    }
}
...
static int sensor_g_mbus_config(struct v4l2_subdev *sd, struct v4l2_mbus_config *config)
{
    ...
    if (2 == sensor->lane_num) {
        ...
        dev_err(&sensor->client->dev, "unsupported lane_num(%d)\n", sensor->lane_num);
    }
    return 0;
}

static int gc5035_enum_frame_interval(struct v4l2_subdev *sd, struct v4l2_subdev_pad_config *cfg, struct v4l2_subdev_frame_interval_enum *fie)
{
    struct gc5035 *gc5035 = to_gc5035(sd);
...
static const struct dev_pm_ops gc5035_pm_ops = {
    SET_RUNTIME_PM_OPS(gc5035_runtime_suspend, gc5035_runtime_resume, NULL)
};
...
static int gc5035_set_exposure_reg(struct gc5035 *gc5035, u32 exposure)
{
    ...
    ret |= gc5035_write_reg(gc5035->client, GC5035_REG_EXPOSURE_H, (caltime >> 8) & 0x3F);
    ret |= gc5035_write_reg(gc5035->client, GC5035_REG_EXPOSURE_L, caltime & 0xFF);
...
static int gc5035_set_gain_reg(struct gc5035 *gc5035, u32 a_gain)
{
    ...
    ret = gc5035_write_reg(gc5035->client, GC5035_REG_SET_PAGE, GC5035_SET_PAGE_ONE);
    ret |= gc5035_write_reg(gc5035->client, GC5035_REG_AGAIN, GC5035_AGC_Param[i][1]);
    temp_gain = a_gain;
    ...
    ret |= gc5035_write_reg(gc5035->client, GC5035_REG_DGAIN_INT, temp_gain >> 6);
    ret |= gc5035_write_reg(gc5035->client, GC5035_REG_DGAIN_FRAC, (temp_gain << 2) & 0xfc);
    return ret;
}

static int gc5035_set_ctrl(struct v4l2_ctrl *ctrl)
{
    struct gc5035 *gc5035 = container_of(ctrl->handler, struct gc5035, ctrl_handler);
    struct i2c_client *client = gc5035->client;
    s64 max;
    int ret = 0;
    ...
    switch (ctrl->id) {
        case V4L2_CID_VBLANK:
            ...
            __v4l2_ctrl_modify_range(gc5035->exposure, gc5035->exposure->minimum, max, gc5035->exposure->step, gc5035->exposure->default_value);
            break;
    }
    ...
    switch (ctrl->id) {
        ...
        case V4L2_CID_VBLANK:
            ret = gc5035_write_reg(gc5035->client, GC5035_REG_SET_PAGE, GC5035_SET_PAGE_ONE);
            ret |= gc5035_write_reg(gc5035->client, GC5035_REG_VTS_H, ((ctrl->val + gc5035->cur_mode->height) >> 8) & 0xff);
            ret |= gc5035_write_reg(gc5035->client, GC5035_REG_VTS_L, (ctrl->val + gc5035->cur_mode->height) & 0xff);
            break;
...
static int gc5035_initialize_controls(struct gc5035 *gc5035)
{
    ...
    v4l2_ctrl_new_std(handler, NULL, V4L2_CID_PIXEL_RATE, 0, GC5035_PIXEL_RATE, 1, GC5035_PIXEL_RATE);

    h_blank = mode->hts_def - mode->width;
    gc5035->hblank = v4l2_ctrl_new_std(handler, NULL, V4L2_CID_HBLANK, h_blank, h_blank, 1, h_blank);
    if (gc5035->hblank) {
        gc5035->hblank->flags |= V4L2_CTRL_FLAG_READ_ONLY;
    }

    vblank_def = mode->vts_def - mode->height;
    gc5035->vblank = v4l2_ctrl_new_std(handler, &gc5035_ctrl_ops, V4L2_CID_VBLANK, vblank_def, GC5035_VTS_MAX - mode->height, 1, vblank_def);

    exposure_max = mode->vts_def - 4;
    gc5035->exposure = v4l2_ctrl_new_std(handler, &gc5035_ctrl_ops, V4L2_CID_EXPOSURE, GC5035_EXPOSURE_MIN, exposure_max, GC5035_EXPOSURE_STEP, mode->exp_def);

    gc5035->anal_gain = v4l2_ctrl_new_std(handler, &gc5035_ctrl_ops, V4L2_CID_ANALOGUE_GAIN, GC5035_GAIN_MIN, GC5035_GAIN_MAX, GC5035_GAIN_STEP, GC5035_GAIN_DEFAULT);

    gc5035->test_pattern = v4l2_ctrl_new_std_menu_items(handler, &gc5035_ctrl_ops, V4L2_CID_TEST_PATTERN, ARRAY_SIZE(gc5035_test_pattern_menu) - 1, 0, 0, gc5035_test_pattern_menu);

    if (handler->error) {
        ret = handler->error;
        dev_err(&gc5035->client->dev, "Failed to init controls(%d)\n", ret);
...
static int gc5035_check_sensor_id(struct gc5035 *gc5035, struct i2c_client *client)
{
    struct device *dev = &gc5035->client->dev;
    u16 id = 0;
    u8 reg_H = 0;
...
static int gc5035_configure_regulators(struct gc5035 *gc5035)
{
    ...
    return devm_regulator_bulk_get(&gc5035->client->dev, GC5035_NUM_SUPPLIES, gc5035->supplies);
}
...
static void free_gpio(struct gc5035 *sensor)
{
    ...
    if (!IS_ERR(sensor->pwren_gpio)) {
        temp_gpio = desc_to_gpio(sensor->pwren_gpio);
        dev_info(dev, "free gpio(%d)!\n", temp_gpio);
        gpio_free(temp_gpio);
    }
}
...
static int gc5035_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    // int pwren, pwrdn, reset;
    // enum of_gpio_flags flag;

    dev_info(dev, "driver version: %02x.%02x.%02x", DRIVER_VERSION >> 16, (DRIVER_VERSION & 0xff00) >> 8, DRIVER_VERSION & 0x00ff);
    ...
    ret = of_property_read_u32(node, RKMODULE_CAMERA_MODULE_INDEX, &gc5035->module_index);
    ret |= of_property_read_string(node, RKMODULE_CAMERA_MODULE_FACING, &gc5035->module_facing);
    ret |= of_property_read_string(node, RKMODULE_CAMERA_MODULE_NAME, &gc5035->module_name);
    ret |= of_property_read_string(node, RKMODULE_CAMERA_LENS_NAME, &gc5035->len_name);
    if (ret) {
    ...
    gc5035->pwren_gpio = devm_gpiod_get(dev, "pwen", GPIOD_OUT_HIGH);
    if (IS_ERR(gc5035->pwren_gpio)) {
        dev_err(dev, "Failed to get pwen-gpios\n");
    }
    ...
    // pwren = of_get_named_gpio_flags(node, "gpio_pwen", 0, &flag);
    // if (!gpio_is_valid(pwren)) {
    //     dev_err(dev, "no pwen pin available\n");
    // } else {
    //     ret = devm_gpio_request_one(dev, pwren, GPIOF_OUT_INIT_LOW, "pwen");
    //     if (ret < 0) {
    //         dev_err(dev, "request pwen pin failed!!!\n");
    //     } else {
    //         dev_info(dev, "set pwren_gpio\n");
    //         gpio_direction_output(pwren, 1);
    //     }
    // }

    // pwrdn = of_get_named_gpio_flags(node, "gpio_pwdn", 0, &flag);
    // if (!gpio_is_valid(pwrdn)) {
    //     dev_err(dev, "no pwrdn pin available\n");
    // } else {
    //     ret = devm_gpio_request_one(dev, pwrdn, GPIOF_OUT_INIT_LOW, "pwdn");
    //     if (ret < 0) {
    //         dev_err(dev, "request pwrdn pin failed!!!\n");
    //     } else {
    //         dev_info(dev, "set pwrdn\n");
    //         gpio_direction_output(pwrdn, 1);
    //     }
    // }

    // reset = of_get_named_gpio_flags(node, "gpio_reset", 0, &flag);
    // if (!gpio_is_valid(reset)) {
    //     dev_err(dev, "no reset pin available\n");
    // } else {
    //     ret = devm_gpio_request_one(dev, reset, GPIOF_OUT_INIT_LOW, "reset");
    //     if (ret < 0) {
    //         dev_err(dev, "request reset pin failed!!!\n");
    //     } else {
    //         dev_info(dev, "set reset\n");
    ...
    */
    ...
    if (!IS_ERR(gc5035->pinctrl)) {
        gc5035->pins_default = pinctrl_lookup_state(gc5035->pinctrl, OF_CAMERA_PINCTRL_STATE_DEFAULT);
        if (IS_ERR(gc5035->pins_default)) {
            dev_err(dev, "could not get default pinstate\n");
        }

        gc5035->pins_sleep = pinctrl_lookup_state(gc5035->pinctrl, OF_CAMERA_PINCTRL_STATE_SLEEP);
    ...
    snprintf(sd->name, sizeof(sd->name), "m%02d_%s_%s %s", gc5035->module_index, facing, GC5035_NAME, dev_name(sd->dev));
