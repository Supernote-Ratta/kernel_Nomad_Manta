...
#define FLAG_EVENT (EVENT_RX | EVENT_TIMER_MUX | EVENT_TIMER_STATE)

#define PACKET_IS_CONTROL_MSG(header, type) (PD_HEADER_CNT(header) == 0 && PD_HEADER_TYPE(header) == type)
#define PACKET_IS_DATA_MSG(header, type) (PD_HEADER_CNT(header) != 0 && PD_HEADER_TYPE(header) == type)
...
#define MODE_DP_PIN_DP_MASK (MODE_DP_PIN_C | MODE_DP_PIN_D | MODE_DP_PIN_E | MODE_DP_PIN_F)
...
static bool is_write_reg(struct device *dev, unsigned int reg)
{
    if (reg >= FUSB_REG_FIFO) {
        ...
        return ((reg < (FUSB_REG_CONTROL4 + 1)) && (reg > 0x01)) ? true : false;
    }
}
...
static void fusb_set_pos_power(struct fusb30x_chip *chip, int max_vol, int max_cur)
{
    int i;
    int pos_find;
    int tmp;
    ...
    for (i = PD_HEADER_CNT(chip->rec_head) - 1; i >= 0; i--) {
        switch (CAP_POWER_TYPE(chip->rec_load[i])) {
            case 0:
                /* Fixed Supply */
                if ((CAP_FPDO_VOLTAGE(chip->rec_load[i]) * 50) <= max_vol && (CAP_FPDO_CURRENT(chip->rec_load[i]) * 10) <= max_cur) {
                    chip->pos_power = i + 1;
            ...
            case 1:
                /* Battery */
                if ((CAP_VPDO_MAX_VOLTAGE(chip->rec_load[i]) * 50) <= max_vol && (CAP_VPDO_CURRENT(chip->rec_load[i]) * 10) <= max_cur) {
                    chip->pos_power = i + 1;
...
{
    bool plugged = false, flip = false, dfp = false, ufp = false, dp = false, usb_ss = false, hpd = false;
    union extcon_property_value property;

    if (chip->notify.is_cc_connected) {
        chip->notify.orientation = (chip->cc_polarity == TYPEC_POLARITY_CC1) ? TYPEC_ORIENTATION_CC1 : TYPEC_ORIENTATION_CC2;
    }

    /* avoid notify repeated */
    if (memcmp(&chip->notify, &chip->notify_cmp, sizeof(struct notify_info))) {
        dump_notify_info(chip);
        chip->notify.attention = false;
        memcpy(&chip->notify_cmp, &chip->notify, sizeof(struct notify_info));

        plugged = chip->notify.is_cc_connected || chip->notify.is_pd_connected;
        if (chip->notify.orientation != TYPEC_ORIENTATION_NONE) {
            flip = (chip->notify.orientation == TYPEC_ORIENTATION_CC1) ? false : true;
        }
        dp = chip->notify.is_enter_mode;

        if (dp) {
            dfp = true;
            usb_ss = (chip->notify.pin_assignment_def & MODE_DP_PIN_MF_MASK) ? true : false;
        ...
        extcon_set_property(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY, property);
        extcon_set_property(chip->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY, property);
        extcon_set_property(chip->extcon, EXTCON_DISP_DP, EXTCON_PROP_USB_TYPEC_POLARITY, property);

        property.intval = usb_ss;
        extcon_set_property(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_SS, property);
        extcon_set_property(chip->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS, property);
        extcon_set_property(chip->extcon, EXTCON_DISP_DP, EXTCON_PROP_USB_SS, property);
        ...
        if (chip->notify.power_role == POWER_ROLE_SINK &&
            ...
            property.intval = (chip->pd_output_cur << 15 | chip->pd_output_vol);
            extcon_set_property(chip->extcon, EXTCON_CHG_USB_FAST, EXTCON_PROP_USB_TYPEC_POLARITY, property);
...
static void platform_set_vbus_lvl_enable(struct fusb30x_chip *chip, int vbus_5v, int vbus_other)
{
    ...
    if (chip->gpio_vbus_5v) {
        ...
        ret = extcon_set_state(chip->extcon, EXTCON_USB_VBUS_EN, vbus_5v);
        extcon_sync(chip->extcon, EXTCON_USB_VBUS_EN);
        dev_info(chip->dev, "fusb302 send extcon to %s vbus 5v. ret = %d\n", vbus_5v ? "enable" : "disable", ret);
    }
...
static int tcpm_get_cc_pull_up(struct fusb30x_chip *chip, enum CC_ORIENTATION cc)
{
    int ret;
    u32 val, store;
    ...
    val = store;
    val &= ~(SWITCHES0_MEAS_CC1 | SWITCHES0_MEAS_CC2 | SWITCHES0_PU_EN1 | SWITCHES0_PU_EN2);
...
static int tcpm_get_cc_pull_down(struct fusb30x_chip *chip, enum CC_ORIENTATION cc)
{
    u32 val, store;
    ...
    val |= (cc == TYPEC_ORIENTATION_CC1) ? SWITCHES0_MEAS_CC1 : SWITCHES0_MEAS_CC2;
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES0, SWITCHES0_MEAS_CC1 | SWITCHES0_MEAS_CC2 | SWITCHES0_PU_EN1 | SWITCHES0_PU_EN2 | SWITCHES0_PDWN1 | SWITCHES0_PDWN2, val);
    usleep_range(250, 300);
...
static void tcpm_set_cc_pull_mode(struct fusb30x_chip *chip, enum CC_MODE mode)
{
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES0, SWITCHES0_PU_EN1 | SWITCHES0_PU_EN2 | SWITCHES0_PDWN1 | SWITCHES0_PDWN2, val);
...
static int tcpm_set_cc(struct fusb30x_chip *chip, enum role_mode mode)
{
    switch (mode) {
        case ROLE_MODE_DFP:
            ...
            regmap_update_bits(chip->regmap, FUSB_REG_CONTROL2, CONTROL2_MODE | CONTROL2_TOG_RD_ONLY, CONTROL2_MODE_DFP | CONTROL2_TOG_RD_ONLY);
            break;
        case ROLE_MODE_UFP:
            tcpm_set_cc_pull_mode(chip, CC_PULL_UP);
            regmap_update_bits(chip->regmap, FUSB_REG_CONTROL2, CONTROL2_MODE | CONTROL2_TOG_RD_ONLY, CONTROL2_MODE_UFP);
            break;
        case ROLE_MODE_DRP:
            tcpm_set_cc_pull_mode(chip, CC_PULL_NONE);
            regmap_update_bits(chip->regmap, FUSB_REG_CONTROL2, CONTROL2_MODE | CONTROL2_TOG_RD_ONLY, CONTROL2_MODE_DRP | CONTROL2_TOG_RD_ONLY);
            break;
        default:
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_CONTROL4, CONTROL4_TOG_USRC_EXIT, CONTROL4_TOG_USRC_EXIT);
    regmap_update_bits(chip->regmap, FUSB_REG_CONTROL2, CONTROL2_TOGGLE, CONTROL2_TOGGLE);
...
static int tcpm_set_rx_enable(struct fusb30x_chip *chip, int enable)
{
    ...
    if (enable) {
        ...
        regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES0, SWITCHES0_MEAS_CC1 | SWITCHES0_MEAS_CC2, val);
        fusb302_flush_rx_fifo(chip);
        regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_AUTO_CRC, SWITCHES1_AUTO_CRC);
    } else {
        regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES0, SWITCHES0_MEAS_CC1 | SWITCHES0_MEAS_CC2, 0);
        regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_AUTO_CRC, 0);
    }
...
{
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_POWERROLE | SWITCHES1_DATAROLE, (chip->notify.power_role << 7) | (chip->notify.data_role << 4));
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_SPECREV, 2 << 5);
    return 0;
}

static int tcpm_set_polarity(struct fusb30x_chip *chip, enum typec_cc_polarity polarity)
{
    u8 val = 0;
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES0, SWITCHES0_VCONN_CC1 | SWITCHES0_VCONN_CC2 | SWITCHES0_MEAS_CC1 | SWITCHES0_MEAS_CC2 | SWITCHES0_PU_EN1 | SWITCHES0_PU_EN2, val);
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_TXCC1 | SWITCHES1_TXCC2, val);
...
static int tcpm_set_vconn(struct fusb30x_chip *chip, int enable)
{
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES0, SWITCHES0_VCONN_CC1 | SWITCHES0_VCONN_CC2, val);
...
static void tcpm_init(struct fusb30x_chip *chip)
{
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_RESET, RESET_SW_RESET, RESET_SW_RESET);
    fusb302_pd_reset(chip);
    /* set auto_retry and number of retries */
    regmap_update_bits(chip->regmap, FUSB_REG_CONTROL3, CONTROL3_AUTO_RETRY | CONTROL3_N_RETRIES, CONTROL3_AUTO_RETRY | CONTROL3_N_RETRIES);
    /* set interrupts */
    val = 0xff;
    ...
    val = 0xff;
    val &= ~(MASKA_M_RETRYFAIL | MASKA_M_HARDSENT | MASKA_M_TXSENT | MASKA_M_HARDRST | MASKA_M_TOGDONE);
    regmap_write(chip->regmap, FUSB_REG_MASKA, val);

    val = ~MASKB_M_GCRCSEND;
    regmap_write(chip->regmap, FUSB_REG_MASKB, val);

    /* tower changed: default is TYPEC_RP_USB*/
    tcpm_select_rp_value(chip, TYPEC_RP_USB);
    /* tower changed end.*/
    /* Interrupts Enable */
    regmap_update_bits(chip->regmap, FUSB_REG_CONTROL0, CONTROL0_INT_MASK, ~CONTROL0_INT_MASK);
...
static void tcpc_alert(struct fusb30x_chip *chip, u32 *evt)
{
    ...
    if ((interrupt & INTERRUPT_COMP_CHNG) && (CC_STATE_ROLE(chip) != CC_STATE_TOGSS_IS_UFP)) {
        regmap_read(chip->regmap, FUSB_REG_STATUS0, &val);
        if (val & STATUS0_COMP) {
            *evt |= EVENT_CC;
        }
...
static void set_state_unattached(struct fusb30x_chip *chip)
{
    ...
    regmap_update_bits(chip->regmap, FUSB_REG_MASK, MASK_M_COMP_CHNG, MASK_M_COMP_CHNG);
...
static void set_mesg(struct fusb30x_chip *chip, int cmd, int is_DMT)
{
    ...
    chip->send_head = ((chip->msg_id & 0x7) << 9) | ((chip->notify.power_role & 0x1) << 8) | (1 << 6) | ((chip->notify.data_role & 0x1) << 5);
            ...
            case DMT_REQUEST:
                ...
                switch (CAP_POWER_TYPE(rec_load)) {
                    ...
                    case 1:
                        ...
                        chip->sink_max_volt = CAP_VPDO_MAX_VOLTAGE(rec_load);
                        chip->sink_min_volt = CAP_VPDO_MIN_VOLTAGE(rec_load);
                        chip->sink_opr_power = CAP_VPDO_CURRENT(rec_load);
            ...
            case DMT_SINKCAPABILITIES:
                ...
                switch (chip->sink_supply_type) {
                    case 0:
                         ...
                         */
                        chip->send_load[0] = (chip->sink_supply_type << 30) | (1 << 26) | (chip->sink_volt << 10) | (chip->sink_opr_cur);
                        break;
                    case 1:
                        /* Battery Supply */
                        chip->send_load[0] = (chip->sink_supply_type << 30) | (chip->sink_max_volt << 20) | (chip->sink_min_volt << 10) | (chip->sink_opr_cur);
                        break;
                    default:
 ...
 */
static int pd_dfp_dp_get_pin_assignment(struct fusb30x_chip *chip, uint32_t caps, uint32_t status)
{
    uint32_t pin_caps;
...
static void set_vdm_mesg(struct fusb30x_chip *chip, int cmd, int type, int mode)
{
    ...
    switch (cmd) {
        ...
        case VDM_DP_CONFIG:
            ...
            chip->notify.pin_assignment_def = pd_dfp_dp_get_pin_assignment(chip, chip->notify.dp_caps, chip->notify.dp_status);

            chip->send_load[1] = (chip->notify.pin_assignment_def << 8) | (1 << 2) | 2;
    ...
    switch (GET_VDMHEAD_CMD_TYPE(vdm_header)) {
        ...
        case VDM_TYPE_ACK:
            switch (GET_VDMHEAD_CMD(vdm_header)) {
                ...
                case VDM_DISCOVERY_SVIDS:
                    for (i = 0; i < 6; i++) {
                        tmp = (chip->rec_load[i + 1] >> 16) & 0x0000ffff;
                        if (tmp) {
                ...
                case VDM_DISCOVERY_MODES:
                    ...
                    if (PD_HEADER_CNT(chip->rec_head) > 1) {
                        ...
                        chip->notify.pin_assignment_support = PD_DP_PIN_CAPS(tmp);
                        chip->val_tmp |= 1;
                        dev_dbg(chip->dev, "DisplayPort Capabilities: 0x%08x\n", chip->rec_load[1]);
                    }
                    break;
                case VDM_ENTER_MODE:
                    chip->val_tmp = 1;
                ...
                case VDM_DP_CONFIG:
                    chip->val_tmp = 1;
                    dev_info(chip->dev, "DP config successful, pin_assignment 0x%x\n", chip->notify.pin_assignment_def);
...
static int vdm_send_discoverymodes(struct fusb30x_chip *chip, u32 evt)
{
    ...
    if ((chip->val_tmp >> 1) != chip->vdm_svid_num) {
        switch (chip->vdm_send_state) {
            case 0:
                set_vdm_mesg(chip, VDM_DISCOVERY_MODES, VDM_TYPE_INIT, 0);
                chip->tx_state = 0;
                chip->vdm_send_state++;
            case 1:
                tmp = policy_send_data(chip);
                if (tmp == tx_success) {
                    ...
                    dev_warn(chip->dev, "VDM_DISCOVERY_MODES send failed\n");
                    chip->vdm_state = VDM_STATE_ERR;
                    return -EPIPE;
                }
            ...
            default:
                if (chip->val_tmp & 1) {
                    ...
                    dev_warn(chip->dev, "VDM_DISCOVERY_MODES time out\n");
                    chip->vdm_state = VDM_STATE_ERR;
                    chip->work_continue |= EVENT_WORK_CONTINUE;
                    return -ETIMEDOUT;
                }
    ...
    switch (chip->vdm_send_state) {
        ...
        case 1:
            ...
            if (tmp == tx_success) {
                ...
                dev_warn(chip->dev, "VDM_DP_STATUS_UPDATE send failed\n");
                /* disable auto_vdm_machine */
                chip->vdm_state = VDM_STATE_ERR;
                return -EPIPE;
            }
...
static void fusb_state_unattached(struct fusb30x_chip *chip, u32 evt)
{
    ...
    if ((evt & EVENT_CC) && chip->cc_state) {
        ...
        tcpm_set_polarity(chip, (chip->cc_state & CC_STATE_TOGSS_CC1) ? TYPEC_POLARITY_CC1 : TYPEC_POLARITY_CC2);
...
static void fusb_state_try_attach_set(struct fusb30x_chip *chip, enum role_mode mode)
{
    ...
    tcpm_init(chip);
    tcpm_set_cc(chip, (mode == ROLE_MODE_DFP) ? ROLE_MODE_DFP : ROLE_MODE_UFP);
    chip->timer_mux = T_PD_TRY_DRP;
    fusb_timer_start(&chip->timer_mux_machine, chip->timer_mux);
    set_state(chip, (mode == ROLE_MODE_DFP) ? attach_try_src : attach_try_snk);
}
...
static void fusb_state_attach_wait_sink(struct fusb30x_chip *chip, u32 evt)
{
    ...
    if (evt & EVENT_TIMER_MUX) {
        ...
        if (chip->debounce_cnt > N_DEBOUNCE_CNT) {
            chip->timer_mux = T_DISABLED;
            if ((chip->cc1 == TYPEC_CC_VOLT_RP && chip->cc2 == TYPEC_CC_VOLT_OPEN) || (chip->cc2 == TYPEC_CC_VOLT_RP && chip->cc1 == TYPEC_CC_VOLT_OPEN)) {
                chip->timer_mux = T_PD_SOURCE_ON;
                fusb_timer_start(&chip->timer_mux_machine, chip->timer_mux);
                set_state(chip, attached_sink);
            } else {
...
static void fusb_state_attach_wait_source(struct fusb30x_chip *chip, u32 evt)
{
    ...
    if (evt & EVENT_TIMER_MUX) {
        ...
        if (chip->debounce_cnt > N_DEBOUNCE_CNT) {
            if (((!chip->cc1) || (!chip->cc2)) && ((chip->cc1 == TYPEC_CC_VOLT_RD) || (chip->cc2 == TYPEC_CC_VOLT_RD))) {
                if (chip->role == ROLE_MODE_DRP && chip->try_role == ROLE_MODE_UFP && !chip->try_role_complete) {
                    fusb_state_try_attach_set(chip, ROLE_MODE_UFP);
                } else {
...
static void fusb_state_try_attach(struct fusb30x_chip *chip, u32 evt, enum role_mode mode)
{
    if ((evt & EVENT_CC) && chip->cc_state) {
        chip->try_role_complete = true;
        if (CC_STATE_ROLE(chip) == CC_STATE_TOGSS_IS_UFP) {
            set_state(chip, (mode == ROLE_MODE_UFP) ? attach_wait_sink : error_recovery);
        } else {
            set_state(chip, (mode == ROLE_MODE_DFP) ? attach_wait_source : error_recovery);
        }
        ...
        if (!chip->try_role_complete) {
            chip->try_role_complete = true;
            fusb_state_try_attach_set(chip, (mode == ROLE_MODE_DFP) ? ROLE_MODE_UFP : ROLE_MODE_DFP);
        } else {
...
static void fusb_state_src_transition_supply(struct fusb30x_chip *chip, u32 evt)
{
    u32 tmp;
    ...
    switch (chip->sub_state) {
        ...
        default:
            tmp = policy_send_data(chip);
            if (tmp == tx_success) {
                dev_info(chip->dev, "PD connected as DFP, supporting 5V\n");
    ...
    switch (chip->sub_state) {
        case 0:
            ...
            if (chip->notify.data_role) {
                regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_DATAROLE, SWITCHES1_DATAROLE);
            } else {
                regmap_update_bits(chip->regmap, FUSB_REG_SWITCHES1, SWITCHES1_DATAROLE, 0);
            }
...
static void fusb_state_swap_msg_process(struct fusb30x_chip *chip, u32 evt)
{
    if (evt & EVENT_RX) {
        ...
        } else if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_VCONN_SWAP)) {
            if (chip->notify.data_role) {
                set_state(chip, chip->conn_state);
            } else {
                set_state(chip, policy_vcs_ufp_evaluate_swap);
            }
        } else if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_DR_SWAP)) {
            if (chip->notify.data_role) {
...
#define VDM_IS_ACTIVE(chip) (chip->notify.data_role && chip->vdm_state < VDM_STATE_READY)
...
static void fusb_state_vcs_set_vconn(struct fusb30x_chip *chip, u32 evt, bool on)
{
    if (on) {
        tcpm_set_vconn(chip, 1);
        set_state(chip, chip->notify.data_role ? policy_vcs_dfp_send_ps_rdy : policy_vcs_ufp_send_ps_rdy);
    } else {
        tcpm_set_vconn(chip, 0);
        if (chip->notify.power_role) {
            set_state(chip, policy_src_ready);
...
static void fusb_state_vcs_wait_for_vconn(struct fusb30x_chip *chip, u32 evt)
{
    switch (chip->sub_state) {
        case 0:
        ...
        case 1:
            if (evt & EVENT_RX) {
                if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_PS_RDY)) {
                    set_state(chip, chip->notify.data_role ? policy_vcs_dfp_turn_off_vconn : policy_vcs_ufp_turn_off_vconn);
                }
            } else if (evt & EVENT_TIMER_STATE) {
                if (chip->notify.power_role) {
                    set_state(chip, policy_src_send_hardrst);
...
static void fusb_state_src_prs_transition_to_off(struct fusb30x_chip *chip, u32 evt)
{
    switch (chip->sub_state) {
        case 0:
            chip->timer_state = T_SRC_TRANSITION;
        ...
        case 2:
            if (evt & EVENT_RX) {
                if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_PS_RDY)) {
                    ...
                    dev_dbg(chip->dev, "rec careless msg: head %x\n", chip->rec_head);
                }
            } else if (evt & EVENT_TIMER_STATE) {
                chip->notify.power_role = POWER_ROLE_SOURCE;
                tcpm_set_msg_header(chip);
...
static void fusb_state_drs_evaluate(struct fusb30x_chip *chip, u32 evt)
{
    if (chip->pd_cap_info.data_role_swap)
         ...
         */
    {
        set_state(chip, chip->notify.data_role ? policy_drs_dfp_reject : policy_drs_ufp_accept);
    } else {
        set_state(chip, chip->notify.data_role ? policy_drs_dfp_reject : policy_drs_ufp_reject);
    }
}
...
{
    chip->notify.data_role = chip->notify.data_role ? DATA_ROLE_UFP : DATA_ROLE_DFP;
    tcpm_set_msg_header(chip);
    set_state(chip, chip->notify.power_role ? policy_src_ready : policy_snk_ready);
}
...
static void fusb_state_src_get_sink_cap(struct fusb30x_chip *chip, u32 evt)
{
    ...
    switch (chip->sub_state) {
        ...
        default:
            if (evt & EVENT_RX) {
                if (PACKET_IS_DATA_MSG(chip->rec_head, DMT_SINKCAPABILITIES)) {
                    for (tmp = 0;
                         tmp < PD_HEADER_CNT(chip->rec_head);
                         tmp++) {
                        chip->partner_cap[tmp] = chip->rec_load[tmp];
                    }
                    set_state(chip, policy_src_ready);
                } else {
...
{
    if (evt & EVENT_RX) {
        if (PACKET_IS_DATA_MSG(chip->rec_head, DMT_SOURCECAPABILITIES)) {
            chip->is_pd_support = true;
            chip->timer_mux = T_DISABLED;
    ...
    } else if ((evt & EVENT_TIMER_MUX) && (chip->hardrst_count > N_HARDRESET_COUNT)) {
        if (chip->is_pd_support) {
            set_state(chip, error_recovery);
        } else {
            set_state(chip, disabled);
...
static void fusb_state_snk_transition_default(struct fusb30x_chip *chip, u32 evt)
{
    switch (chip->sub_state) {
        case 0:
            chip->notify.is_pd_connected = false;
...
static void fusb_state_snk_ready(struct fusb30x_chip *chip, u32 evt)
{
    if (evt & EVENT_RX) {
        if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_GETSINKCAP)) {
            ...
            chip->tx_state = tx_idle;
            policy_send_data(chip);
        } else if (PACKET_IS_DATA_MSG(chip->rec_head, DMT_VENDERDEFINED)) {
            process_vdm_msg(chip);
            chip->work_continue |= EVENT_WORK_CONTINUE;
            chip->timer_state = T_DISABLED;
        } else if (!VDM_IS_ACTIVE(chip)) {
            ...
            if (evt & EVENT_RX) {
                if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_ACCEPT)) {
                    ...
                    if (cmd == CMT_VCONN_SWAP) {
                        set_state(chip, chip->vconn_enabled ? policy_vcs_dfp_wait_for_ufp_vconn : policy_vcs_dfp_turn_on_vconn);
                        ...
                        set_state(chip, chip->notify.data_role ? policy_drs_dfp_change : policy_drs_ufp_change);
                    }
                } else if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_REJECT) || PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_WAIT)) {
                    chip->timer_state = T_DISABLED;
                    if (chip->notify.power_role) {
                        set_state(chip, policy_src_ready);
                    } else {
    ...
    switch (chip->sub_state) {
        ...
        case 1:
            if (evt & EVENT_RX) {
                if (PACKET_IS_CONTROL_MSG(chip->rec_head, CMT_PS_RDY)) {
                    if (chip->role == ROLE_MODE_DRP) {
                        set_state(chip, policy_snk_prs_assert_rp);
                    } else {
...
static void fusb_state_snk_send_softreset(struct fusb30x_chip *chip, u32 evt)
{
    ...
    switch (chip->sub_state) {
        ...
        default:
            if (evt & EVENT_RX) {
                if ((!PD_HEADER_CNT(chip->rec_head)) && (PD_HEADER_TYPE(chip->rec_head) == CMT_ACCEPT)) {
                    fusb_soft_reset_parameter(chip);
                    chip->timer_state = T_TYPEC_SINK_WAIT_CAP;
                    fusb_timer_start(&chip->timer_state_machine, chip->timer_state);
                    set_state(chip, policy_snk_wait_caps);
...
static int fusb_initialize_gpio(struct fusb30x_chip *chip)
{
    ...
    chip->gpio_vbus_5v = devm_gpiod_get_optional(chip->dev, "vbus-5v", GPIOD_OUT_LOW);
    if (IS_ERR(chip->gpio_vbus_5v)) {
        dev_warn(chip->dev, "Could not get named GPIO for VBus5V!\n");
        chip->gpio_vbus_5v = NULL;
    } else {
        gpiod_set_value(chip->gpio_vbus_5v, 0);
    }

    chip->gpio_vbus_other = devm_gpiod_get_optional(chip->dev, "vbus-other", GPIOD_OUT_LOW);
    if (IS_ERR(chip->gpio_vbus_other)) {
        dev_warn(chip->dev, "Could not get named GPIO for VBusOther!\n");
        chip->gpio_vbus_other = NULL;
    } else {
        gpiod_set_value(chip->gpio_vbus_other, 0);
    }

    chip->gpio_discharge = devm_gpiod_get_optional(chip->dev, "discharge", GPIOD_OUT_LOW);
    if (IS_ERR(chip->gpio_discharge)) {
        dev_warn(chip->dev, "Could not get named GPIO for discharge!\n");
...
static enum hrtimer_restart fusb_timer_handler(struct hrtimer *timer)
{
    ...
    if (i != fusb30x_port_used) {
        kthread_queue_work(fusb30x_port_info[i]->irq_worker, &fusb30x_port_info[i]->irq_work);
    }
...
static void fusb_initialize_timer(struct fusb30x_chip *chip)
{
    ...
    hrtimer_init(&chip->timer_mux_machine, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
...
static int fusb30x_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (!of_property_read_string(chip->dev->of_node, "fusb302,role", (const char **)&string[0])) {
    ...
    if (chip->role == ROLE_MODE_NONE) {
        dev_warn(chip->dev, "Can't get property of role, set role to default DRP\n");
        chip->role = ROLE_MODE_DRP;
        string[0] = "ROLE_MODE_DRP";
    }

    if (!of_property_read_string(chip->dev->of_node, "fusb302,try_role", (const char **)&string[1])) {
    ...
    ret = extcon_set_property_capability(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_TYPEC_POLARITY);
    if (ret) {
        dev_err(&client->dev, "failed to set USB property capability: %d\n", ret);
        return ret;
    }

    ret = extcon_set_property_capability(chip->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_TYPEC_POLARITY);
    if (ret) {
        dev_err(&client->dev, "failed to set USB_HOST property capability: %d\n", ret);
        return ret;
    }

    ret = extcon_set_property_capability(chip->extcon, EXTCON_DISP_DP, EXTCON_PROP_USB_TYPEC_POLARITY);
    if (ret) {
        dev_err(&client->dev, "failed to set DISP_DP property capability: %d\n", ret);
        return ret;
    }

    ret = extcon_set_property_capability(chip->extcon, EXTCON_USB, EXTCON_PROP_USB_SS);
    if (ret) {
        dev_err(&client->dev, "failed to set USB USB_SS property capability: %d\n", ret);
        return ret;
    }

    ret = extcon_set_property_capability(chip->extcon, EXTCON_USB_HOST, EXTCON_PROP_USB_SS);
    if (ret) {
        dev_err(&client->dev, "failed to set USB_HOST USB_SS property capability: %d\n", ret);
        return ret;
    }

    ret = extcon_set_property_capability(chip->extcon, EXTCON_DISP_DP, EXTCON_PROP_USB_SS);
    if (ret) {
        dev_err(&client->dev, "failed to set DISP_DP USB_SS property capability: %d\n", ret);
        return ret;
    }

    ret = extcon_set_property_capability(chip->extcon, EXTCON_CHG_USB_FAST, EXTCON_PROP_USB_TYPEC_POLARITY);
    if (ret) {
        dev_err(&client->dev, "failed to set USB_PD property capability: %d\n", ret);
        return ret;
    }
    ...
    ret = request_irq(chip->gpio_int_irq, cc_interrupt_handler, IRQF_TRIGGER_LOW, "fsc_interrupt_int_n", chip);
    if (ret) {
        dev_err(&client->dev, "irq request failed\n");
        goto IRQ_ERR;
    }

    dev_info(chip->dev, "port %d probe success with role %s, try_role %s\n", chip->port_num, string[0], string[1]);
...
static const struct dev_pm_ops fusb30x_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(fusb30x_pm_suspend, fusb30x_pm_resume)
};
