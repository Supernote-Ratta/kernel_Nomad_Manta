...
int wifi_send_cmd(int portno, struct scatterlist *sg, int sg_num, int total)
{
	...
	if(!port || !port->state)
		return -EIO;
	return bulkout_write_sg(port, sg, sg_num, total);
}
/************************************************************************
 *Decription:
 *Author:jiayong.yang
...
int modem_assert(void)
{
	...
	if(port && port->state) {
		ret = usb_control_msg(port->udev, usb_sndctrlpipe(port->udev, 0),
				...
				0,0,NULL,0,1000);
		skw_usb_err("SND ASSERT CMD ret = %d\n", ret);
	}
	return ret;
}
...
static int send_modem_service_command(u16 service, u16 command)
{
	...
	if(port && port->state) {
		reinit_completion(&download_done);
		ret = usb_control_msg(port->udev, usb_sndctrlpipe(port->udev, 0),
				VENDOR_MSG_SERVICE_CTRL, USB_DIR_OUT| USB_TYPE_VENDOR|USB_RECIP_DEVICE,
				service, command, NULL, 0, 1000);
	}
	if(command == SERVICE_START) {
		wait_for_completion_interruptible_timeout(&download_done, msecs_to_jiffies(timeout + 500*service));
	} else {
...
{
	int count=100;
	while ((modem_status == MODEM_HALT)  && count--)
			msleep(10);

	skw_usb_info("Enter modem_status=%d:%d\n", modem_status, count);
...
int skw_BT_service_stop(void)
{
	...
	if (modem_status == MODEM_HALT) {
		...
		while ((modem_status == MODEM_HALT)  && count--)
			msleep(10);
		skw_usb_info("Exit modem_status=%d:%d\n", modem_status, count);
		return 0;
	}
	if (service_state_map & (1<<BT_SERVICE)){
...
int usb_port_async_entry(void *para)
{
	...
	skw_usb_info("%s %d running packet %d ...\n", __func__, port->portno, mpc);
	...
	while(port->state){
		...
		if(port->rx_submit) {
			...
			while(offset < read) {
				...
				if(size + offset > read)
					size = read  - offset;
					skw_port_log(port->portno,
						 "submit len=%d size =%d msg: 0x%x 0x%x 0x%x \n",
				...
				const struct usb_device_id *id)
{
	...
	if (!strncmp(iface_desc->string, "Boot", 4))
		dloader = 1;
	...
	if(dloader)
		...
		pdev = NULL;
		if (!strncmp(iface_desc->string, "BTDATA", 6))
			ucom_pdata.data_port = iface_desc->desc.bInterfaceNumber;
		else if (!strncmp(iface_desc->string, "BTCMD", 5))
			ucom_pdata.cmd_port = iface_desc->desc.bInterfaceNumber;
		else if (!strncmp(iface_desc->string, "BTAUDIO", 7)) {
	...
	if(!dloader) {
		...
		if (port->portno<=1) {
			sema_init(&port->sem, 0);
			if (!strncmp(iface_desc->string, "WIFIDATA", 8)) {
			...
			if(port->thread)
				wake_up_process(port->thread);
		} else if(!strncmp(iface_desc->string, "LOOP", 4)) {
			sema_init(&port->sem, 0);
	...
	if (!strncmp(iface_desc->string, "LOG", 3))
		log_port = port;
	return 0;
err0:
...
int skw_boot_loader(struct seekwave_device *boot_data)
{
	...
	skw_usb_info("status:%d , chip_en_gpio=%d ", modem_status, usb_boot_data->chip_en);
	chip_en_gpio = usb_boot_data->chip_en;
	if(chip_en_gpio > 0)
		gpio_set_value(chip_en_gpio, 1);
	if(!boot_data->first_dl_flag ){
		if (!usb_ports[0]->is_dloader) {
			usb_setup_service_devices();
