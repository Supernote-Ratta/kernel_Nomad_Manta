...
static int goodix_parse_dt(struct device_node *node, struct goodix_ts_board_data *board_data)
{
    ...
    if (!r) {
        ...
        if (strlen(name_tmp) < sizeof(board_data->iovdd_name)) {
            ...
            ts_info("invalied iovdd name length: %ld > %ld", strlen(name_tmp), sizeof(board_data->iovdd_name));
        }
    }
...
static void goodix_cmd_init(struct goodix_ts_device *dev, struct goodix_ts_cmd *ts_cmd, u8 cmds, u16 cmd_data, u32 reg_addr)
{
    u16 checksum = 0;
...
static int _do_goodix_send_config(struct goodix_ts_device *dev, struct goodix_ts_config *config)
{
    int r = 0;
    int try_times = 0;
    u8 buf[3] = {0};
    ...
    for (try_times = 0; try_times < TS_WAIT_CMD_FREE_RETRY_TIMES; try_times++) {
        if (!goodix_i2c_read(dev, command_reg, buf, 1) && buf[0] == TS_CMD_REG_READY) {
            break;
        }
        usleep_range(10000, 11000);
    }
    ...
    goodix_cmd_init(dev, &ts_cmd, COMMAND_START_SEND_CFG, 0, dev->reg.command);
    if (goodix_send_command(dev, &ts_cmd)) {
    ...
    goodix_cmd_init(dev, &ts_cmd, COMMAND_END_SEND_CFG, 0, dev->reg.command);
    if (goodix_send_command(dev, &ts_cmd)) {
    ...
    if (dev->ic_type == IC_TYPE_YELLOWSTONE) {
        ...
        for (try_times = 0; try_times < TS_WAIT_CMD_FREE_RETRY_TIMES; try_times++) {
            ...
            if (!r && (buf[0] == TS_CMD_CFG_ERR || buf[0] == TS_CMD_CFG_OK)) {
                break;
            }
            usleep_range(10000, 11000);
        }
        ...
        goodix_cmd_init(dev, &ts_cmd, COMMAND_END_SEND_CFG_YS, 0, dev->reg.command);
        if (goodix_send_command(dev, &ts_cmd)) {
        ...
        if (try_times >= TS_WAIT_CMD_FREE_RETRY_TIMES) {
            ts_err("failed send cfg, reg:0x%04x is 0x%x not 0xff", command_reg, buf[0]);
            r = -EINVAL;
            goto exit;
        }
    }
...
static int goodix_read_config_ys(struct goodix_ts_device *dev, u8 *buf, int size)
{
    ...
    if (checksum) {
        ts_err("Config head checksum err:0x%x,data:%*ph", checksum, TS_CFG_HEAD_LEN_YS, buf);
        ret = -EINVAL;
        goto err_out;
    }
    ...
    for (i = 0; i < sub_bags; i++) {
        ...
        ret = goodix_i2c_read(dev, cfg_addr + offset + 2, buf + offset + 2, subbag_len + 2);
        if (ret) {
            goto err_out;
        }
        ...
        offset += subbag_len + 4;
        ts_debug("sub bag %d, data:%*ph", buf[offset], buf[offset + 1] + 4, buf + offset);
    }
    ret = offset;
...
/* success return config length else return -1 */
static int goodix_read_config_nor(struct goodix_ts_device *dev, u8 *buf, int size)
{
    u32 cfg_addr = dev->reg.cfg_addr;
    int sub_bags = 0;
    int offset = 0;
    ...
    for (i = 0; i < sub_bags; i++) {
        ...
        ret = goodix_i2c_read(dev, cfg_addr + offset + 2, buf + offset + 2, subbag_len + 1);
        if (ret) {
            goto err_out;
        }
...
static int goodix_read_config(struct goodix_ts_device *dev, u8 *config_data, int size)
{
    struct goodix_ts_cmd ts_cmd;
    u8 cmd_flag;
...
static void goodix_parse_finger_ys(struct goodix_ts_device *dev, struct goodix_touch_data *touch_data, unsigned char *buf, int touch_num)
{
    unsigned int id = 0, x = 0, y = 0, w = 0;
    static u32 pre_finger_map;
    u32 cur_finger_map = 0;
...
static void goodix_parse_pen_ys(struct goodix_ts_device *dev, struct goodix_pen_data *pen_data, unsigned char *buf, int touch_num)
{
    ts_info("unsupported");
}

static int goodix_touch_handler_ys(struct goodix_ts_device *dev, struct goodix_ts_event *ts_event, u8 *pre_buf, u32 pre_buf_len)
{
    struct goodix_touch_data *touch_data = &ts_event->touch_data;
    struct goodix_pen_data *pen_data = &ts_event->pen_data;
    static u8 buffer[IRQ_HEAD_LEN_YS + BYTES_PER_COORD * GOODIX_MAX_TOUCH + 2];
    ...
    if (unlikely(touch_num > 1)) {
        r = goodix_i2c_read_trans(dev, dev->reg.coor + pre_buf_len, &buffer[pre_buf_len], (touch_num - 1) * BYTES_PER_COORD);
        if (unlikely(r < 0)) {
            goto exit_clean_sta;
        }
    }
    ...
    if (touch_num > 0) {
        chksum = checksum_u8_ys(&buffer[IRQ_HEAD_LEN_YS], touch_num * BYTES_PER_COORD + 2);
    ...
    if (touch_num >= 1 && (point_type == POINT_TYPE_STYLUS || point_type == POINT_TYPE_STYLUS_HOVER)) {
        /* stylus info */
        if (pre_finger_num) {
        ...
        if (pre_pen_num) {
            ...
            ts_event->event_type = EVENT_TOUCH;
            goodix_parse_finger_ys(dev, touch_data, buffer, touch_num);
...
static int goodix_touch_handler_nor(struct goodix_ts_device *dev, struct goodix_ts_event *ts_event, u8 *pre_buf, u32 pre_buf_len)
{
    struct goodix_touch_data *touch_data = &ts_event->touch_data;
    struct goodix_pen_data *pen_data = &ts_event->pen_data;
    static u8 buffer[IRQ_HEAD_LEN_NOR + BYTES_PER_COORD * GOODIX_MAX_TOUCH + 2];
    int touch_num = 0, r = -EINVAL;
...
static int goodix_event_handler(struct goodix_ts_device *dev, struct goodix_ts_event *ts_event)
{
    ...
    if (dev->ic_type == IC_TYPE_YELLOWSTONE && checksum_u8_ys(pre_buf, IRQ_HEAD_LEN_YS)) {
