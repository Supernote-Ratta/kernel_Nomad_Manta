...
static int sync_read(struct goodix_tools_dev *dev, void __user *arg)
{
    ...
    if (tools_data.length > MAX_BUF_LENGTH) {
        ts_err("buffer too long:%d > %d", tools_data.length, MAX_BUF_LENGTH);
        return -EINVAL;
    }
    ...
    ret = copy_to_user((u8 *)arg + I2C_MSG_HEAD_LEN, tools_data.data, tools_data.length);
    if (ret) {
        ret = -EFAULT;
        ts_err("Copy_to_user failed");
        goto out;
...
static int goodix_tools_module_exit(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    struct goodix_tools_dev *tools_dev = module->priv_data;
