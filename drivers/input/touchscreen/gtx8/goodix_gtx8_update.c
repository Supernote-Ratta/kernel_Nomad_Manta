...
static int goodix_check_update(struct goodix_ts_device *dev, const struct firmware_info *fw_info)
{
    ...
    if (fw_ver.valid) {
        ...
        /*if (memcmp(fw_ver.pid, fw_info->fw_pid, dev->reg.pid_len)) {
            ts_err("Product ID is not match %s != %s", fw_ver.pid, fw_info->fw_pid);
            return -EPERM;
        }*/
...
static int goodix_load_isp(struct goodix_ts_device *ts_dev, struct firmware_data *fw_data)
{
    ...
    if (ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
        r = goodix_reg_write(ts_dev, HW_REG_CPU_RUN_FROM_YS, reg_val, 8);
    } else {
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
    ...
    if (ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
        ts_dev->hw_ops->write(ts_dev, HW_REG_CPU_RUN_FROM_YS, temp_buf, 8);
    } else {
    ...
    for (retry = 0; retry < 20; retry++) {
        if (ts_dev->ic_type == IC_TYPE_YELLOWSTONE) {
            reg_val[0] = 0x00;
            r = goodix_reg_write(ts_dev, HW_REG_ESD_KEY_DIS, reg_val, 1);
            if (r < 0) {
                ts_info("failed dis esd key");
                continue;
            }
            reg_val[0] = 0x95;
            r = goodix_reg_write(ts_dev, HW_REG_ESD_KEY_EN, reg_val, 1);
            if (r < 0) {
...
static int goodix_flash_config(struct goodix_ts_device *ts_dev)
{
    ...
    if (!ts_dev || !ts_dev->cfg_bin_state || !ts_dev->normal_cfg.initialized) {
...
#define FW_UPDATE_RETRY     2
    ...
    if (fwu_ctrl->status == UPSTA_PREPARING || fwu_ctrl->status == UPSTA_UPDATING) {
...
start_update:
    ...
    if (GOODIX_FLASH_CONFIG_WITH_ISP && fwu_ctrl->mode & UPDATE_MODE_FLASH_CFG) {
 ...
 */
static ssize_t goodix_sysfs_update_en_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
    int mode = 0;
...
static ssize_t goodix_sysfs_update_progress_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct fw_update_ctrl *fw_ctrl = &goodix_fw_update_ctrl;
    return scnprintf(buf, PAGE_SIZE, "%d\n", fw_ctrl->progress);
}

static ssize_t goodix_sysfs_update_result_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *result = NULL;
    struct fw_update_ctrl *fw_ctrl = &goodix_fw_update_ctrl;
...
static ssize_t goodix_sysfs_update_fwversion_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct goodix_ts_version fw_ver;
    struct fw_update_ctrl *fw_ctrl = &goodix_fw_update_ctrl;
    int r = 0;
...
static ssize_t goodix_sysfs_fwsize_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct fw_update_ctrl *fw_ctrl = &goodix_fw_update_ctrl;
    int r = -EINVAL;
...
static ssize_t goodix_sysfs_fwsize_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct fw_update_ctrl *fw_ctrl = &goodix_fw_update_ctrl;
    struct firmware *fw;
    u8 **data;
...
static ssize_t goodix_sysfs_force_update_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    return count;
}
...
static DEVICE_ATTR(fwversion, 0444, goodix_sysfs_update_fwversion_show, NULL);
static DEVICE_ATTR(fwsize, 0664, goodix_sysfs_fwsize_show, goodix_sysfs_fwsize_store);
static DEVICE_ATTR(force_update, 0220, NULL, goodix_sysfs_force_update_store);
