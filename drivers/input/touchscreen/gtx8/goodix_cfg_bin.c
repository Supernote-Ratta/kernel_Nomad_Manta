...
static int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
    ...
    ts_info("Driver bin info: ver %s, len %d, pkgs %d", cfg_bin->head.bin_version, cfg_bin->head.bin_len, cfg_bin->head.pkg_num);
    for (i = 0; i < cfg_bin->head.pkg_num; i++) {
        ts_debug("---------------------------------------------");
...
static int goodix_get_reg_and_cfg(struct goodix_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin)
{
    ...
    for (i = 0; i < cfg_bin->head.pkg_num; i++) {
        /*get ic type*/
        if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "normandy", strlen("normandy"))) {
            ts_dev->ic_type = IC_TYPE_NORMANDY;
        } else if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "yellowstone", strlen("yellowstone"))) {
        ...
        if (cfg_bin->cfg_pkgs[i].cnst_info.cfg_type == TS_NORMAL_CFG || cfg_bin->cfg_pkgs[i].cnst_info.cfg_type == TS_HIGH_SENSE_CFG) {
