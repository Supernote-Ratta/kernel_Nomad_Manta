...
static u8 huion_change_x2y = FALSE;
static u8 huion_x_reverse = TRUE;
static u8 huion_y_reverse = TRUE;
...
//GTP_DEBUG("x,y,w %d %d %d\n", input_x, input_y, input_w);
            ...
            input_x = ((input_x*SCREEN_MAX_X)/TOUCH_MAX_X);
            input_y = ((input_y*SCREEN_MAX_Y)/TOUCH_MAX_Y);
#endif
            {
                                  ...
                                  struct huion_ts_data *ts)
{
    ...
    input_set_abs_params(ts->input_dev,
                         ABS_PRESSURE, 0, MAX_PRESS_VALUE, 0, 0);
	...
	//ts->input_dev->id.version = ((gversionbuf[12]<<24)| (gversionbuf[13]<<16)| (gversionbuf[14]<<8) | gversionbuf[15]);
    //only get the month and day cause the id.version size is 2 bytes.
...
static int huion_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(parabuf[1] != 0xc8)  //do not match huion devices
	{
		printk("do not match huion devices: %d",ret);        
		goto	probe_init_error;
	}

	client->addr = (0x11 >> 1);
	memcpy(gversionbuf,huibuf,sizeof(gversionbuf));
