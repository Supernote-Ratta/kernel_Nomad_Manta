		...
		struct cyttsp5_hid_desc *desc)
{
	...
	printk("%s: cmd[0]:0x%x,cmd[1]:0x%x \n",__func__,cmd[0],cmd[1]);
 ...
 * touch report
 */
static const struct cyttsp5_tch_abs_params tch_hdr_default[CY_TCH_NUM_HDR] = {
 ...
 * button report
 */
static const struct cyttsp5_tch_abs_params tch_abs_default[CY_TCH_NUM_ABS] = {
	/* byte offset, size, min, max, bit offset, report */
...
static int cyttsp5_core_suspend(struct device *dev)
{
    ...
    printk("%s mem_sleep_current:%d is_suspend:%d\n", __func__,mem_sleep_current,cd->is_suspend);
    if (cd->is_suspend) {
        return 0;
    }
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	parade_debug(dev, DEBUG_LEVEL_1, "%s: cyttsp5_mt_probe Completed Successfully\n",
		__func__);
	...
	parade_debug(dev, DEBUG_LEVEL_1, "%s: cyttsp5_btn_probe Completed Successfully\n",
		__func__);
	...
	parade_debug(dev, DEBUG_LEVEL_1, "%s: cyttsp5_proximity_probe Completed Successfully\n",
		__func__);

	/* Probe registered modules */
	cyttsp5_probe_modules(cd);
	parade_debug(dev, DEBUG_LEVEL_1, "%s: cyttsp5_probe_modules Completed Successfully\n",
		__func__);
...
return 0;
	...
	is_cyttsp5_probe_success = true;
	parade_debug(dev, DEBUG_LEVEL_1, "%s: TTDL Core Probe Completed Successfully\n",
		__func__);
	return 0;
