...
struct upgrade_module slide_module_list[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, slide_fw_file, sizeof(slide_fw_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, slide_fw_file2, sizeof(slide_fw_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, slide_fw_file3, sizeof(slide_fw_file3)},
};

struct upgrade_func *slide_upgrade_func_list[] = {
    &upgrade_func_ft6456,
};

struct fts_upgrade *fwupgrade_slide;
    ...
    enum FW_STATUS *fw_sts)
{
    ...
    if (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0)
        ...
        cmd_len = 2;
    ret = fts_slide_write(cmd, cmd_len);
    if (ret < 0) {
    ...
    ret = fts_slide_read(cmd, cmd_len, val, 2);
    if (ret < 0) {
...
static int fts_fwupg_reset_to_boot(struct fts_upgrade *upg)
{
    ...
    ret = fts_slide_write_reg(reg, FTS_UPGRADE_AA);
    if (ret < 0) {
    ...
    ret = fts_slide_write_reg(reg, FTS_UPGRADE_55);
    if (ret < 0) {
    ...
    u32 ecc_length)
{
    ...
    ret = fts_slide_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
        msleep(1);
        ret = fts_slide_read(cmd, 1, val, 1);
        if (ret < 0) {
    ...
    ret = fts_slide_read(cmd, 1, val, 2);
    if (ret < 0) {
...
static int fts_pram_ecc_cal_xor(void)
{
    ...
    ret = fts_slide_read_reg(FTS_ROMBOOT_CMD_ECC, &reg_val);
    if (ret < 0) {
        FTS_ERROR("read pramboot ecc fail");
        return ret;
    }
...
static int fts_pram_write_buf(struct fts_upgrade *upg, u8 *buf, u32 len)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        if ((upg->ts_data->bus_type == BUS_TYPE_SPI) && (upg->ts_data->bus_ver == BUS_VER_V2)) {
            ...
            ret = fts_slide_write(packet_buf, FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
            if (ret < 0) {
        ...
        ret = fts_slide_write(packet_buf, packet_len + cmdlen);
        if (ret < 0) {
...
static int fts_pram_init(void)
{
    ...
    ret = fts_slide_read(wbuf, 1, &reg_val, 1);
    if (ret < 0) {
...
static bool fts_fwupg_check_fw_valid(void)
{
    ...
    ret = fts_slide_wait_tp_to_valid();
    if (ret < 0) {
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_slide_fwupg_reset_in_boot(void)
{
    int ret = 0;
    u8 cmd = FTS_CMD_RESET;
...
* Return: return 0 if success, otherwise return error code
***********************************************************************/
int fts_slide_fwupg_enter_into_boot(void)
{
    int ret = 0;
    bool fwvalid = false;
    bool state = false;
    struct fts_upgrade *upg = fwupgrade_slide;
    ...
    if (fwvalid) {
        ...
        ret = fts_slide_fwupg_reset_in_boot();
        if (ret < 0) {
    ...
    int retries_delay)
{
    ...
    for (i = 0; i < retries; i++) {
        ...
        ret = fts_slide_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_slide_fwupg_erase(u32 delay)
{
    int ret = 0;
    u8 cmd = 0;
    bool flag = false;
 ...
 ***********************************************************************/
int fts_slide_fwupg_ecc_cal(u32 saddr, u32 len)
{
    int ret = 0;
    u32 i = 0;
    ...
    for (i = 0; i < packet_num; i++) {
        ...
        ret = fts_slide_write(wbuf, cmdlen);
        if (ret < 0) {
    ...
    ret = fts_slide_read(wbuf, 1, val, ecc_len);
    if (ret < 0) {
    ...
    u32 delay)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        if ((upg->ts_data->bus_type == BUS_TYPE_SPI) && (upg->ts_data->bus_ver == BUS_VER_V2)) {
            ...
            ret = fts_slide_write(packet_buf, FTS_LEN_SET_ADDR);
            if (ret < 0) {
 ...
 ***********************************************************************/
int fts_slide_flash_read_buf(u32 saddr, u8 *buf, u32 len)
{
    int ret = 0;
    u32 i = 0;
    u32 packet_number = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        if (upg->ts_data->bus_type == BUS_TYPE_I2C) {
            ...
            wbuf[2] = BYTE_OFF_8(addr);
            wbuf[3] = BYTE_OFF_0(addr);
            ret = fts_slide_write(wbuf, FTS_CMD_READ_LEN);
            if (ret < 0) {
            ...
            ret = fts_slide_read(NULL, 0, buf + offset, packet_len);
            if (ret < 0) {
            ...
            if (upg->ts_data->bus_ver == BUS_VER_V2) {
                ...
                wbuf[2] = BYTE_OFF_8(addr);
                wbuf[3] = BYTE_OFF_0(addr);
                ret = fts_slide_write(wbuf, FTS_LEN_SET_ADDR);
                if (ret < 0) {
                    FTS_ERROR("set flash address fail");
                    return ret;
                }
                ...
                wbuf[0] = FTS_CMD_READ;
                ret = fts_slide_read(wbuf, 1, buf + offset, packet_len);
                if (ret < 0) {
                ...
                wbuf[4] = BYTE_OFF_8(packet_len);
                wbuf[5] = BYTE_OFF_0(packet_len);
                ret = fts_slide_read(wbuf, FTS_CMD_READ_LEN_SPI, \
...
static int fts_flash_read(u32 addr, u8 *buf, u32 len)
{
    ...
    ret = fts_slide_fwupg_enter_into_boot();
    if (ret < 0) {
        FTS_ERROR("enter into pramboot/bootloader fail");
        goto read_flash_err;
    }

    ret = fts_slide_flash_read_buf(addr, buf, len);
    if (ret < 0) {
...
static int fts_slide_read_file_default(char *file_name, u8 **file_buf)
{
...
static int fts_slide_read_file_request_firmware(char *file_name, u8 **file_buf)
{
...
static int fts_slide_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;

    ret = fts_slide_read_file_request_firmware(file_name, file_buf);
    if (ret < 0) {
        ret = fts_slide_read_file_default(file_name, file_buf);
        if (ret < 0) {
...
int fts_slide_upgrade_bin(char *fw_name, bool force)
{
    int ret = 0;
    u32 slide_fw_file_len = 0;
    u8 *slide_fw_file_buf = NULL;
    struct fts_upgrade *upg = fwupgrade_slide;
    ...
    fts_slide_esdcheck_switch(upg->ts_data, DISABLE);

    ret = fts_slide_read_file(fw_name, &slide_fw_file_buf);
    if ((ret < 0) || (ret < FTS_MIN_LEN)) {
        FTS_ERROR("read fw bin file(%s) fail, len:%d", fw_name, ret);
        goto err_bin;
    }

    slide_fw_file_len = ret;
    FTS_INFO("fw bin file len:%d", slide_fw_file_len);
    if (force) {
        if (upg->func->force_upgrade) {
            ret = upg->func->force_upgrade(slide_fw_file_buf, slide_fw_file_len);
        } else {
        ...
        if (upg->func->lic_upgrade) {
            ret = upg->func->lic_upgrade(slide_fw_file_buf, slide_fw_file_len);
        } else {
            FTS_INFO("lic_upgrade function is null, no upgrade");
        }
#endif
        if (upg->func->upgrade) {
            ret = upg->func->upgrade(slide_fw_file_buf, slide_fw_file_len);
        } else {
...
err_bin:
    fts_slide_esdcheck_switch(upg->ts_data, ENABLE);
    fts_slide_irq_enable();
    upg->ts_data->fw_loading = 0;

    if (slide_fw_file_buf) {
        vfree(slide_fw_file_buf);
        slide_fw_file_buf = NULL;
    }
    return ret;
}

int fts_slide_enter_test_environment(bool test_state)
{
    return 0;
}
#if FTS_AUTO_LIC_UPGRADE_EN
static int fts_lic_get_vid_in_tp(u16 *vid)
{
    ...
    ret = fts_slide_read_reg(FTS_REG_VENDOR_ID, &val[0]);
    if (fts_data->ic_info.is_incell)
        ret = fts_slide_read_reg(FTS_REG_MODULE_ID, &val[1]);
    if (ret < 0) {
...
static int fts_lic_get_ver_in_tp(u8 *ver)
{
    ...
    ret = fts_slide_read_reg(FTS_REG_LIC_VER, ver);
    if (ret < 0) {
...
static int fts_param_get_ver_in_tp(u8 *ver)
{
    ...
    ret = fts_slide_read_reg(FTS_REG_IDE_PARA_VER_ID, ver);
    if (ret < 0) {
...
static int fts_param_ide_in_tp(u8 *val)
{
    ...
    ret = fts_slide_read_reg(FTS_REG_IDE_PARA_STATUS, val);
    if (ret < 0) {
...
static int fts_fwupg_get_ver_in_tp(u8 *ver)
{
    ...
    ret = fts_slide_read_reg(FTS_REG_FW_VER, ver);
    if (ret < 0) {
 ...
 * Return: return 0 if success, otherwise return error code
 ***********************************************************************/
int fts_slide_fwupg_upgrade(struct fts_upgrade *upg)
{
    int ret = 0;
...
static void fts_fwupg_auto_upgrade(struct fts_upgrade *upg)
{
    ...
    ret = fts_slide_fwupg_upgrade(upg);
    if (ret < 0)
...
static int fts_fwupg_get_vendorid(struct fts_upgrade *upg, int *vid)
{
    ...
    fwvalid = fts_fwupg_check_fw_valid();
    if (fwvalid) {
        ret = fts_slide_read_reg(FTS_REG_VENDOR_ID, &vendor_id);
        if (upg->ts_data->ic_info.is_incell)
            ret = fts_slide_read_reg(FTS_REG_MODULE_ID, &module_id);
    } else {
        if (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_1) {
            ret = fts_slide_read(&cmd, 1, cfgbuf, FTS_HEADER_LEN);
        } else {
...
int fts_slide_fw_recovery(void)
{
    return 0;
}

int fts_slide_enter_gesture_fw(void)
{
    return 0;
}
int fts_slide_enter_normal_fw(void)
{
    return 0;
}
...
static int fts_fwupg_get_module_info(struct fts_upgrade *upg)
{
    int ret = 0;
    int i = 0;
    struct upgrade_module *info = &slide_module_list[0];
...
static int fts_get_slide_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    int ret = 0;
    const struct firmware *fw = NULL;
    u8 *tmpbuf = NULL;
...
static int fts_get_slide_fw_file_via_i(struct fts_upgrade *upg)
{
...
/*****************************************************************************
 *  Name: fts_fwupg_get_slide_fw_file
 ...
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
static int fts_fwupg_get_slide_fw_file(struct fts_upgrade *upg)
{
    int ret = 0;
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        ret = fts_get_slide_fw_file_via_request_firmware(upg);
        if (ret != 0) {
            get_fw_i_flag = true;
        }
    } else {
    ...
    if (get_fw_i_flag) {
        ret = fts_get_slide_fw_file_via_i(upg);
    }
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    /* get fw */
    ret = fts_fwupg_get_slide_fw_file(upg);
    if (ret < 0) {
...
int fts_slide_fwupg_init(struct fts_slide_ts_data *ts_data)
{
    int i = 0;
    int j = 0;
    u16 ic_stype = 0;
    struct upgrade_func *func = slide_upgrade_func_list[0];
    int func_count = sizeof(slide_upgrade_func_list) / sizeof(slide_upgrade_func_list[0]);
    ...
    fwupgrade_slide = (struct fts_upgrade *)kzalloc(sizeof(*fwupgrade_slide), GFP_KERNEL);
    if (NULL == fwupgrade_slide) {
    ...
    if (1 == func_count) {
        fwupgrade_slide->func = func;
    } else {
        for (i = 0; i < func_count; i++) {
            func = slide_upgrade_func_list[i];
    ...
    if (NULL == fwupgrade_slide->func) {
        FTS_ERROR("no upgrade function match, can't upgrade");
        kfree(fwupgrade_slide);
        fwupgrade_slide = NULL;
        return -ENODATA;
    }

    fwupgrade_slide->ts_data = ts_data;
...
int fts_slide_fwupg_exit(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    cancel_work_sync(&ts_data->fwupg_work);

    if (fwupgrade_slide) {
        if (fwupgrade_slide->fw_from_request) {
            vfree(fwupgrade_slide->fw);
            fwupgrade_slide->fw = NULL;
        }
