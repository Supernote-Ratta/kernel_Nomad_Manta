...
static int fts_ex_mode_switch(enum _ex_mode mode, u8 value)
{
    ...
    switch (mode) {
    case MODE_GLOVE:
        ret = fts_slide_write_reg(FTS_REG_GLOVE_MODE_EN, m_val);
        if (ret < 0) {
            FTS_ERROR("MODE_GLOVE switch to %d fail", m_val);
        }
        break;
    case MODE_COVER:
        ret = fts_slide_write_reg(FTS_REG_COVER_MODE_EN, m_val);
        if (ret < 0) {
            FTS_ERROR("MODE_COVER switch to %d fail", m_val);
        }
        break;
    case MODE_CHARGER:
        ret = fts_slide_write_reg(FTS_REG_CHARGER_MODE_EN, m_val);
        if (ret < 0) {
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_slide_read_reg(FTS_REG_GLOVE_MODE_EN, &val);
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_slide_read_reg(FTS_REG_COVER_MODE_EN, &val);
