...
*         -Exx - error
*****************************************************************************/
int fts_slide_gesture_readdata(struct fts_slide_ts_data *ts_data, u8 *touch_buf)
{
    int ret = 0;
    int i = 0;
    int index = 0;
    ...
    if (ts_data->gesture_bmode == GESTURE_BM_TOUCH) {
        ...
        ret = fts_slide_read(&buf[2], 1, &buf[2], FTS_GESTURE_DATA_LEN - 2);
        if (ret < 0) {
...
void fts_slide_gesture_recovery(struct fts_slide_ts_data *ts_data)
{
    if (ts_data->gesture_support && ts_data->suspended) {
        ...
        fts_slide_write_reg(0xD1, 0xFF);
        fts_slide_write_reg(0xD2, 0xFF);
        fts_slide_write_reg(0xD5, 0xFF);
        fts_slide_write_reg(0xD6, 0xFF);
        fts_slide_write_reg(0xD7, 0xFF);
        fts_slide_write_reg(0xD8, 0xFF);
        fts_slide_write_reg(FTS_REG_GESTURE_EN, ENABLE);
    }
}
...
int fts_slide_gesture_suspend(struct fts_slide_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        ...
        fts_slide_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        msleep(1);
        fts_slide_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
    }
...
int fts_slide_gesture_resume(struct fts_slide_ts_data *ts_data)
{
    ...
    for (i = 0; i < 5; i++) {
        fts_slide_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        msleep(1);
        fts_slide_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
