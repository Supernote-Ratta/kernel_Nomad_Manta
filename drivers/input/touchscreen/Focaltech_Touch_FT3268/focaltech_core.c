...
*****************************************************************************/
enum slide_tch_abs {	/* for ordering within the extracted touch data array */
	SL_TCH_X,	/* X */
	SL_TCH_Y,	/* Y */
	SL_TCH_P,	/* P (Z) */
	SL_TCH_T,	/* TOUCH ID */
	SL_TCH_E,	/* EVENT ID */
	SL_TCH_O,	/* OBJECT ID */
	SL_TCH_TIP,	/* OBJECT ID */
	SL_TCH_MAJ,	/* TOUCH_MAJOR */
	SL_TCH_MIN,	/* TOUCH_MINOR */
	SL_TCH_OR,	/* ORIENTATION */
	SL_TCH_NUM_ABS,
};
...
struct fts_slide_ts_data *fts_slide_data;
int ftslider[SL_TCH_NUM_ABS];
/*****************************************************************************
* Static function prototypes
*****************************************************************************/
static int fts_ts_suspend(struct device *dev);
static int fts_ts_resume(struct device *dev);
extern int slider_left,slider_right; //after slider down 100ms,record l_x r_x
extern int slider_left_down, slider_right_down;
extern int t_lr[];
extern int ratta_mt_clean_attime(int left_right);
extern int ratta_mt_record(int type, bool record, int track, int tch[], unsigned long jiffs);

static void fts_slide_down_left_work(struct work_struct *work)
{
	struct fts_slide_ts_data *ts_data = container_of(work,
			struct fts_slide_ts_data, down_left_delay_work.work);

	printk("%s: slider_left_down=%d\n", __func__, slider_left_down);
	slider_left_down = 0;
	ratta_mt_clean_attime(1);
	ts_data->l_x_l = 0;
	ts_data->l_x_h = 0;
	ts_data->l_x = 0;
	slider_left = ts_data->l_x;

}
static void fts_slide_down_right_work(struct work_struct *work)
{
	struct fts_slide_ts_data *ts_data = container_of(work,
			struct fts_slide_ts_data, down_right_delay_work.work);
	...
	ts_data->r_x_h = 0;
	ts_data->r_x = 0;
	slider_right = ts_data->r_x;
}

int fts_slide_check_cid(struct fts_slide_ts_data *ts_data, u8 id_h)
{
    int i = 0;
...
static void fts_get_version(struct fts_slide_ts_data *ts_data)
{

	u8 fwver = 0;
	int ret = 0;

	ret = fts_slide_read_reg(FTS_REG_FW_VER, &fwver);
...
*****************************************************************************/
int fts_slide_wait_tp_to_valid(void)
{
    int ret = 0;
    int cnt = 0;
    u8 idh = 0;
    struct fts_slide_ts_data *ts_data = fts_slide_data;
    u8 chip_idh = ts_data->ic_info.ids.chip_idh;

    do {
        ret = fts_slide_read_reg(FTS_REG_CHIP_ID, &idh);
        if ((idh == chip_idh) || (fts_slide_check_cid(ts_data, idh) == 0)) {
...
*  Return:
*****************************************************************************/
void fts_slide_tp_state_recovery(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    /* wait tp stable */
    fts_slide_wait_tp_to_valid();
    /* recover TP charger state 0x8B */
    /* recover TP glove state 0xC0 */
    /* recover TP cover state 0xC1 */
    fts_slide_ex_mode_recovery(ts_data);
    /* recover TP gesture state 0xD0 */
    fts_slide_gesture_recovery(ts_data);
    FTS_FUNC_EXIT();
}

int fts_slide_reset_proc(int hdelayms)
{
    FTS_DEBUG("tp reset");
    gpio_direction_output(fts_slide_data->pdata->reset_gpio, 0);
    msleep(1);
    gpio_direction_output(fts_slide_data->pdata->reset_gpio, 1);
    if (hdelayms) {
        msleep(hdelayms);
    }
...
void fts_slide_irq_disable(void)
{
    unsigned long irqflags;

    FTS_FUNC_ENTER();
    spin_lock_irqsave(&fts_slide_data->irq_lock, irqflags);

    if (!fts_slide_data->irq_disabled) {
        disable_irq_nosync(fts_slide_data->irq);
        fts_slide_data->irq_disabled = true;
    }

    spin_unlock_irqrestore(&fts_slide_data->irq_lock, irqflags);
    FTS_FUNC_EXIT();
}

void fts_slide_irq_enable(void)
{
    ...
    if (fts_slide_data->irq_disabled) {
        enable_irq(fts_slide_data->irq);
        fts_slide_data->irq_disabled = false;
    }
...
void fts_slide_hid2std(void)
{
    int ret = 0;
    u8 buf[3] = {0xEB, 0xAA, 0x09};

    if (fts_slide_data->bus_type != BUS_TYPE_I2C)
        return;

    ret = fts_slide_write(buf, 3);
    if (ret < 0) {
        FTS_ERROR("hid2std cmd write fail");
    } else {
        msleep(10);
        buf[0] = buf[1] = buf[2] = 0;
        ret = fts_slide_read(NULL, 0, buf, 3);
        if (ret < 0) {
...
static int fts_match_cid(struct fts_slide_ts_data *ts_data,
...
static int fts_get_chip_types(
    struct fts_slide_ts_data *ts_data,
...
static int fts_slide_read_bootid(struct fts_slide_ts_data *ts_data, u8 *id)
{
    int ret = 0;
    u8 chip_id[2] = { 0 };
    u8 id_cmd[4] = { 0 };
    ...
    ret = fts_slide_read(id_cmd, id_cmd_len, chip_id, 2);
...
*****************************************************************************/
static int fts_get_ic_information(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
    int cnt = 0;
    u8 chip_id[2] = { 0 };
    ...
    do {
        ret = fts_slide_read_reg(FTS_REG_CHIP_ID, &chip_id[0]);
        ret = fts_slide_read_reg(FTS_REG_CHIP_ID2, &chip_id[1]);
        if ((ret < 0) || (0x0 == chip_id[0]) || (0x0 == chip_id[1])) {
    ...
    if ((cnt * INTERVAL_READ_REG) >= TIMEOUT_READ_REG) {
        for (cnt = 0; cnt < 3; cnt++) {
            ...
            ret = fts_slide_read_bootid(ts_data, &chip_id[0]);
            if (ret <  0) {
...
void fts_slide_release_all_finger(void)
{
    struct fts_slide_ts_data *ts_data = fts_slide_data;
...
*****************************************************************************/
static int fts_input_report_key(struct fts_slide_ts_data *ts_data, struct ts_event *kevent)
{
    int i = 0;
    int x = kevent->x;
    int y = kevent->y;
...
#if FTS_MT_PROTOCOL_B_EN
static int fts_input_report_b(struct fts_slide_ts_data *ts_data, struct ts_event *events)
{
    int i = 0;
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
...
#if 1
        ftslider[SL_TCH_Y] = events[i].x;
		ftslider[SL_TCH_X] = events[i].y;
        ftslider[SL_TCH_T] = events[i].id;
        ftslider[SL_TCH_TIP] = EVENT_DOWN(events[i].flag);
        if(ftslider[SL_TCH_Y]==1)
            ftslider[SL_TCH_Y] = 30;
        else if(ftslider[SL_TCH_Y]==99)
            ftslider[SL_TCH_Y] = 70;
		t = ftslider[SL_TCH_T];
#endif
                FTS_DEBUG("[B]P%d(%d, %d)[p:%d,tm:%d] flag:%d down?:%d!",
                          events[i].id, events[i].x, events[i].y,
                          events[i].p, events[i].area,events[i].flag,EVENT_DOWN(events[i].flag));
...
#endif
		if(ftslider[SL_TCH_T]>3){
			t_trace = ftslider[SL_TCH_T]-4;
		}else{
			t_trace = ftslider[SL_TCH_T];
		}
        if(ftslider[SL_TCH_Y]==30){
			for(k=(8*t_trace);k<((8*t_trace)+8);k++){
				if(ftslider[SL_TCH_T]<4){
					ts_data->l_x_l &= ~(1<<k);
				}else{
					ts_data->l_x_h &= ~(1<<k);
				}
			...
			if(ftslider[SL_TCH_T]<4){
				ts_data->l_x_l += (ftslider[SL_TCH_X]<<(8*t_trace));
			}
			else{
				ts_data->l_x_h += (ftslider[SL_TCH_X]<<(8*t_trace));
			}
			ts_data->l_x = 0;
			m = 0;
			for(l=0;l<4;l++){
				if(ts_data->l_x_l & (0xff<<8*l)){
					ts_data->l_x += (((ts_data->l_x_l & (0xff<<8*l))>>8*l)<<8*m);
					m++;
				}
			}
			for(l=0;l<4;l++){
				if(ts_data->l_x_h & (0xff<<8*l)){
					ts_data->l_x += (((ts_data->l_x_h & (0xff<<8*l))>>8*l)<<8*m);
					m++;
				}
			}
#if 0
			if(l_x_pre == 0)
				schedule_delayed_work(&ts_data->filter_left_delay_work, msecs_to_jiffies(100));
#endif
			cancel_delayed_work(&ts_data->down_left_delay_work);
			schedule_delayed_work(&ts_data->down_left_delay_work, msecs_to_jiffies(100));
			slider_left_down = 1;
			//t_lr[ftslider[SL_TCH_T]]=TRACE_LEFT;
            printk("=====%s ts_data->l_x:%lx\n",__func__,ts_data->l_x);
		}else if(ftslider[SL_TCH_Y]==70){
    			for(k=(8*t_trace);k<((8*t_trace)+8);k++){
    				if(ftslider[SL_TCH_T]<4){
    					ts_data->r_x_l &= ~(1<<k);
    				}else{
    			...
    			if(ftslider[SL_TCH_T]<4){
    				ts_data->r_x_l += (ftslider[SL_TCH_X]<<(8*t_trace));
    			}
    			else{
    				ts_data->r_x_h += (ftslider[SL_TCH_X]<<(8*t_trace));
    			}
    			ts_data->r_x = 0;
    			m = 0;
    			for(l=0;l<4;l++){
    				if(ts_data->r_x_l & (0xff<<8*l)){
    					ts_data->r_x += (((ts_data->r_x_l & (0xff<<8*l))>>8*l)<<8*m);
    					m++;
    				}
    			}
    			for(l=0;l<4;l++){
    				if(ts_data->r_x_h & (0xff<<8*l)){
    					ts_data->r_x += (((ts_data->r_x_h & (0xff<<8*l))>>8*l)<<8*m);
    					m++;
    				}
    			}
#if 0
    			if(r_x_pre == 0)
    				schedule_delayed_work(&ts_data->filter_right_delay_work, msecs_to_jiffies(100));
#endif
    			cancel_delayed_work(&ts_data->down_right_delay_work);
    			schedule_delayed_work(&ts_data->down_right_delay_work, msecs_to_jiffies(100));
    			slider_right_down = 1;
    			//t_lr[ftslider[SL_TCH_T]]=TRACE_RIGHT;
                printk("=====%s ts_data->r_x:%lx\n",__func__,ts_data->r_x);
    		}
			...
			if(ts_data->ft_mode == 0){
			if((ftslider[SL_TCH_Y] == 30)||(ftslider[SL_TCH_Y] == 70)){ 
				//240624 Sometimes,new touch is not left or right,do nothing
				//240710 slider x > 240 ,not report down
				if((ftslider[SL_TCH_X]>240)&&(ftslider[SL_TCH_TIP] != 1)){
				}else{
					ratta_mt_record(0,1,t,ftslider,jiffies);
				}
			}
		}	
        } else {
        ...
        #endif
        #if 1
        printk("=====%s tip=0 t:%d X:%x Y:%x\n",__func__,ftslider[SL_TCH_T],ftslider[SL_TCH_X],ftslider[SL_TCH_Y]);
			if(((t_lr[ftslider[SL_TCH_T]]==TRACE_LEFT)&&(ftslider[SL_TCH_Y]!=30))||((t_lr[ftslider[SL_TCH_T]]==TRACE_RIGHT)&&(ftslider[SL_TCH_Y]!=70))){
				if(t_lr[ftslider[SL_TCH_T]]==TRACE_LEFT){
					for(k=(8*t_trace);k<((8*t_trace)+8);k++){
						if(ftslider[SL_TCH_T]<4){
							ts_data->l_x_l &= ~(1<<k);
						}else{
						...
						if((ts_data->l_x_l+ts_data->l_x_h)==0){
							if (ts_data->log_level >= 2) FTS_DEBUG("=====%s cancel_delayed_work filter_left\n",__func__);
							cancel_delayed_work(&ts_data->filter_left_delay_work);
							ts_data->l_x = 0;
							slider_left = ts_data->l_x;
					...
					ftslider[SL_TCH_Y] = 30;
					//t_lr[ftslider[SL_TCH_T]]=TRACE_NONE;
					printk("=====%s tip=0 slider_left:%lx\n",__func__,slider_left);
				}else if(t_lr[ftslider[SL_TCH_T]]==TRACE_RIGHT){
					for(k=(8*t_trace);k<((8*t_trace)+8);k++){
						if(ftslider[SL_TCH_T]<4){
							ts_data->r_x_l &= ~(1<<k);
						}else{
						...
						if((ts_data->r_x_l+ts_data->r_x_h)==0){
							if (ts_data->log_level >= 2) FTS_DEBUG("=====%s cancel_delayed_work filter_right\n",__func__);
							cancel_delayed_work(&ts_data->filter_right_delay_work);
							ts_data->r_x = 0;
							slider_right = ts_data->r_x;
						}else if(slider_right != 0){
							ts_data->r_x = 0;
							m = 0;
							for(l=0;l<4;l++){
								if(ts_data->r_x_l & (0xff<<8*l)){
				...
				for(k=(8*t_trace);k<((8*t_trace)+8);k++){
					...
					if((ts_data->l_x_l+ts_data->l_x_h)==0){
                        ...
                        FTS_DEBUG("=====%s cancel_delayed_work filter_left\n",__func__);
						cancel_delayed_work(&ts_data->filter_left_delay_work);
						ts_data->l_x = 0;
						slider_left = ts_data->l_x;
					}else if(slider_left != 0){
			...
			//ts_data->num_prv_rec = ts_data->touch_event_num;
			//0: report key 1:report x,y 2:report null
			if(ts_data->ft_mode == 0){
				ratta_mt_record(0,1,t,ftslider,jiffies);
			}
			if((ts_data->ft_mode == 0)||(ts_data->ft_mode == 2)){
...
#else
static int fts_input_report_a(struct fts_slide_ts_data *ts_data, struct ts_event *events)
{
    int i = 0;
...
#if FTS_PEN_EN
static int fts_input_pen_report(struct fts_slide_ts_data *ts_data, u8 *pen_buf)
{
...
static int fts_slide_read_touchdata_spi(struct fts_slide_ts_data *ts_data, u8 *buf)
{
    int ret = 0;

    ts_data->touch_addr = 0x01;
    ret = fts_slide_read(&ts_data->touch_addr, 1, buf, ts_data->touch_size);
...
static int fts_slide_read_touchdata_i2c(struct fts_slide_ts_data *ts_data, u8 *buf)
{
    int ret = 0;
...
static int fts_slide_read_parse_touchdata(struct fts_slide_ts_data *ts_data, u8 *touch_buf)
{
    int ret = 0;
    u8 gesture_en = 0xFF;
    ...
    if (ts_data->bus_type == BUS_TYPE_SPI)
        ret = fts_slide_read_touchdata_spi(ts_data, touch_buf);
    else if (ts_data->bus_type == BUS_TYPE_I2C)
        ret = fts_slide_read_touchdata_i2c(ts_data, touch_buf);
    ...
    if (ts_data->suspended && ts_data->gesture_support) {
        ret = fts_slide_read_reg(FTS_REG_GESTURE_EN, &gesture_en);
...
static int fts_irq_read_report(struct fts_slide_ts_data *ts_data)
{
    int i = 0;
    ...
    touch_etype = fts_slide_read_parse_touchdata(ts_data, touch_buf);
    switch (touch_etype) {
    case TOUCH_DEFAULT:
    ...
    case TOUCH_GESTURE:
        if (0 == fts_slide_gesture_readdata(ts_data, touch_buf)) {
            FTS_INFO("succuss to get gesture data in irq handler");
        }
        break;

    case TOUCH_FW_INIT:
        fts_slide_release_all_finger();
        fts_slide_tp_state_recovery(ts_data);
        break;
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    fts_slide_prc_queue_work(ts_data);
...
static int fts_irq_registration(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
...
#if FTS_PEN_EN
static int fts_input_pen_init(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
...
static int fts_input_init(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
    int key_num = 0;
    struct fts_ts_platform_data *pdata = ts_data->pdata;
...
static int fts_buffer_init(struct fts_slide_ts_data *ts_data)
{
...
*****************************************************************************/
#if FTS_PINCTRL_EN
static int fts_pinctrl_init(struct fts_slide_ts_data *ts)
{
    int ret = 0;
...
static int fts_pinctrl_select_normal(struct fts_slide_ts_data *ts)
{
    int ret = 0;
...
static int fts_pinctrl_select_suspend(struct fts_slide_ts_data *ts)
{
    int ret = 0;
...
static int fts_pinctrl_select_release(struct fts_slide_ts_data *ts)
{
    int ret = 0;
...
static int fts_power_source_ctrl(struct fts_slide_ts_data *ts_data, int enable)
{
    int ret = 0;
...
*****************************************************************************/
static int fts_power_source_init(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
...
static int fts_power_source_exit(struct fts_slide_ts_data *ts_data)
{
#if FTS_PINCTRL_EN
...
static int fts_power_source_suspend(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
...
static int fts_power_source_resume(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
...
static int fts_gpio_configure(struct fts_slide_ts_data *data)
{
    int ret = 0;
...
static int fts_bus_init(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    fts_slide_esdcheck_suspend(ts_data);

    if (ts_data->gesture_support) {
        fts_slide_gesture_suspend(ts_data);
    } else {
        //fts_slide_irq_disable();

        FTS_INFO("make TP enter into sleep mode");
        ret = fts_slide_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP);
        if (ret < 0)
...
#endif
        fts_slide_reset_proc(200);
    }

    fts_slide_wait_tp_to_valid();
    fts_slide_ex_mode_recovery(ts_data);

    fts_slide_esdcheck_resume(ts_data);

    if (ts_data->gesture_support) {
        fts_slide_gesture_resume(ts_data);
    } else {
        //fts_slide_irq_enable();
    }
...
{
    struct fts_slide_ts_data *ts_data = container_of(work, struct fts_slide_ts_data, resume_work);
...
static int drm_check_dt(struct fts_slide_ts_data *ts_data)
{
    int i = 0;
    int count = 0;
...
{
    struct fts_slide_ts_data *ts_data = container_of(self, struct fts_slide_ts_data, fb_notif);
    struct fb_event *evdata = v;
    FTS_FUNC_ENTER();
    if (ts_data && v) {
        FTS_INFO("notifier,event=:%lu \n",event);
...
static int fts_notifier_callback_init(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
    FTS_FUNC_ENTER();
		...
		ret = htfy_ebc_register_notifier(&ts_data->fb_notif);
		//ret = ebc_register_notifier(&ts_data->fb_notif);
		if (ret) {
...
static int fts_notifier_callback_exit(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
#if IS_ENABLED(CONFIG_FB)
		if (ebc_unregister_notifier(&ts_data->fb_notif))
...
int fts_slide_ts_probe_entry(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
    ...
    ret = fts_slide_create_apk_debug_channel(ts_data);
    if (ret) {
        FTS_ERROR("create apk debug node fail");
    }

    ret = fts_slide_create_sysfs(ts_data);
    if (ret) {
        FTS_ERROR("create sysfs node fail");
    }

    ret = fts_slide_point_report_check_init(ts_data);
    if (ret) {
        FTS_ERROR("init point report check fail");
    }

    ret = fts_slide_ex_mode_init(ts_data);
    if (ret) {
        FTS_ERROR("init glove/cover/charger fail");
    }

    ret = fts_slide_gesture_init(ts_data);
    if (ret) {
        FTS_ERROR("init gesture fail");
    }


    ret = fts_slide_esdcheck_init(ts_data);
    if (ret) {
    ...
    ret = fts_slide_fwupg_init(ts_data);
    if (ret) {
	...
	//INIT_DELAYED_WORK(&ts_data->filter_left_delay_work, fts_slide_filter_left_work);
	//INIT_DELAYED_WORK(&ts_data->filter_right_delay_work, fts_slide_filter_right_work);
	INIT_DELAYED_WORK(&ts_data->down_left_delay_work, fts_slide_down_left_work);
	INIT_DELAYED_WORK(&ts_data->down_right_delay_work, fts_slide_down_right_work);
    FTS_FUNC_EXIT();
    return 0;
...
int fts_slide_ts_remove_entry(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    cancel_work_sync(&fts_slide_data->resume_work);
    fts_slide_point_report_check_exit(ts_data);
    fts_slide_release_apk_debug_channel(ts_data);
    fts_slide_remove_sysfs(ts_data);
    fts_slide_ex_mode_exit(ts_data);

    fts_slide_fwupg_exit(ts_data);


    fts_slide_esdcheck_exit(ts_data);

    fts_slide_gesture_exit(ts_data);
