...
int fts_slide_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    if (!ts_data || !ts_data->client || !data || !datalen
        ...
        FTS_ERROR("fts_slide_data/client/cmdlen(%d)/data/datalen(%d) is invalid",
                  cmdlen, datalen);
        return -EINVAL;
    }
...
{
    return fts_slide_read(&addr, 1, value, 1);
}
...
int fts_slide_write_reg(u8 addr, u8 value)
{
    ...
    buf[0] = addr;
    buf[1] = value;
    return fts_slide_write(buf, sizeof(buf));
}
...
{
    int ret = 0;
    struct fts_slide_ts_data *ts_data = NULL;
    ...
    ts_data = (struct fts_slide_ts_data *)kzalloc(sizeof(*ts_data), GFP_KERNEL);
    if (!ts_data) {
        FTS_ERROR("allocate memory for fts_slide_data fail");
        return -ENOMEM;
    }
    ...
    ret = fts_slide_ts_probe_entry(ts_data);
    if (ret) {
...
{
    struct fts_slide_ts_data *ts_data = i2c_get_clientdata(client);
    FTS_INFO("Touch Screen(I2C BUS) driver remove...");
    if (ts_data) {
        fts_slide_ts_remove_entry(ts_data);
        i2c_set_clientdata(client, NULL);
        kfree_safe(ts_data);
    }
    return 0;
...
static const struct of_device_id fts_dt_match[] = {
    {.compatible = "focaltech,fts_slider", },
    {},
};
