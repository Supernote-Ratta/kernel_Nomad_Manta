...
#define PROC_NAME                               "ftxxxxslider-debug"
    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    struct fts_slide_ts_data *ts_data = PDE_DATA(file_inode(filp));
    ...
    case PROC_SET_TEST_FLAG:
        ...
        if (writebuf[1] == 0) {
            fts_slide_esdcheck_switch(ts_data, ENABLE);
        } else {
            fts_slide_esdcheck_switch(ts_data, DISABLE);
        }
        break;
    ...
    case PROC_WRITE_REGISTER:
        ret = fts_slide_write_reg(writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        if (ts_data->bus_type == BUS_TYPE_I2C) {
            ret = fts_slide_write(writebuf + 1, writelen);
            if (ret < 0) {
    ...
    case PROC_SET_SLAVE_ADDR:
        if (ts_data->bus_type == BUS_TYPE_I2C) {
            fts_slide_bus_configure(ts_data, &writebuf[1], buflen - 1);
        }
        break;
    ...
    case PROC_ENTER_TEST_ENVIRONMENT:
        ...
        if (0 == writebuf[1]) {
            fts_slide_enter_test_environment(0);
        } else {
            fts_slide_enter_test_environment(1);
        }
        break;
    ...
    case PROC_WRITE_DATA_DIRECT:
        writelen = buflen - 1;
        ret = fts_slide_bus_transfer_direct(writebuf + 1, writelen, NULL, 0);
        if (ret < 0) {
    ...
    struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = fts_slide_read_reg(proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        if (ts_data->bus_type == BUS_TYPE_SPI)
            ret = fts_slide_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        else if (ts_data->bus_type == BUS_TYPE_I2C)
            ret = fts_slide_read(NULL, 0, readbuf, num_read_chars);
    ...
    case PROC_READ_DATA_DIRECT:
        num_read_chars = buflen;
        ret = fts_slide_bus_transfer_direct(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
{
    struct fts_slide_ts_data *ts_data = PDE_DATA(inode);
...
int fts_slide_create_apk_debug_channel(struct fts_slide_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
    ...
    ts_data->proc_ta.proc_entry = proc_create_data("fts_slider_ta", 0777, NULL, \
...
void fts_slide_release_apk_debug_channel(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
...
{
    struct fts_slide_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts_slide_read_reg(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts_slide_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts_slide_write_reg(reg, val);
            } else {
                rw_op.res = fts_slide_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_slide_upgrade_bin(fwname, 0);
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_slide_upgrade_bin(fwname, 1);
    mutex_unlock(&input_dev->mutex);
...
static ssize_t fts_slide_l_x_show(struct device *dev,
...
static ssize_t fts_slide_r_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fts_slide_ts_data *ts_data = dev_get_drvdata(dev);
	struct input_dev *input_dev = ts_data->input_dev;
...
static ssize_t fts_slide_ft_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fts_slide_ts_data *ts_data = dev_get_drvdata(dev);
	struct input_dev *input_dev = ts_data->input_dev;
...
static ssize_t fts_slide_ft_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (retval < 0) {
		...
		goto fts_slide_ft_mode_error_exit;
	}
...
static DEVICE_ATTR(fts_slider_ta_mode, S_IRUGO | S_IWUSR, fts_tamode_show, fts_tamode_store);
static DEVICE_ATTR(l_x, S_IRUSR | S_IWUSR, fts_slide_l_x_show, NULL);
static DEVICE_ATTR(r_x, S_IRUSR | S_IWUSR, fts_slide_r_x_show, NULL);
static DEVICE_ATTR(ft_mode, S_IRUSR | S_IWUSR, fts_slide_ft_mode_show,fts_slide_ft_mode_store);
...
static struct attribute *fts_attributes[] = {
    ...
    &dev_attr_fts_slider_ta_mode.attr,
    &dev_attr_l_x.attr,
	&dev_attr_r_x.attr,
	&dev_attr_ft_mode.attr,
    NULL
};
