...
int idc_esdcheck_lcderror(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
    u8 val = 0;
...
static int fts_esdcheck_tp_reset(struct fts_slide_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
...
static bool get_chip_id(struct fts_slide_ts_data *ts_data)
{
    int ret = 0;
    int i = 0;
    u8 idh = 0;
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (ret < 0) {
            ...
            if ((idh == chip_id) || (fts_slide_check_cid(ts_data, idh) == 0)) {
                break;
            } else {
...
*****************************************************************************/
static bool get_flow_cnt(struct fts_slide_ts_data *ts_data)
{
    int     ret = 0;
    u8      reg_value = 0;
    u8      reg_addr = 0;
    ...
    ret = fts_slide_read(&reg_addr, 1, &reg_value, 1);
    if (ret < 0) {
...
static int esdcheck_algorithm(struct fts_slide_ts_data *ts_data)
{
    int     ret = 0;
    u8      reg_value = 0;
    u8      reg_addr = 0;
    ...
    ret = fts_slide_read_reg(reg_addr, &reg_value);
    if ( ret < 0 ) {
        fts_esdcheck_data.nack_cnt++;
...
static void esdcheck_func(struct work_struct *work)
{
    struct fts_slide_ts_data *ts_data = container_of(work,
                                  struct fts_slide_ts_data, esdcheck_work.work);
