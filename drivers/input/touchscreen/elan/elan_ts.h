...
struct elan_finger_struct {
	...
	int recv_2nd_packet;	/*recv second packet flag*/
};
...
struct elan_gesture_struct {
	int gesture_code;	/*elan define gesture code*/
	int gbuf_valid_size; /*elan define gesture include information size*/ 
};

struct elan_report_struct {
	struct elan_finger_struct finger;
	struct elan_stylus_struct stylus;
	struct elan_gesture_struct gesture;
...
struct elan_ts_data {
	...
	spinlock_t irq_lock_flag;		/*spin lock*/
	bool irq_disabled;				/*irq enable/disable flage, 1=>irq has been disable, 0=>has been enable*/
	struct mutex irq_mutex;			/* Guards against concurrent access to the irq_lock_flag*/
	int recv_size;					/*for recvsize*/
	int elan_swapbit;				/*bit0-3 for finger, bit4-7 for stylus*/
	int gesture_switch;				/*elan gesture switch*/
};
...
/*
 * elan gesture mode open depend on fw
 */
enum elan_gesture_packet {
	GESTURE_HEAD = 0x89,
	GESTURE_O = 0x02,
	GESTURE_V = 0x03,/*V*/
	GESTURE_IV = 0x04,/*^*/
