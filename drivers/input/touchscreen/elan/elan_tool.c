...
static ssize_t store_swapcoord(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int swapbit = 0;
	...
	sscanf(buf, "%x", &swapbit);
	ts->elan_swapbit = swapbit;
	
	return count;
}

static ssize_t show_swapcoord(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);
				
	return sprintf(buf, "swapbit = %x\n", ts->elan_swapbit);
}
static DEVICE_ATTR(swapcoord, S_IWUSR | S_IRUSR, show_swapcoord, store_swapcoord);


static ssize_t store_gesture_switch(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int gesture_switch = 0;
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	
	sscanf(buf, "%x", &gesture_switch);
	ts->gesture_switch = gesture_switch;
	
	return count;
}

static ssize_t show_gesture_switch(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct elan_ts_data *ts = i2c_get_clientdata(client);
	
	return sprintf(buf, "gesture_switch = %x\n", ts->gesture_switch);
}
static DEVICE_ATTR(gesture_switch, S_IWUSR | S_IRUSR, show_gesture_switch, store_gesture_switch);
