...
/************************key event define**************************/
static const int key_value[] = {KEY_MENU, KEY_HOMEPAGE, KEY_BACK, KEY_POWER};
...
void elan_irq_disable(struct elan_ts_data *ts)
{
	unsigned long irqflags;

	spin_lock_irqsave(&ts->irq_lock_flag, irqflags);
	...
	spin_unlock_irqrestore(&ts->irq_lock_flag, irqflags);
}

void elan_irq_enable(struct elan_ts_data *ts)
{
...
static int hid_fw_packet_handler(struct i2c_client *client)
{
	...
	fw_info->tx = rbuf[7];
	/*finger resolution*/
	if(ts->elan_swapbit & 0x01) {
		fw_info->finger_xres = (rbuf[7] - 1) * fw_info->finger_osr;
		fw_info->finger_yres = (rbuf[6] - 1) * fw_info->finger_osr;
	}else {
	...
	/*pen resolution*/
	if(ts->elan_swapbit & 0x10) {
		fw_info->pen_xres = (rbuf[7] - 1) * pen_osr;
		fw_info->pen_yres = (rbuf[6] - 1) * pen_osr;
	}else {
...
static int elan_ts_recv_data(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	rc = ts->ops->recv(rbuf, ts->recv_size);
	if ( rc < 0) {
...
//			report->stylus.barrel				= rbuf[3] >> 1;
			...
			ts->recv_size = (rbuf[3] == 0x00? sizeof(rbuf): rbuf[0]);
		}else if(rbuf[2] == GESTURE_HEAD) {
			report->gesture.gesture_code = rbuf[3];
			report->gesture.gbuf_valid_size = 8;
			report->tool_type = ELAN_GESTURE;
			memcpy(buf,rbuf,report->gesture.gbuf_valid_size);
		}
	} else if (ts->chip_type == NORMAL_TYPE_PROTOCOL) {
		if (rbuf[0] == NOR2_FID) {
...
static void  elants_a_report(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	if (report->tool_type == ELAN_FINGER) {
		for (reportid = 0; reportid < finger.fvalid_num; reportid++ ) {
			if (finger.fid ==  HID_FID) {  /*hid over i2c protocol*/
				...
				if (fbits) {
					...
					/*
					 *bit 0-2 for finger: 
					 *bit[0]: 0:normal, 1:swap x-y
					 *bit[1]: 0:normal, 1: rotate_x
				     *bit[2]: 0:normal, 1: rotate_y
					*/
					if(ts->elan_swapbit & 0x01)
					...
					if(ts->elan_swapbit & 0x02)
						x = ts->hw_info.screen_x - x;
				
					if(ts->elan_swapbit & 0x04)
		...
		if (stylus.inrange_status) {
			/*
			 *bit 4-6 for pen: 
			 *bit[4]: 0:normal, 1:swap x-y
			 *bit[5]: 0:normal, 1: rotate_x
			 *bit[6]: 0:normal, 1: rotate_y
			*/
			...
			if(ts->elan_swapbit & 0x20)
				x = ts->hw_info.screen_x - x;
			
			if(ts->elan_swapbit & 0x40)
				y = ts->hw_info.screen_y - y;
			...
			if (contact == 0x01) {/* no contact, report hover function  */
				if(hover_flag) {
					input_report_abs(ts->pen_idev, ABS_X, x);
					input_report_abs(ts->pen_idev, ABS_Y, y);
					input_report_key(ts->pen_idev, BTN_TOUCH, 0);
					input_report_abs(ts->pen_idev, ABS_PRESSURE, 0);
					input_report_abs(ts->pen_idev, ABS_DISTANCE, 1);
					input_report_key(ts->pen_idev, BTN_TOOL_PEN, 1);
					print_log(ts->level, "[elan pen] Hover DISTANCE=15, x=%d y=%d\n", x, y);
				} else {
					input_report_key(ts->pen_idev, BTN_TOOL_PEN, 1);
					input_report_key(ts->pen_idev, BTN_TOUCH, 0);
					input_report_abs(ts->pen_idev, ABS_PRESSURE, 0);
				...
				input_report_abs(ts->pen_idev, ABS_DISTANCE, 0);
				input_report_abs(ts->pen_idev, ABS_TILT_X, x_tilt);
				input_report_abs(ts->pen_idev, ABS_TILT_Y, y_tilt);
				input_report_key(ts->pen_idev, BTN_TOOL_PEN, 1);
				input_report_key(ts->pen_idev, BTN_STYLUS, buf[3] & 0x04);	//btn1: barrel
				input_report_key(ts->pen_idev, BTN_TOOL_RUBBER, buf[3] & 0x10);	//btn2: eraser
				//input_report_key(ts->pen_idev, BTN_STYLUS2, buf[3] & 0x10);
				//print_log(ts->level,  "[elan] pen X:Y:P:TX:TY ====%d:%d:%d:%d:%d\n",x,y,p,x_tilt,y_tilt);
			}
		} else {
			input_report_key(ts->pen_idev, BTN_TOUCH, 0);
			input_report_key(ts->pen_idev, BTN_TOOL_PEN, 0);
			input_report_abs(ts->pen_idev, ABS_PRESSURE, 0);
			input_report_key(ts->pen_idev, BTN_STYLUS, 0);	//btn1: barrel
			input_report_key(ts->pen_idev, BTN_TOOL_RUBBER, 0);	//btn2: eraser
		}
		input_sync(ts->pen_idev);
	} else if(report->tool_type == ELAN_GESTURE) {
		switch(buf[3]) {
			case DOUBLE_CLICK:
	...
	if (report->tool_type == ELAN_FINGER) {
		...
		if(stylus.inrange_status) {
			...
			*/
			...
			if(ts->elan_swapbit & 0x20)
				x = ts->fw_info.pen_xres - x;//ts->hw_info.screen_x - x;
			
			if(ts->elan_swapbit & 0x40)
				y = ts->fw_info.pen_yres - y;
			...
			input_report_abs(ts->pen_idev, ABS_MT_DISTANCE, !stylus.tip_status);
			input_report_key(ts->pen_idev,  BTN_STYLUS, buf[3] & 0x04);
			input_report_key(ts->pen_idev,  BTN_STYLUS2, buf[3] & 0x10);
			...
			input_report_key(ts->pen_idev, BTN_TOUCH, 0);
			input_report_abs(ts->pen_idev, ABS_MT_DISTANCE, 0);
			input_report_key(ts->pen_idev, BTN_STYLUS, 0);
			input_report_key(ts->pen_idev,  BTN_STYLUS2, 0);
...
static void  elan_ts_work_func(struct work_struct *work)
{
	...
	if(ts->report.finger.recv_2nd_packet)//for hid_iic finger number > 5
		goto recv_2nd_packet;
...
static int elan_request_pen_input_dev(struct elan_ts_data *ts)
{
	...
	if (ts->report_type == PROTOCOL_TYPE_B) {
		...
		input_set_abs_params(ts->pen_idev, ABS_MT_POSITION_X, 0,  ts->fw_info.pen_xres, 0, 0);
		input_set_abs_params(ts->pen_idev, ABS_MT_POSITION_Y, 0,  ts->fw_info.pen_yres, 0, 0);
		...
		input_set_abs_params(ts->pen_idev, ABS_X, 0, ts->fw_info.pen_xres, 0, 0);
		input_set_abs_params(ts->pen_idev, ABS_Y, 0, ts->fw_info.pen_yres, 0, 0);
...
*/
	...
	/*get elan pointer diretion*/
	ret = of_property_read_u32(np, "direction_switch", &data);
	if (ret == 0) {
		ts->elan_swapbit = data;
		dev_info(&ts->client->dev,"[elan]:elan pointer direction switch = %2x", ts->elan_swapbit);
	} else {
		ret = -EINVAL;//hw_info->rst_gpio;
		goto read_report_type_failed;	
	}
    ts->elan_swapbit = 0x55;
	dev_info(&ts->client->dev,"[elan]:elan pointer direction switch remodify= %2x", ts->elan_swapbit);

	//ts->hw_info.screen_x = 1920;
	//ts->hw_info.screen_y = 2560;
#if 1
	/*get lcm coordinate*/
	ret = of_property_read_u32_array(np, "lcm_resolution", lcm_coordinate,2);
	if (ret == 0) {
...
//#define SM_BUS
	...
	private_ts = ts;
	spin_lock_init(&ts->irq_lock_flag);
	ts->recv_size = HID_RECV_LEN;
...
static int elan_ts_suspend(struct device *dev)
{
	...
	uint8_t disable_report[HID_CMD_LEN] = {0x04,0x00,0x23,0x00,0x03,0x00,0x04,0x54,0xCA,0x00,0x01};
	...
	if (ts->power_lock == 0) {
		...
		if(ts->gesture_switch) {

			elan_switch_irq(ts, 0);
			err = ts->ops->send(disable_report, sizeof(disable_report));
			if(err != sizeof(disable_report))
				dev_err(&client->dev,"[elan] %s set disable report failed %d\n",__func__, err);
