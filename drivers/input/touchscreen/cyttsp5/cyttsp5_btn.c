...
static inline void cyttsp5_btn_key_action(struct cyttsp5_btn_data *bd, int btn_no, int btn_state)
{
    ...
    parade_debug(dev, DEBUG_LEVEL_1, "%s: btn=%d key_code=%d %s\n", __func__, btn_no, si->btn[btn_no].key_code, btn_state == CY_BTN_PRESSED ? "PRESSED" : "RELEASED");
}
...
static void cyttsp5_get_btn_touches(struct cyttsp5_btn_data *bd)
{
    ...
    for (cur_btn = 0; cur_btn < num_btns; cur_btn++) {
        ...
        cur_btn_state = (si->xy_data[0] >> (cur_btn * CY_BITS_PER_BTN)) & CY_NUM_BTN_EVENT_ID;
...
static void cyttsp5_log_btn_data(struct cyttsp5_btn_data *bd)
{
    ...
    for (cur = 0; cur < si->num_btns; cur++) {
        ...
        snprintf(pr_buf, CY_MAX_PRBUF_SIZE, "%s%X (%02X)", pr_buf, value, le16_to_cpu(si->xy_data[1 + cur * 2]));
...
static int cyttsp5_btn_open(struct input_dev *input)
{
    ...
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_BTN_NAME, cyttsp5_btn_attention, CY_MODE_OPERATIONAL);

    /* set up startup call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_BTN_NAME, cyttsp5_startup_attention, 0);

    /* set up suspend call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_SUSPEND, CYTTSP5_BTN_NAME, cyttsp5_btn_suspend_attention, 0);

    /* set up resume call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_RESUME, CYTTSP5_BTN_NAME, cyttsp5_btn_resume_attention, 0);
...
static void cyttsp5_btn_close(struct input_dev *input)
{
    ...
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_BTN_NAME, cyttsp5_btn_attention, CY_MODE_OPERATIONAL);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_BTN_NAME, cyttsp5_startup_attention, 0);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_SUSPEND, CYTTSP5_BTN_NAME, cyttsp5_btn_suspend_attention, 0);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_RESUME, CYTTSP5_BTN_NAME, cyttsp5_btn_resume_attention, 0);
...
static int cyttsp5_setup_input_attention(struct device *dev)
{
    ...
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_BTN_NAME, cyttsp5_setup_input_attention, 0);
...
int cyttsp5_btn_probe(struct device *dev)
{
    ...
    scnprintf(bd->phys, sizeof(bd->phys), "%s/input%d", dev_name(dev), cd->phys_num++);
    ...
    if (bd->si) {
        ...
        _cyttsp5_subscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_BTN_NAME, cyttsp5_setup_input_attention, 0);
    }
