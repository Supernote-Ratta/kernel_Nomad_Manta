...
static inline int get_inp_dev_name(struct device_node *dev_node, const char **inp_dev_name)
{
    return of_property_read_string(dev_node, "cy,inp_dev_name", inp_dev_name);
}

static s16 *create_and_get_u16_array(struct device_node *dev_node, const char *name, int *size)
{
    const __be32 *values;
    s16 *val_array;
    int len;
...
static struct touch_framework *create_and_get_touch_framework(struct device_node *dev_node)
{
...
{
    struct cyttsp5_virtual_keys *vkeys = container_of(attr, struct cyttsp5_virtual_keys, kobj_attr);
    u16 *data = vkeys->data;
    int size = vkeys->size;
    int index;
    int i;
    ...
    for (i = 0; i < size; i += VIRTUAL_KEY_ELEMENT_SIZE) {
        index += scnprintf(buf + index, CY_MAX_PRBUF_SIZE - index, "0x01:%d:%d:%d:%d:%d\n", data[i], data[i + 1], data[i + 2], data[i + 3], data[i + 4]);
    }

    return index;
}

static int setup_virtual_keys(struct device_node *dev_node, const char *inp_dev_name, struct cyttsp5_virtual_keys *vkeys)
{
    char *name;
    u16 *data;
    int size;
...
static void free_virtual_keys(struct cyttsp5_virtual_keys *vkeys)
{
    if (board_properties_kobj) {
        sysfs_remove_file(board_properties_kobj, &vkeys->kobj_attr.attr);
    }
...
#ifdef ENABLE_VIRTUAL_KEYS
    rc = setup_virtual_keys(dev_node, pdata->inp_dev_name, &ext_pdata->vkeys);
    if (rc) {
...
{
    struct cyttsp5_mt_platform_data *mt_pdata = (struct cyttsp5_mt_platform_data *)pdata;
    struct cyttsp5_extended_mt_platform_data *ext_mt_pdata = container_of(mt_pdata, struct cyttsp5_extended_mt_platform_data, pdata);
...
{
    struct cyttsp5_btn_platform_data *btn_pdata = (struct cyttsp5_btn_platform_data *)pdata;
...
{
    struct cyttsp5_proximity_platform_data *proximity_pdata = (struct cyttsp5_proximity_platform_data *)pdata;
...
static int get_device_type(struct device_node *dev_node, enum cyttsp5_device_type *type)
{
    const char *name;
...
static inline void *create_and_get_device_pdata(struct device_node *dev_node, enum cyttsp5_device_type type)
{
...
static struct touch_settings *create_and_get_touch_setting(struct device_node *core_node, const char *name)
{
...
static struct cyttsp5_core_platform_data *create_and_get_core_pdata(struct device_node *core_node)
{
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
    ...
    for_each_child_of_node(adap_dev->of_node, core_node) {
        ...
        for_each_child_of_node(core_node, dev_node) {
            ...
            *pdata_ptr[type].pdata = create_and_get_device_pdata(dev_node, type);
