...
#ifdef VERBOSE_DEBUG
void cyttsp5_pr_buf(struct device *dev, u8 *dptr, int size, const char *data_name)
{
...
static int tthe_print(struct cyttsp5_core_data *cd, u8 *buf, int buf_len, const u8 *data_name)
{
    ...
    if (cd->show_timestamp) {
        sprintf(data_name_with_time_stamp, "[%u] %s", jiffies_to_msecs(jiffies), data_name);
...
static int _cyttsp5_request_tthe_print(struct device *dev, u8 *buf, int buf_len, const u8 *data_name)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static struct cyttsp5_hid_report *cyttsp5_get_hid_report_(struct cyttsp5_core_data *cd, u8 report_type, u8 report_id, bool create)
{
    ...
    for (i = 0; i < cd->num_hid_reports; i++) {
        if (cd->hid_reports[i]->type == report_type && cd->hid_reports[i]->id == report_id) {
...
/* Must be called with cd->hid_report_lock acquired */
static struct cyttsp5_hid_field *cyttsp5_create_hid_field_(struct cyttsp5_hid_report *report)
{
...
static int cyttsp5_add_parameter(struct cyttsp5_core_data *cd, u8 param_id, u32 param_value, u8 param_size)
{
...
int request_exclusive(struct cyttsp5_core_data *cd, void *ownptr, int timeout_ms)
{
...
static int cyttsp5_hid_exec_cmd_(struct cyttsp5_core_data *cd, struct cyttsp5_hid_cmd *hid_cmd)
{
    int rc;
    u8 *cmd;
    u8 cmd_length;
    ...
    memcpy(&cmd[cmd_offset], &cd->hid_desc.command_register, sizeof(cd->hid_desc.command_register));
    ...
    if (hid_cmd->has_data_register) {
        memcpy(&cmd[cmd_offset], &cd->hid_desc.data_register, sizeof(cd->hid_desc.data_register));
        cmd_offset += sizeof(cd->hid_desc.data_register);
    }

    /* Set Data */
    if (hid_cmd->write_length && hid_cmd->write_buf) {
        memcpy(&cmd[cmd_offset], hid_cmd->write_buf, hid_cmd->write_length);
        cmd_offset += hid_cmd->write_length;
    }

    rc = cyttsp5_adap_write_read_specific(cd, cmd_length, cmd, hid_cmd->read_buf);
    if (rc) {
...
static int cyttsp5_hid_exec_cmd_and_wait_(struct cyttsp5_core_data *cd, struct cyttsp5_hid_cmd *hid_cmd)
{
    int rc;
    int t;
    u16 timeout_ms;
    ...
    t = wait_event_timeout(cd->wait_q, (*cmd_state == 0), msecs_to_jiffies(timeout_ms));
    if (IS_TMO(t)) {
...
static int cyttsp5_hid_cmd_set_power_(struct cyttsp5_core_data *cd, u8 power_state)
{
    int rc;
    struct cyttsp5_hid_cmd hid_cmd = {
    ...
    /* validate */
    if ((cd->response_buf[2] != HID_RESPONSE_REPORT_ID) || ((cd->response_buf[3] & 0x3) != power_state) || ((cd->response_buf[4] & 0xF) != HID_CMD_SET_POWER)) {
        rc = -EINVAL;
    }
...
static int cyttsp5_hid_cmd_set_power(struct cyttsp5_core_data *cd, u8 power_state)
{
    int rc;
...
static int cyttsp5_hid_output_validate_bl_response(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
    u16 size;
    u16 crc;
    u8 status;
    ...
    if (cd->response_buf[size - 3] != LOW_BYTE(crc) || cd->response_buf[size - 2] != HI_BYTE(crc)) {
...
static int cyttsp5_hid_output_validate_app_response(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
    int command_code;
    u16 size;
    ...
    command_code = cd->response_buf[HID_OUTPUT_RESPONSE_CMD_OFFSET] & HID_OUTPUT_RESPONSE_CMD_MASK;
...
static void cyttsp5_check_set_parameter(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output, bool raw)
{
    u8 *param_buf;
    u32 param_value = 0;
    u8 param_size;
    ...
    if (raw && hid_output->length >= 10 && hid_output->length <= 13
        && !memcmp(&hid_output->write_buf[0], &cd->hid_desc.output_register, sizeof(cd->hid_desc.output_register))
...
static void cyttsp5_check_command(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output, bool raw)
{
    cyttsp5_check_set_parameter(cd, hid_output, raw);
}

static int cyttsp5_hid_output_validate_response(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
...
static int cyttsp5_hid_send_output_user_(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
    int rc;
    ...
    rc = cyttsp5_adap_write_read_specific(cd, hid_output->length, hid_output->write_buf, NULL);
    if (rc) {
        dev_err(cd->dev, "%s: Fail cyttsp5_adap_transfer\n", __func__);
    }
...
static int cyttsp5_hid_send_output_user_and_wait_(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
    int rc;
    int t;
    ...
    t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0), msecs_to_jiffies(CY_HID_OUTPUT_USER_TIMEOUT));
    if (IS_TMO(t)) {
...
static int cyttsp5_hid_send_output_(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
    int rc;
    u8 *cmd;
    u16 length;
    ...
    /* Set Output register */
    memcpy(&cmd[cmd_offset], &cd->hid_desc.output_register, sizeof(cd->hid_desc.output_register));
    ...
    if (hid_output->write_length && hid_output->write_buf) {
        memcpy(&cmd[cmd_offset], hid_output->write_buf, hid_output->write_length);
...
static int cyttsp5_hid_send_output_and_wait_(struct cyttsp5_core_data *cd, struct cyttsp5_hid_output *hid_output)
{
    int rc;
    int t;
#ifdef VERBOSE_DEBUG
    ...
    //printk("%s time=%d\n", __func__, timeout_ms);
    t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0), msecs_to_jiffies(timeout_ms));
    if (IS_TMO(t)) {
        dev_err(cd->dev, "%s: HID output cmd execution timed out\n", __func__);
        rc = -ETIME;
        goto error;
...
static void cyttsp5_si_get_cydata(struct cyttsp5_core_data *cd)
{
    ...
    struct cyttsp5_cydata_dev *cydata_dev = (struct cyttsp5_cydata_dev *)&cd->response_buf[HID_SYSINFO_CYDATA_OFFSET];
    ...
    cyttsp5_pr_buf(cd->dev, (u8 *)cydata_dev, sizeof(struct cyttsp5_cydata_dev), "sysinfo_cydata");
}
...
static void cyttsp5_si_get_sensing_conf_data(struct cyttsp5_core_data *cd)
{
    ...
    struct cyttsp5_sensing_conf_data_dev *scd_dev = (struct cyttsp5_sensing_conf_data_dev *)&cd->response_buf[HID_SYSINFO_SENSING_OFFSET];
    ...
    cyttsp5_pr_buf(cd->dev, (u8 *)scd_dev, sizeof(struct cyttsp5_sensing_conf_data_dev), "sensing_conf_data");
}
...
static int cyttsp5_si_setup(struct cyttsp5_core_data *cd)
{
    ...
    if (!si->xy_data) {
        si->xy_data = kzalloc(max_tch * si->desc.tch_record_size, GFP_KERNEL);
    }
    if (!si->xy_data) {
        return -ENOMEM;
    }
...
static int cyttsp5_si_get_btn_data(struct cyttsp5_core_data *cd)
{
    ...
    int i;
    int rc = 0;
    unsigned int btns = cd->response_buf[HID_SYSINFO_BTN_OFFSET] & HID_SYSINFO_BTN_MASK;
...
static int _cyttsp5_request_hid_output_suspend_scanning(struct device *dev, int protect)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static int _cyttsp5_request_hid_output_resume_scanning(struct device *dev, int protect)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static int cyttsp5_hid_output_get_param_(struct cyttsp5_core_data *cd, u8 param_id, u32 *value)
{
    int write_length = 1;
    u8 param[1] = { param_id };
    u8 read_param_id;
...
static int cyttsp5_hid_output_get_param(struct cyttsp5_core_data *cd, u8 param_id, u32 *value)
{
    int rc;
...
int _cyttsp5_request_hid_output_get_param(struct device *dev, int protect, u8 param_id, u32 *value)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static int cyttsp5_hid_output_set_param_(struct cyttsp5_core_data *cd, u8 param_id, u32 value, u8 size)
{
    u8 write_buf[6];
    u8 *ptr = &write_buf[2];
    int rc;
...
static int cyttsp5_hid_output_set_param(struct cyttsp5_core_data *cd, u8 param_id, u32 value, u8 size)
{
    int rc;
...
int _cyttsp5_request_hid_output_set_param(struct device *dev, int protect, u8 param_id, u32 value, u8  size)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static int cyttsp5_hid_output_enter_easywake_state_(struct cyttsp5_core_data *cd, u8 data, u8 *return_data)
{
    int write_length = 1;
    u8 param[1] = { data };
    int rc;
...
static int cyttsp5_hid_output_verify_config_block_crc_(struct cyttsp5_core_data *cd, u8 ebid, u8 *status, u16 *calculated_crc, u16 *stored_crc)
{
    int write_length = 1;
    u8 param[1] = { ebid };
    u8 *ptr;
...
static int cyttsp5_hid_output_verify_config_block_crc(struct cyttsp5_core_data *cd, u8 ebid, u8 *status, u16 *calculated_crc, u16 *stored_crc)
{
    int rc;
    ...
    rc = cyttsp5_hid_output_verify_config_block_crc_(cd, ebid, status, calculated_crc, stored_crc);
...
static int _cyttsp5_request_hid_output_verify_config_block_crc( struct device *dev, int protect, u8 ebid, u8 *status, u16 *calculated_crc, u16 *stored_crc)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);

    if (protect) {
        return cyttsp5_hid_output_verify_config_block_crc(cd, ebid, status, calculated_crc, stored_crc);
    }

    return cyttsp5_hid_output_verify_config_block_crc_(cd, ebid, status, calculated_crc, stored_crc);
}

static int cyttsp5_hid_output_get_config_row_size_(struct cyttsp5_core_data *cd, u16 *row_size)
{
    int rc;
    struct cyttsp5_hid_output hid_output = {
...
static int cyttsp5_hid_output_get_config_row_size(struct cyttsp5_core_data *cd, u16 *row_size)
{
    int rc;
...
static int _cyttsp5_request_hid_output_get_config_row_size(struct device *dev, int protect, u16 *row_size)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static int cyttsp5_hid_output_read_conf_block_(struct cyttsp5_core_data *cd, u16 row_number, u16 length, u8 ebid, u8 *read_buf, u16 *crc)
{
    int read_ebid;
    int read_length;
    int status;
...
static int cyttsp5_hid_output_read_conf_ver_(struct cyttsp5_core_data *cd, u16 *config_ver)
{
    int rc;
...
static int setup_report_descriptor(struct cyttsp5_core_data *cd)
{
    ...
    for (i = CY_TCH_X; i < CY_TCH_NUM_ABS; i++) {
        field = find_report_desc_field(cd, cyttsp5_tch_abs_field_map[i], tch_collection_usage_page);
        if (field) {
    ...
    for (i = CY_TCH_TIME; i < CY_TCH_NUM_HDR; i++) {
        field = find_report_desc_field(cd, cyttsp5_tch_hdr_field_map[i], tch_collection_usage_page);
        if (field) {
            parade_debug(cd->dev, DEBUG_LEVEL_2,
                         " Field %p: rep_cnt:%d rep_sz:%d off:%d data:%02X min:%d max:%d usage_page:%08X\n",
                         field, field->report_count, field->report_size,
...
static int cyttsp5_get_report_descriptor_(struct cyttsp5_core_data *cd)
{
    ...
    memcpy(cmd, &cd->hid_desc.report_desc_register, sizeof(cd->hid_desc.report_desc_register));
    ...
    t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0), msecs_to_jiffies(CY_HID_GET_REPORT_DESCRIPTOR_TIMEOUT));
    if (IS_TMO(t)) {
        dev_err(cd->dev, "%s: HID get descriptor timed out\n", __func__);
        rc = -ETIME;
        goto error;
    ...
    cyttsp5_pr_buf(cd->dev, cd->response_buf, cd->hid_core.hid_report_desc_len, "Report Desc");

    rc = parse_report_descriptor(cd, cd->response_buf + 3, get_unaligned_le16(&cd->response_buf[0]) - 3);
    if (rc) {
...
static int cyttsp5_get_mode(struct cyttsp5_core_data *cd, struct cyttsp5_hid_desc *desc)
{
...
static void call_atten_cb(struct cyttsp5_core_data *cd, enum cyttsp5_atten_type type, int mode)
{
    ...
    list_for_each_entry_safe(atten, atten_n, &cd->atten_list[type], node) {
        if (!mode || atten->mode & mode) {
            spin_unlock(&cd->spinlock);
            parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: attention for '%s'", __func__, dev_name(atten->dev));
...
static void cyttsp5_start_wd_timer(struct cyttsp5_core_data *cd)
{
    ...
    mod_timer(&cd->watchdog_timer, jiffies + msecs_to_jiffies(cd->watchdog_interval));
}
...
{
    struct cyttsp5_core_data *cd = container_of(work, struct cyttsp5_core_data, watchdog_work);
    int rc;
    /*fix CDT207254
...
queue_startup:
    if (rc) {
        dev_err(cd->dev, "%s: failed to access device in watchdog timer r=%d\n", __func__, rc);
...
static int cyttsp5_put_device_into_easy_wakeup_(struct cyttsp5_core_data *cd)
{
    ...
    rc = cyttsp5_hid_output_enter_easywake_state_(cd, cd->easy_wakeup_gesture, &status);
    if (rc || status == 0) {
        return -EBUSY;
    }
...
static void cyttsp5_get_touch_axis(struct cyttsp5_core_data *cd, int *axis, int size, int max, u8 *data, int bofs)
{
    int nbyte;
    int next;
...
static int move_tracking_hetmap_data(struct cyttsp5_core_data *cd, struct cyttsp5_sysinfo *si)
{
#ifdef TTHE_TUNER_SUPPORT
    int size = get_unaligned_le16(&cd->input_buf[0]);
...
static int move_sensor_data(struct cyttsp5_core_data *cd, struct cyttsp5_sysinfo *si)
{
#ifdef TTHE_TUNER_SUPPORT
    int size = get_unaligned_le16(&cd->input_buf[0]);
...
static int move_button_data(struct cyttsp5_core_data *cd, struct cyttsp5_sysinfo *si)
{
#ifdef TTHE_TUNER_SUPPORT
    int size = get_unaligned_le16(&cd->input_buf[0]);
    ...
    cyttsp5_pr_buf(cd->dev, (u8 *)si->xy_mode, BTN_INPUT_HEADER_SIZE, "xy_mode");

    memcpy(si->xy_data, &cd->input_buf[BTN_INPUT_HEADER_SIZE], BTN_REPORT_SIZE);
    cyttsp5_pr_buf(cd->dev, (u8 *)si->xy_data, BTN_REPORT_SIZE, "xy_data");
    return 0;
}

static int move_touch_data(struct cyttsp5_core_data *cd, struct cyttsp5_sysinfo *si)
{
    int max_tch = si->sensing_conf_data.max_tch;
    int num_cur_tch;
    int length;
    ...
    cyttsp5_pr_buf(cd->dev, (u8 *)si->xy_mode, si->desc.tch_header_size, "xy_mode");

    cyttsp5_get_touch_axis(cd, &num_cur_tch, tch->size, tch->max, si->xy_mode + 3 + tch->ofs, tch->bofs);
    ...
    if ((length + si->desc.tch_header_size) > CY_MAX_INPUT) {
        dev_err(cd->dev, "%s:%d: size=%d is larger than the max %d!!!\n", __func__, __LINE__, length + si->desc.tch_header_size, CY_MAX_INPUT);
        return -EINVAL;
    }
...
static int parse_touch_input(struct cyttsp5_core_data *cd, int size)
{
    ...
    if (!si->ready) {
        dev_err(cd->dev, "%s: Need system information to parse touches\n", __func__);
        return 0;
    }
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
    ...
    if (size == 0) {
        ...
        if (cd->hid_cmd_state == HID_OUTPUT_START_BOOTLOADER + 1 || cd->hid_cmd_state == HID_OUTPUT_BL_LAUNCH_APP + 1 || cd->hid_cmd_state == HID_OUTPUT_USER_CMD + 1) {
            cd->hid_cmd_state = 0;
        }
        wake_up(&cd->wait_q);
        mutex_unlock(&cd->system_lock);
    ...
    mod_timer_pending(&cd->watchdog_timer, jiffies + msecs_to_jiffies(cd->watchdog_interval));
#endif
    if (report_id != cd->sysinfo.desc.tch_report_id && report_id != cd->sysinfo.desc.btn_report_id && report_id != HID_SENSOR_DATA_REPORT_ID && report_id != HID_TRACKING_HEATMAP_REPOR_ID) {
        is_command = 1;
    }
...
static int cyttsp5_read_input(struct cyttsp5_core_data *cd)
{
    ...
    if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
        if (cd->sleep_state == SS_SLEEP_ON) {
            ...
            t = wait_event_timeout(cd->wait_q, (cd->wait_until_wake == 1), msecs_to_jiffies(2000));
            if (IS_TMO(t)) {
                cyttsp5_queue_startup(cd);
            }
            goto read;
...
int _cyttsp5_subscribe_attention(struct device *dev, enum cyttsp5_atten_type type, char *id, int (*func)(struct device *), int mode)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ...
    list_for_each_entry(atten, &cd->atten_list[type], node) {
        if (atten->id == id && atten->mode == mode) {
            spin_unlock(&cd->spinlock);
            kfree(atten_new);
            parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: %s=%p %s=%d\n", __func__, "already subscribed attention", dev, "mode", mode);
...
int _cyttsp5_unsubscribe_attention(struct device *dev, enum cyttsp5_atten_type type, char *id, int (*func)(struct device *), int mode)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    struct atten_node *atten, *atten_n;
    ...
    list_for_each_entry_safe(atten, atten_n, &cd->atten_list[type], node) {
        if (atten->id == id && atten->mode == mode) {
            list_del(&atten->node);
            spin_unlock(&cd->spinlock);
            parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: %s=%p %s=%d\n", __func__, "unsub for atten->dev", atten->dev, "atten->mode", atten->mode);
            kfree(atten);
            return 0;
        }
    }
...
static int cyttsp5_reset_and_wait(struct cyttsp5_core_data *cd)
{
    ...
    t = wait_event_timeout(cd->wait_q, (cd->hid_reset_cmd_state == 0), msecs_to_jiffies(CY_HID_RESET_TIMEOUT));
    if (IS_TMO(t)) {
...
static struct cyttsp5_loader_platform_data *_cyttsp5_request_loader_pdata(struct device *dev)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
static int cyttsp5_core_wake_device_from_deep_sleep_(struct cyttsp5_core_data *cd)
{
    int rc;
...
static int cyttsp5_restore_parameters_(struct cyttsp5_core_data *cd)
{
    ...
    list_for_each_entry(param, &cd->param_list, node) {
        ...
        rc = cyttsp5_hid_output_set_param_(cd, param->id, param->value, param->size);
        if (rc) {
            goto exit;
        }
        spin_lock(&cd->spinlock);
...
reset:
    ...
    dev_info(cd->dev, "cyttsp5 Protocol Version: %d.%d\n", cd->sysinfo.cydata.pip_ver_major, cd->sysinfo.cydata.pip_ver_minor);
...
{
    struct cyttsp5_core_data *cd = container_of(work, struct cyttsp5_core_data, startup_work);
    int rc;
 ...
 */
#if defined(CONFIG_PM_RUNTIME) || (KERNEL_VERSION(3, 19, 0) <= LINUX_VERSION_CODE)
...
static int cyttsp5_pm_notifier(struct notifier_block *nb, unsigned long action, void *data)
{
    struct cyttsp5_core_data *cd = container_of(nb, struct cyttsp5_core_data, pm_notifier);
 ...
 */
static ssize_t cyttsp5_ic_ver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    struct cyttsp5_cydata *cydata = &cd->sysinfo.cydata;
 ...
 */
static ssize_t cyttsp5_drv_ver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    return snprintf(buf, CY_MAX_PRBUF_SIZE, "Driver: %s\nVersion: %s\nDate: %s\n", cy_driver_core_name, cy_driver_core_version, cy_driver_core_date);
}

/*
 * HW reset via sysfs
 */
static ssize_t cyttsp5_hw_reset_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    int rc;
 ...
 */
static ssize_t cyttsp5_hw_irq_stat_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    int retval;
    ...
    return snprintf(buf, CY_MAX_PRBUF_SIZE, "Function irq_stat() undefined.\n");
}

/*
 * Show IRQ enable/disable status via sysfs
 */
static ssize_t cyttsp5_drv_irq_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
    ...
    mutex_lock(&cd->system_lock);
    if (cd->irq_enabled) {
        ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "Driver interrupt is ENABLED\n");
    } else {
        ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "Driver interrupt is DISABLED\n");
    }
    mutex_unlock(&cd->system_lock);
 ...
 */
static ssize_t cyttsp5_drv_irq_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    unsigned long value;
    int retval = 0;
...
static int cyttsp5_ic_parse_input_dec(struct device *dev, const char *buf, size_t buf_size, u32 *ic_buf, size_t ic_buf_size)
{
    const char *pbuf = buf;
    unsigned long value;
    ...
    parade_debug(dev, DEBUG_LEVEL_1, "%s: pbuf=%p buf=%p size=%zu %s=%zu buf=%s\n", __func__, pbuf, buf, buf_size, "scan buf size", (size_t)CYTTSP5_INPUT_ELEM_SZ_DEC, buf);

    while (pbuf <= (buf + buf_size)) {
        if (i >= CY_MAX_CONFIG_BYTES_DEC) {
            dev_err(dev, "%s: %s size=%d max=%d\n", __func__, "Max cmd size exceeded", i, CY_MAX_CONFIG_BYTES_DEC);
            return -EINVAL;
        }
        if (i >= ic_buf_size) {
            dev_err(dev, "%s: %s size=%d buf_size=%zu\n", __func__, "Buffer size exceeded", i, ic_buf_size);
            return -EINVAL;
        }
        while (((*pbuf == ' ') || (*pbuf == ',')) && (pbuf < (buf + buf_size))) {
            last = *pbuf;
            pbuf++;
        }
 ...
 */
static ssize_t cyttsp5_drv_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    unsigned long value;
    int rc;
 ...
 */
static ssize_t cyttsp5_sleep_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
...
static ssize_t cyttsp5_easy_wakeup_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
...
static ssize_t cyttsp5_easy_wakeup_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    unsigned long value;
    int ret;
 ...
 */
static ssize_t cyttsp5_easy_wakeup_gesture_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
 ...
 */
static ssize_t cyttsp5_easy_wakeup_gesture_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret = 0;
    int i;
    ...
    for (i = 0; i < cd->gesture_data_length; i++) {
        ret += snprintf(buf + ret, CY_MAX_PRBUF_SIZE - ret, "0x%02X\n", cd->gesture_data[i]);
    }

    ret += snprintf(buf + ret, CY_MAX_PRBUF_SIZE - ret, "(%d bytes)\n", cd->gesture_data_length);
...
static ssize_t cyttsp5_panel_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    ssize_t ret;
...
static ssize_t cyttsp5_platform_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static struct device_attribute attributes[] = {
    ...
    __ATTR(drv_irq, S_IRUSR | S_IWUSR, cyttsp5_drv_irq_show, cyttsp5_drv_irq_store),
    __ATTR(drv_debug, S_IWUSR, NULL, cyttsp5_drv_debug_store),
    __ATTR(sleep_status, S_IRUSR, cyttsp5_sleep_status_show, NULL),
    __ATTR(easy_wakeup_gesture, S_IRUSR | S_IWUSR, cyttsp5_easy_wakeup_gesture_show, cyttsp5_easy_wakeup_gesture_store),
#ifdef EASYWAKE_TSG6
    __ATTR(easy_wakeup_gesture_id, S_IRUSR, cyttsp5_easy_wakeup_gesture_id_show, NULL),
    __ATTR(easy_wakeup_gesture_data, S_IRUSR, cyttsp5_easy_wakeup_gesture_data_show, NULL),
#endif
 ...
 */
static ssize_t cyttsp5_ttdl_restart_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    int rc;
    struct i2c_client *client = (struct i2c_client *)container_of(dev, struct i2c_client, dev);
...
static ssize_t tthe_debugfs_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    struct cyttsp5_core_data *cd = filp->private_data;
    int size;
    int ret;

    wait_event_interruptible(cd->wait_q, cd->tthe_buf_len != 0 || cd->tthe_exit);
    mutex_lock(&cd->tthe_lock);
    if (cd->tthe_exit) {
        mutex_unlock(&cd->tthe_lock);
        return 0;
...
static struct cyttsp5_core_nonhid_cmd _cyttsp5_core_nonhid_cmd = {
    ...
    .verify_config_block_crc = _cyttsp5_request_hid_output_verify_config_block_crc,
    ...
    .initialize_baselines = _cyttsp5_request_hid_output_initialize_baselines,
...
static int cyttsp5_probe_module(struct cyttsp5_core_data *cd, struct cyttsp5_module *module)
{
...
static void cyttsp5_release_module(struct cyttsp5_core_data *cd, struct cyttsp5_module *module)
{
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
    ...
    rc = request_threaded_irq(cd->irq, NULL, cyttsp5_irq, IRQF_TRIGGER_LOW | IRQF_ONESHOT, "cyttsp-irq", cd); // | IRQF_NO_SUSPEND 浼戠湢鏃朵笉disable
    if (rc < 0) {
...
int cyttsp5_probe(const struct cyttsp5_bus_ops *ops, struct device *dev, u16 irq, size_t xfer_buf_size)
{
    ...
    if (cd->supply) {
        printk("cyttsp5 touch supply = %dmv\n",  regulator_get_voltage(cd->supply));
    ...
    cd->hid_core.hid_vendor_id = (cd->cpdata->vendor_id) ? cd->cpdata->vendor_id : CY_HID_VENDOR_ID;
    cd->hid_core.hid_product_id = (cd->cpdata->product_id) ? cd->cpdata->product_id : CY_HID_APP_PRODUCT_ID;
    cd->hid_core.hid_desc_register = cpu_to_le16(cd->cpdata->hid_desc_register);
    ...
    if (cd->cpdata->detect) {
        ...
        rc = cd->cpdata->detect(cd->cpdata, cd->dev, cyttsp5_platform_detect_read);
        if (rc) {
    ...
    timer_setup(&cd->watchdog_timer, cyttsp5_watchdog_timer, (unsigned long)cd);
#endif
    ...
    cd->tthe_debugfs = debugfs_create_file(CYTTSP5_TTHE_TUNER_FILE_NAME, 0644, NULL, cd, &tthe_debugfs_fops);
#endif
