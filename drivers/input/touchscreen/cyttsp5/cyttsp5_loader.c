...
static inline struct cyttsp5_loader_data *cyttsp5_get_loader_data(struct device *dev)
{
    return cyttsp5_get_module_data(dev, &loader_module);
}

#if CYTTSP5_FW_UPGRADE || defined(CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_TTCONFIG_UPGRADE)
 ...
 */
static int cyttsp5_check_firmware_version(struct device *dev, u32 fw_ver_new, u32 fw_revctrl_new)
{
    ...
    parade_debug(dev, DEBUG_LEVEL_1, "%s: img vers:0x%04X new vers:0x%04X\n", __func__, fw_ver_img, fw_ver_new);

    if (fw_ver_new > fw_ver_img) {
        parade_debug(dev, DEBUG_LEVEL_1, "%s: Image is newer, will upgrade\n", __func__);
        return 1;
    }

    if (fw_ver_new < fw_ver_img) {
        parade_debug(dev, DEBUG_LEVEL_1, "%s: Image is older, will NOT upgrade\n", __func__);
        return -1;
    }

    fw_revctrl_img = ld->si->cydata.revctrl;

    parade_debug(dev, DEBUG_LEVEL_1, "%s: img revctrl:0x%04X new revctrl:0x%04X\n", __func__, fw_revctrl_img, fw_revctrl_new);
...
{
    struct cyttsp5_loader_data *ld = container_of(calibration_work, struct cyttsp5_loader_data, calibration_work);
...
static int cyttsp5_calibration_attention(struct device *dev)
{
    ...
    cmd->unsubscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_LOADER_NAME, cyttsp5_calibration_attention, 0);
...
static u8 *cyttsp5_get_row_(struct device *dev, u8 *row_buf, u8 *image_buf, int size)
{
...
static int cyttsp5_ldr_init_(struct device *dev, struct cyttsp5_hex_image *row_image)
{
    return cmd->nonhid_cmd->initiate_bl(dev, 0, 8, (u8 *)cyttsp5_security_key, row_image->row_size, row_image->row_data);
}

static int cyttsp5_ldr_parse_row_(struct device *dev, u8 *row_buf, struct cyttsp5_hex_image *row_image)
{
    int rc = 0;
...
static int cyttsp5_ldr_prog_row_(struct device *dev, struct cyttsp5_hex_image *row_image)
{
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
    ...
    while (p < last_row) {
        ...
        parade_debug(dev, DEBUG_LEVEL_2, "%s: array_id=%02X row_num=%04X(%d) row_size=%04X(%d)\n", __func__, row_image->array_id, row_image->row_num, row_image->row_num, row_image->row_size, row_image->row_size);
        if (rc) {
...
static int cyttsp5_upgrade_firmware(struct device *dev, const u8 *fw_img, int fw_size)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    ...
    if (rc < 0) {
    ...
    } else if (ld->loader_pdata && (ld->loader_pdata->flags & CY_LOADER_FLAG_CALIBRATE_AFTER_FW_UPGRADE)) {
        ...
        rc = cmd->subscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_LOADER_NAME, cyttsp5_calibration_attention, 0);
        if (rc) {
...
static int cyttsp5_check_firmware_version_platform(struct device *dev, struct cyttsp5_touch_firmware *fw)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    u32 fw_ver_new;
    u32 fw_revctrl_new;
...
static struct cyttsp5_touch_firmware *cyttsp5_get_platform_firmware(struct device *dev)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
...
static void _cyttsp5_firmware_cont(const struct firmware *fw, void *context)
{
    ...
    cyttsp5_upgrade_firmware(dev, &(fw->data[header_size + 1]), fw->size - (header_size + 1));
...
static void _cyttsp5_firmware_cont_builtin(const struct firmware *fw, void *context)
{
    struct device *dev = context;
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    int upgrade;
...
static int upgrade_firmware_from_class(struct device *dev)
{
    ...
    parade_debug(dev, DEBUG_LEVEL_2, "%s: Enabling firmware class loader\n", __func__);

    retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_NOHOTPLUG, CY_FW_MANUAL_UPGRADE_FILE_NAME, dev, GFP_KERNEL, dev, _cyttsp5_firmware_cont);
    if (retval < 0) {
...
#define FILENAME_LEN_MAX 64
    ...
    if (panel_id == PANEL_ID_NOT_ENABLED) {
        ...
        snprintf(filename, FILENAME_LEN_MAX, "%s_pid%02X%s", CY_FW_FILE_PREFIX, panel_id, CY_FW_FILE_SUFFIX);
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
    ...
    retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG, filename, dev, GFP_KERNEL, dev, _cyttsp5_firmware_cont_builtin);
    if (retval < 0) {
        dev_err(dev, "%s: Fail request firmware class file load\n", __func__);
        goto exit;
    }
...
static int cyttsp5_write_config_row_(struct device *dev, u8 ebid, u16 row_number, u16 row_size, u8 *data)
{
    int rc;
    u16 actual_write_len;

    rc = cmd->nonhid_cmd->write_conf_block(dev, 0, row_number, row_size, ebid, data, (u8 *)cyttsp5_security_key, &actual_write_len);
    if (rc) {
...
static int cyttsp5_upgrade_ttconfig(struct device *dev, const u8 *ttconfig_data, int ttconfig_size)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    bool wait_for_calibration_complete = false;
    ...
    if (!rc) {
        parade_debug(dev, DEBUG_LEVEL_1, "%s: TT_CFG updated: rows:%d bytes:%d\n", __func__, row_count, table_size);
    }

    rc = cmd->nonhid_cmd->verify_config_block_crc(dev, 0, ebid, &verify_crc_status, &calculated_crc, &stored_crc);
    if (rc || verify_crc_status) {
        ...
        parade_debug(dev, DEBUG_LEVEL_1, "%s: CRC PASS, ebid=%d, status=%d, scrc=%X ccrc=%X\n", __func__, ebid, verify_crc_status, calculated_crc, stored_crc);
    }
...
static int cyttsp5_get_ttconfig_crc(struct device *dev, const u8 *ttconfig_data, int ttconfig_size, u16 *crc)
{
    u16 crc_loc;
...
static int cyttsp5_get_ttconfig_version(struct device *dev, const u8 *ttconfig_data, int ttconfig_size, u16 *version)
{
    if (ttconfig_size < CY_TTCONFIG_VERSION_OFFSET + CY_TTCONFIG_VERSION_SIZE) {
        return -EINVAL;
    }

    *version = get_unaligned_le16(&ttconfig_data[CY_TTCONFIG_VERSION_OFFSET]);

    return 0;
}

static int cyttsp5_check_ttconfig_version(struct device *dev, const u8 *ttconfig_data, int ttconfig_size)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    u16 cfg_crc_new;
    int rc;
    ...
    if (ld->loader_pdata->flags &
        ...
        rc = cyttsp5_get_ttconfig_version(dev, ttconfig_data, ttconfig_size, &cfg_ver_new);
        if (rc) {
            return 0;
        }
        ...
        rc = cyttsp5_get_ttconfig_crc(dev, ttconfig_data, ttconfig_size, &cfg_crc_new);
        if (rc) {
            return 0;
        }
...
static int cyttsp5_check_ttconfig_version_platform(struct device *dev, struct cyttsp5_touch_config *ttconfig)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    u32 fw_ver_config;
    ...
    return cyttsp5_check_ttconfig_version(dev, ttconfig->param_regs->data, ttconfig->param_regs->size);
}

static struct cyttsp5_touch_config *cyttsp5_get_platform_ttconfig(struct device *dev)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
...
static int upgrade_ttconfig_from_platform(struct device *dev)
{
    ...
    if (upgrade) {
        return cyttsp5_upgrade_ttconfig(dev, param_regs->data, param_regs->size);
    }
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_MANUAL_TTCONFIG_UPGRADE
static ssize_t cyttsp5_config_data_write(struct file *filp, struct kobject *kobj, struct bin_attribute *bin_attr, char *buf, loff_t offset, size_t count)
{
...
static ssize_t cyttsp5_config_loading_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    bool config_loading;
...
static int cyttsp5_verify_ttconfig_binary(struct device *dev, u8 *bin_config_data, int bin_config_size, u8 **start, int *len)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    int header_size;
    u16 config_size;
 ...
 *-1: Exit loading
 */
static ssize_t cyttsp5_config_loading_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    long value;
    u8 *start;
    ...
    if (value == 1) {
        ...
        rc = cyttsp5_verify_ttconfig_binary(dev, ld->config_data, ld->config_size, &start, &length);
        if (rc) {
            goto exit_free;
        }
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_FW_UPGRADE
static ssize_t cyttsp5_forced_upgrade_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
...
static DEVICE_ATTR(forced_upgrade, S_IWUSR, NULL, cyttsp5_forced_upgrade_store);
#endif

#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE
static ssize_t cyttsp5_manual_upgrade_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
    struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
    int rc;
...
static DEVICE_ATTR(manual_upgrade, S_IWUSR, NULL, cyttsp5_manual_upgrade_store);
#endif
...
#endif
    cmd->subscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_LOADER_NAME, cyttsp5_loader_attention, CY_MODE_BOOTLOADER);
    cmd->subscribe_attention(dev, CY_ATTEN_LOADER, CYTTSP5_LOADER_NAME, cyttsp5_fw_upgrade_cb, CY_MODE_UNKNOWN);
#endif
#if CYTTSP5_FW_UPGRADE || CYTTSP5_TTCONFIG_UPGRADE
    init_completion(&ld->calibration_complete);
...
#if CYTTSP5_FW_UPGRADE
    cmd->unsubscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_LOADER_NAME, cyttsp5_loader_attention, CY_MODE_BOOTLOADER);
    cmd->unsubscribe_attention(dev, CY_ATTEN_LOADER, CYTTSP5_LOADER_NAME, cyttsp5_fw_upgrade_cb, CY_MODE_UNKNOWN);
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_MANUAL_TTCONFIG_UPGRADE
