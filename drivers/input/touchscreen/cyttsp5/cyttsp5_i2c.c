...
static int cyttsp5_i2c_write_read_specific(struct device *dev, u8 write_len, u8 *write_buf, u8 *read_buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct i2c_msg msgs[2];
    u8 msg_count = 1;
    ...
    if (read_buf) {
        rc = cyttsp5_i2c_read_default_nosize(dev, read_buf, CY_I2C_DATA_SIZE);
    }
...
static int cyttsp5_i2c_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
    ...
    rc = cyttsp5_probe(&cyttsp5_i2c_bus_ops, &client->dev, client->irq, CY_I2C_DATA_SIZE);
