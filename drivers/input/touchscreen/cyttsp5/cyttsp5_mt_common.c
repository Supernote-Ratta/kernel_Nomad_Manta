...
static void cyttsp5_mt_process_touch(struct cyttsp5_mt_data *md, struct cyttsp5_touch *touch)
{
    struct device *dev = md->dev;
    struct cyttsp5_sysinfo *si = md->si;
    int tmp;
    ...
    if (md->pdata->flags & CY_MT_FLAG_FLIP) {
        ...
        if (touch->abs[CY_TCH_OR] > 0) {
            touch->abs[CY_TCH_OR] = md->or_max - touch->abs[CY_TCH_OR];
        } else {
            touch->abs[CY_TCH_OR] = md->or_min - touch->abs[CY_TCH_OR];
        }
        flipped = true;
    } else {
        flipped = false;
    ...
    if (md->pdata->flags & CY_MT_FLAG_INV_X) {
        if (flipped) {
            touch->abs[CY_TCH_X] = si->sensing_conf_data.res_y - touch->abs[CY_TCH_X];
        } else {
            touch->abs[CY_TCH_X] = si->sensing_conf_data.res_x - touch->abs[CY_TCH_X];
        }
        touch->abs[CY_TCH_OR] *= -1;
    }
    if (md->pdata->flags & CY_MT_FLAG_INV_Y) {
        if (flipped) {
            touch->abs[CY_TCH_Y] = si->sensing_conf_data.res_x - touch->abs[CY_TCH_Y];
        } else {
            touch->abs[CY_TCH_Y] = si->sensing_conf_data.res_y - touch->abs[CY_TCH_Y];
        }
        touch->abs[CY_TCH_OR] *= -1;
    }
...
static void cyttsp5_get_mt_touches(struct cyttsp5_mt_data *md, struct cyttsp5_touch *tch, int num_cur_tch)
{
    ...
    for (i = 0; i < num_cur_tch; i++) {
        ...
        if (tch->abs[CY_TCH_O] == CY_OBJ_HOVER) {
            ...
            cyttsp5_report_event(md, CY_ABS_D_OST, tch->abs[CY_TCH_P]);
            tch->abs[CY_TCH_P] = 0;
        } else {
        ...
        for (j = 0; j <= CY_ABS_W_OST; j++) {
            ...
            cyttsp5_report_event(md, CY_ABS_X_OST + j, tch->abs[CY_TCH_X + j]);
        }
...
cyttsp5_get_mt_touches_pr_tch:
    ...
    if (md->mt_function.final_sync) {
        md->mt_function.final_sync(md->input, si->tch_abs[CY_TCH_T].max, mt_sync_count, ids);
    }
...
static void cyttsp5_mt_send_dummy_event(struct cyttsp5_core_data *cd, struct cyttsp5_mt_data *md)
{
    ...
    /* for easy wakeup */
    if (md->mt_function.input_report) {
        md->mt_function.input_report(md->input, ABS_MT_TRACKING_ID, 0, CY_OBJ_STANDARD_FINGER);
    }
    if (md->mt_function.input_sync) {
        md->mt_function.input_sync(md->input);
    }
...
static int cyttsp5_mt_open(struct input_dev *input)
{
    ...
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_MT_NAME, cyttsp5_mt_attention, CY_MODE_OPERATIONAL);

    /* set up startup call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_MT_NAME, cyttsp5_startup_attention, 0);

    /* set up wakeup call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_WAKE, CYTTSP5_MT_NAME, cyttsp5_mt_wake_attention, 0);

    /* set up suspend call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_SUSPEND, CYTTSP5_MT_NAME, cyttsp5_mt_suspend_attention, 0);

    /* set up resume call back */
    _cyttsp5_subscribe_attention(dev, CY_ATTEN_RESUME, CYTTSP5_MT_NAME, cyttsp5_mt_resume_attention, 0);
...
static void cyttsp5_mt_close(struct input_dev *input)
{
    ...
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_MT_NAME, cyttsp5_mt_attention, CY_MODE_OPERATIONAL);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_MT_NAME, cyttsp5_startup_attention, 0);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_WAKE, CYTTSP5_MT_NAME, cyttsp5_mt_wake_attention, 0);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_SUSPEND, CYTTSP5_MT_NAME, cyttsp5_mt_suspend_attention, 0);
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_RESUME, CYTTSP5_MT_NAME, cyttsp5_mt_resume_attention, 0);
...
static int cyttsp5_setup_input_attention(struct device *dev)
{
    ...
    _cyttsp5_unsubscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_MT_NAME, cyttsp5_setup_input_attention, 0);
...
int cyttsp5_mt_probe(struct device *dev)
{
    ...
    scnprintf(md->phys, sizeof(md->phys), "%s/input%d", dev_name(dev),  cd->phys_num++);
    ...
    if (md->si) {
        ...
        _cyttsp5_subscribe_attention(dev, CY_ATTEN_STARTUP, CYTTSP5_MT_NAME, cyttsp5_setup_input_attention, 0);
    }
