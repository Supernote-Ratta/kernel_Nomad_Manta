...
#endif
static struct pt_core_data *priv_data;
 ...
 ******************************************************************************/
static int request_exclusive(struct pt_core_data *cd, void *ownptr,
		int timeout_ms)
{
 ...
 *	*ownptr     - pointer to device
 ******************************************************************************/
static int release_exclusive(struct pt_core_data *cd, void *ownptr)
{
	int rc;
...
static int pt_core_early_suspend(struct tp_device *tp_d)
{
...
static int pt_core_late_resume(struct tp_device *tp_d)
{
    struct pt_core_data *cd = container_of(tp_d, struct pt_core_data, tp);
    int ret;
    printk("%s\n", __func__);
...
const struct dev_pm_ops pt_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(pt_core_suspend, pt_core_resume)
		...
		unsigned long event, void *data)
{
    ...
    printk("pt_fb_notifier_callback :event=:%d blank:%d \n",event,*blank);
...
#endif
    cd->tp.tp_resume = pt_core_late_resume;
    cd->tp.tp_suspend = pt_core_early_suspend;
