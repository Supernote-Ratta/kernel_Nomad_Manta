...
int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
    ...
    */
    ...
    /*get reset resource*/
    gf_dev->reset_gpio = of_get_named_gpio_flags(gf_dev->spi->dev.of_node, "goodix,gpio_reset", 0, &flags);
    if (!gpio_is_valid(gf_dev->reset_gpio)) {
        gf_debug(ERR_LOG, "%s, RESET GPIO is invalid.\n", __func__);
    } else {
        gf_dev->reset_level = (flags & OF_GPIO_ACTIVE_LOW) ? 0 : 1;
...
void gf_hw_power_enable(u8 onoff)
{
    /* TODO: LDO configure */
    static int enable = 1;
...
void gf_hw_reset(struct gf_device *gf_dev, u8 delay)
{
    ...
    if (gpio_is_valid(gf_dev->reset_gpio)) {
        gpio_direction_output(gf_dev->reset_gpio, gf_dev->reset_level);
        gpio_set_value(gf_dev->reset_gpio, gf_dev->reset_level);
        mdelay(delay);
        gpio_set_value(gf_dev->reset_gpio, !gf_dev->reset_level);
    }
}
