...
int ff_spi_init(void)
{
    ...
    FF_LOGI("spi mode 0x%x, %d bits %sper word, %d Hz max.", mode, bits, lsb ? "(lsb first) " : "", g_spidev->max_speed_hz);
...
#if VERBOSE_SPI_DATA
        /* Verbose TX buffer. */
        ff_log_printf(FF_LOG_LEVEL_DBG, LOG_TAG, "ff_spi_WRITE_then_read_buf(%p, %d, ..):", tx_buf, tx_len);
...
#if VERBOSE_SPI_DATA
        /* Verbose RX buffer. */
        ff_log_printf(FF_LOG_LEVEL_DBG, LOG_TAG, "ff_spi_write_then_READ_buf(.., %p, %d):", rx_buf, rx_len);
