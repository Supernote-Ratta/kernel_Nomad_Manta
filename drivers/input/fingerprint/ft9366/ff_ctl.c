...
#define FF_COMPATIBLE_NODE_2 "rockchip,rk3066-spi"
...
int ff_ctl_init_pins(ff_ctl_context_t *g_context)
{
    ...
    FF_LOGD("irq gpio:%d", g_context->irq_gpio);
    if (!gpio_is_valid(g_context->irq_gpio)) {
        FF_LOGE("IRQ GPIO is invalid.");
        return -1;
    }

    rc = gpio_request(g_context->irq_gpio, "fingerprint_irq");
    if (rc) {
        FF_LOGE("Failed to request IRQ GPIO. rc = %d", rc);
        return -1;
    }
...
int ff_ctl_free_pins(void)
{
    ...
    if (gpio_is_valid(g_context->irq_gpio)) {
	    gpio_free(g_context->irq_gpio);
    }
    if (gpio_is_valid(g_context->reset_gpio)) {
	    gpio_free(g_context->reset_gpio);
    }
    if (gpio_is_valid(g_context->cs_gpio)) {
	    gpio_free(g_context->cs_gpio);
    }
...
{
    int err = 0;
    FF_LOGV("'%s' enter. clock: '%s'.", __func__, on ? "enable" : "disabled");
    return err;
}
...
static int ff_ctl_init_driver(void)
{
    ...
    do {
        ...
        err = request_irq(g_context->irq_num, ff_ctl_device_irq, IRQF_TRIGGER_RISING | IRQF_ONESHOT, "ff_irq", (void *)g_context);
        if (err) {
