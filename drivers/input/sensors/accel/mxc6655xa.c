...
#define MXC6655_RANGE                        (16384 * 2)  //16bit->16384即+-2g为+-32768
...
static int sensor_init(struct i2c_client *client)
{
    ...
    if (result) {
        dev_err(&client->dev, "%s:fail to set MXC6655_CONTROL.\n", __func__);
        return result;
    }
    ...
    if (result) {
        dev_err(&client->dev, "%s:fail to set MXC6655_INT_MASK1.\n", __func__);
        return result;
    }
...
static int sensor_convert_data(struct i2c_client *client, char high_byte, char low_byte)
{
    ...
    result = ((int)high_byte << (MXC6655_PRECISION - 8)) | ((int)low_byte >> (16 - MXC6655_PRECISION));
    if (result < MXC6655_BOUNDARY) {
        result = result * MXC6655_GRAVITY_STEP;
    } else {
        result = ~(((~result & (0x7fff >> (16 - MXC6655_PRECISION))) + 1) * MXC6655_GRAVITY_STEP) + 1;
    }
...
static int sensor_report_value(struct i2c_client *client)
{
    ...
    *buffer = sensor->ops->read_reg;
    ...
    if (ret < 0) {
        dev_err(&client->dev, "mxc6655 read data failed, ret = %d\n", ret);
        return ret;
    }
