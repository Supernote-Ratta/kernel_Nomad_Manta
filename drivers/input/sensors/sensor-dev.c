...
static ssize_t accel_calibration_show(struct class *class, struct class_attribute *attr, char *buf)
{
    int ret;
    ...
    if (sensor_cali_data.is_accel_calibrated == 1) {
        return sprintf(buf, "accel calibration: %d, %d, %d\n", sensor_cali_data.accel_offset[0], sensor_cali_data.accel_offset[1], sensor_cali_data.accel_offset[2]);
    }
...
#define ACCEL_OFFSET_XY_MAX 3276
...
static int accel_do_calibration(struct sensor_private_data *sensor)
{
    ...
    sensor_cali_data.accel_offset[2] = 0;//sensor_cali_data.accel_offset[2] > 0 ? sensor_cali_data.accel_offset[2] - ACCEL_SENSITIVE : sensor_cali_data.accel_offset[2] + ACCEL_SENSITIVE;
...
static ssize_t accel_calibration_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
    struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_ACCEL];
    int val, ret;
    int pre_status;
...
static ssize_t gyro_calibration_show(struct class *class, struct class_attribute *attr, char *buf)
{
    int ret;
...
static ssize_t gyro_calibration_store(struct class *class, struct class_attribute *attr, const char *buf, size_t count)
{
    struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_GYROSCOPE];
    int val, ret;
    int pre_status;
...
{
    struct sensor_private_data *sensor = container_of(h, struct sensor_private_data, early_suspend);
...
/* ioctl - I/O control */
static long angle_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
/* ioctl - I/O control */
static long gyro_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    struct sensor_private_data *sensor = g_sensor[SENSOR_TYPE_GYROSCOPE];
    struct i2c_client *client = sensor->client;
    void __user *argp = (void __user *)arg;
...
/* ioctl - I/O control */
static long proximity_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
/* ioctl - I/O control */
static long temperature_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
/* ioctl - I/O control */
static long pressure_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
static int sensor_misc_device_register(struct sensor_private_data *sensor, int type)
{
    ...
    if (result < 0) {
        dev_err(&sensor->client->dev, "fail to register misc device %s\n", sensor->miscdev.name);
        goto error;
    }
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
    ...
    if (result) {
        dev_err(&client->dev, "fail to init sensor irq,ret=%d\n", result);
        goto out_input_register_device_failed;
    }
    ...
    if (result) {
        dev_err(&client->dev, "fail to register misc device %s\n", sensor->miscdev.name);
...
int sensor_register_device(struct i2c_client *client, struct sensor_platform_data *slave_pdata, const struct i2c_device_id *devid, struct sensor_operate *ops)
{
    int result = 0;
...
int sensor_unregister_device(struct i2c_client *client, struct sensor_platform_data *slave_pdata, struct sensor_operate *ops)
{
    int result = 0;
