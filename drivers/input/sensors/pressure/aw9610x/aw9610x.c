/*
*
* Copyright (c) 2020-2022 AWINIC Technology CO., LTD
*
...
*
******************************************************/
static int32_t i2c_write(struct aw9610x *aw9610x, uint16_t reg_addr16, uint32_t reg_data32)
{
    int32_t ret =  -ENOMEM;
    ...
    if (ret < 0) {
        AWLOGE(aw9610x->dev, "Write reg is 0x%x,error value = %d", reg_addr16, ret);
    }

    return ret;
}

static int32_t i2c_read(struct aw9610x *aw9610x, uint16_t reg_addr16, uint32_t *reg_data32)
{
    int32_t ret =  -ENOMEM;
    struct i2c_client *i2c = aw9610x->i2c;
    ...
    if (ret < 0) {
        AWLOGE(aw9610x->dev, "Read reg is 0x%x,error value = %d", reg_addr16, ret);
    }

    reg_data32[0] = ((u32)buf[3]) | ((u32)buf[2] << 8) | ((u32)buf[1] << 16) | ((u32)buf[0] << 24);

    return ret;
}

static int32_t aw9610x_i2c_write(struct aw9610x *aw9610x, uint16_t reg_addr16, uint32_t reg_data32)
{
    int32_t ret = -1;
    uint8_t cnt = 0;
    ...
    while (cnt < AW_I2C_RETRIES) {
        ...
        if (ret < 0) {
            AWLOGE(aw9610x->dev, "write cnt = %d,error = %d", cnt, ret);
        } else {
            break;
        }
...
static int32_t aw9610x_i2c_read(struct aw9610x *aw9610x, uint16_t reg_addr16, uint32_t *reg_data32)
{
    int32_t ret = -1;
    uint8_t cnt = 0;
    ...
    while (cnt < AW_I2C_RETRIES) {
        ...
        if (ret < 0) {
            AWLOGE(aw9610x->dev, "i2c_read cnt=%d error=%d", cnt, ret);
        } else {
            break;
        }
        cnt++;
...
static int32_t aw9610x_i2c_write_bits(struct aw9610x *aw9610x, uint16_t reg_addr16, uint32_t mask, uint32_t reg_data32)
{
    uint32_t reg_val;
...
static void aw9610x_addrblock_load(struct device *dev, const char *buf)
{
...
static int32_t aw9610x_filedata_deal(struct aw9610x *aw9610x)
{
    ...
    if (ret < 0) {
        ...
        for (i = 0; i < AW_SPE_REG_NUM; i++) {
            ...
            if (sscanf(temp_buf, "%08x", &aw9610x->nvspe_data[i]) == 1) {
                AWLOGD(aw9610x->dev, "nv_spe_data[%d] = 0x%08x", i, aw9610x->nvspe_data[i]);
            }
        }
    }
    ...
    if (nv_flag == 0) {
        aw9610x->cali_flag = AW_CALI;
        AWLOGI(aw9610x->dev, "the chip need to cali! nv_flag = 0x%08x", nv_flag);
    } else {
        aw9610x->cali_flag = AW_NO_CALI;
        AWLOGI(aw9610x->dev, "chip not need to cali! nv_flag = 0x%08x", nv_flag);
    }
...
static void aw9610x_get_calidata(struct aw9610x *aw9610x)
{
    ...
    for (i = 0; i < AW_CLA1_SPE_REG_NUM; i++) {
        aw9610x_i2c_read(aw9610x, REG_AFECFG1_CH0 + i * AW_CL1SPE_CALI_OS, &aw9610x->spedata[i]);
    }

    /*class 2 special reg*/
    for (; i < AW_SPE_REG_NUM; i++) {
        aw9610x_i2c_read(aw9610x, REG_ATCCR0 + (i - AW_CHANNEL_MAX) * AW_CL2SPE_CALI_OS, &aw9610x->spedata[i]);
    }
    ...
    for (i = AW_CLA1_SPE_REG_NUM; i < AW_SPE_REG_NUM; i++) {
        ...
        switch (ret) {
            case AW_CHANNEL0:
                aw9610x_i2c_read(aw9610x, REG_COMP_CH0, &reg_val);
                break;
            case AW_CHANNEL1:
                aw9610x_i2c_read(aw9610x, REG_COMP_CH1, &reg_val);
                break;
            case AW_CHANNEL2:
                aw9610x_i2c_read(aw9610x, REG_COMP_CH2, &reg_val);
                break;
            case AW_CHANNEL3:
                aw9610x_i2c_read(aw9610x, REG_COMP_CH3, &reg_val);
                break;
            case AW_CHANNEL4:
                aw9610x_i2c_read(aw9610x, REG_COMP_CH4, &reg_val);
                break;
            case AW_CHANNEL5:
                aw9610x_i2c_read(aw9610x, REG_COMP_CH5, &reg_val);
                break;
            default:
                return;
        }
        aw9610x->spedata[i] = ((reg_val >> 6) & 0x03fffff0) | (aw9610x->spedata[i] & 0xfc00000f);
    }
    /* spedatas come from register*/

    /* write spedatas to nv */
    for (i = 0; i < AW_SPE_REG_NUM; i++) {
        snprintf(temp_buf, sizeof(temp_buf), "%08x", aw9610x->spedata[i]);
...
static void aw9610x_class1_reg(struct aw9610x *aw9610x)
{
    ...
    for (i = 0; i < AW_CLA1_SPE_REG_NUM; i++) {
        ...
        aw9610x_i2c_write_bits(aw9610x, REG_INITPROX0_CH0 + i * AW_CL1SPE_DEAL_OS, ~(0xffff), reg_val);
    }
}
...
static void aw9610x_class2_reg(struct aw9610x *aw9610x)
{
    ...
    for (i = AW_CLA1_SPE_REG_NUM; i < AW_SPE_REG_NUM; i++) {
        aw9610x_i2c_write(aw9610x, REG_ATCCR0 + (i - AW_CLA1_SPE_REG_NUM) * AW_CL2SPE_DEAL_OS, aw9610x->nvspe_data[i]);
    }
}
...
static int32_t aw9610x_input_init(struct aw9610x *aw9610x, uint8_t *err_num)
{
    ...
    aw9610x->channels_arr = devm_kzalloc(aw9610x->dev, sizeof(struct aw_channels_info) * AW_CHANNEL_MAX, GFP_KERNEL);
    ...
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        snprintf(aw9610x->channels_arr[i].name, sizeof(aw9610x->channels_arr->name), "aw_sar%d_ch%d", aw9610x->sar_num, i);
...
static void aw9610x_reg_version_comp(struct aw9610x *aw9610x, struct aw_bin *aw_bin)
{
    uint8_t i = 0;
    ...
    (aw_bin->header_info[0].chip_type[7] == '\0')) {
        ...
        for (i = 0; i < 6; i++) {
            ...
            if (blfilt1_tmp == 1) {
                ...
                aw9610x_i2c_write_bits(aw9610x, REG_BLRSTRNG_CH0 + (0x3c * i), ~(0x3f), 1 << i);
            }
        }
    }
}

static void aw9610x_bin_valid_loaded(struct aw9610x *aw9610x, struct aw_bin *aw_bin_data_s)
{
    uint32_t i;
    int32_t ret = 0;
    uint16_t reg_addr;
...
static void aw9610x_cfg_all_loaded(const struct firmware *cont, void *context)
{
    ...
    if (!cont) {
        ...
        AWLOGI(aw9610x->dev, "%s request successfully", aw9610x->cfg_name);
    }
...
static int32_t aw9610x_cfg_update(struct aw9610x *aw9610x)
{
    ...
    if (aw9610x->firmware_flag == true) {
        snprintf(aw9610x->cfg_name, sizeof(aw9610x->cfg_name), "aw9610x_%d.bin", aw9610x->sar_num);
        request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG, aw9610x->cfg_name, aw9610x->dev, GFP_KERNEL, aw9610x, aw9610x_cfg_all_loaded);
    } else {
...
static int32_t aw9610x_sar_cfg_init(struct aw9610x *aw9610x, int32_t flag)
{
    ...
    schedule_delayed_work(&aw9610x->cfg_work, msecs_to_jiffies(cfg_timer_val));
    ...
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        if (((status1 >> i) & 0x01) == 1) {
            if (aw9610x->satu_flag[i] == 0) {
                ret = aw9610x_i2c_read(aw9610x, REG_BLFILT_CH0 + i * AW_CL1SPE_DEAL_OS, &aw9610x->satu_data[i]);
                if (ret < 0) {
                    return ret;
                }
                ret = aw9610x_i2c_write(aw9610x, REG_BLFILT_CH0 + i * AW_CL1SPE_DEAL_OS, ((aw9610x->satu_data[i] | 0x1fc) & 0x3fffffff));
                if (ret < 0) {
                    return ret;
                }
            ...
            if (aw9610x->satu_flag[i] == 1) {
                if (((status0 >> (i + 24)) & 0x01) == 0) {
                    ret = aw9610x_i2c_write(aw9610x, REG_BLFILT_CH0 + i * AW_CL1SPE_DEAL_OS, aw9610x->satu_data[i]);
                    if (ret < 0) {
                        return ret;
                    }
 ...
 *
 ******************************************************/
static ssize_t aw9610x_set_reg(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    uint32_t databuf[2] = { 0, 0 };

    if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
        aw9610x_i2c_write(aw9610x, (uint16_t)databuf[0], (uint32_t)databuf[1]);
    }

    return count;
}

static ssize_t aw9610x_get_reg(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;
    uint32_t i = 0;
    ...
    for (i = 0; i < reg_num; i++) {
        if (aw9610x_reg_access[i].rw & REG_RD_ACCESS) {
            aw9610x_i2c_read(aw9610x, aw9610x_reg_access[i].reg, &reg_val);
            len += snprintf(buf + len, PAGE_SIZE - len, "reg:0x%04x=0x%08x\n", aw9610x_reg_access[i].reg, reg_val);
        }
    }
...
static ssize_t aw9610x_diff_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;
    uint8_t i = 0;
...
static ssize_t aw9610x_parasitic_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;
    uint8_t i = 0;
    ...
    for (i = 0; i < AW_CHANNEL_MAX; i++) {
        ...
        snprintf(temp_data, sizeof(temp_data), "%d.%d", coff_data_int, coff_data_dec);
...
static ssize_t aw9610x_awrw_get(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
...
static ssize_t aw9610x_factory_cali_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ...
    if (sscanf(buf, "%d", &databuf[0]) == 1) {
        if ((databuf[0] == 1) && (aw9610x->pwprox_dete == true)) {
            ...
            AWLOGE(aw9610x->dev, "aw_unsupport the pw_prox_dete=%d", aw9610x->pwprox_dete);
            return count;
        }
...
static ssize_t aw9610x_power_prox_get(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;

    if (aw9610x->pwprox_dete == false) {
        len += snprintf(buf + len, PAGE_SIZE - len, "unsupport powerprox!");
        return len;
    }
...
static ssize_t aw9610x_awrw_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
...
static ssize_t aw9610x_set_update(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ssize_t ret;
    uint32_t state;
...
static ssize_t aw9610x_aot_cali_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ssize_t ret;
    uint32_t state;
    uint32_t data_en = 0;
...
static ssize_t aw9610x_get_satu(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;

    if (aw9610x->satu_release != 0) {
        len += snprintf(buf + len, PAGE_SIZE - len, "satu_ralease function is supporting! the flag = %d\n", aw9610x->satu_release);
    } else {
        len += snprintf(buf + len, PAGE_SIZE - len, "satu_ralease function unsupport! the flag = %d\n", aw9610x->satu_release);
    }

    return len;
}

static ssize_t aw9610x_set_satu(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ssize_t ret;
    uint32_t state;
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
...
static ssize_t aw9610x_operation_mode_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ssize_t ret = 0;
    ...
    if (aw9610x->mode == AW9610X_ACTIVE_MODE && aw9610x->old_mode != AW9610X_ACTIVE_MODE) {
        ...
        aw9610x_i2c_write(aw9610x, REG_CMD, AW9610X_ACTIVE_MODE);
    } else if (aw9610x->mode == AW9610X_SLEEP_MODE && aw9610x->old_mode != AW9610X_SLEEP_MODE) {
        if (aw9610x->old_mode == AW9610X_DEEPSLEEP_MODE) {
            aw9610x_i2c_write(aw9610x, REG_OSCEN, AW9610X_CPU_WORK_MASK);
        } else {
    ...
    } else if ((aw9610x->mode == AW9610X_DEEPSLEEP_MODE) && aw9610x->old_mode != AW9610X_DEEPSLEEP_MODE && ((aw9610x->vers == AW9610XA) || (aw9610x->vers == AW9610XB))) {
        if (aw9610x->old_mode != AW9610X_SLEEP_MODE) {
            disable_irq(aw9610x->to_irq);
            aw9610x_i2c_read(aw9610x, REG_IRQSRC, &irq_status_temp);
        }
...
static ssize_t aw9610x_operation_mode_get(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;
    ...
    } else if ((aw9610x->mode == AW9610X_DEEPSLEEP_MODE) && ((aw9610x->vers == AW9610XA) || (aw9610x->vers == AW9610XB))) {
...
static ssize_t aw9610x_chip_info_get(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw9610x *aw9610x = dev_get_drvdata(dev);
    ssize_t len = 0;
    uint32_t reg_data = 0;
...
void aw9610x_int_output(struct aw9610x *aw9610x, int32_t level)
{
    ...
    if (level == 0) {
        if (aw9610x->pinctrl.pinctrl) {
            pinctrl_select_state(aw9610x->pinctrl.pinctrl, aw9610x->pinctrl.int_out_low);
        } else {
            pr_info("%s Failed set int pin output low\n", __func__);
        }
    } else if (level == 1) {
        if (aw9610x->pinctrl.pinctrl) {
            pinctrl_select_state(aw9610x->pinctrl.pinctrl, aw9610x->pinctrl.int_out_high);
        } else {
...
static int32_t aw9610x_pinctrl_init(struct aw9610x *aw9610x)
{
    ...
    snprintf(pin_default_name, sizeof(pin_default_name), "aw_default_sar%d", aw9610x->sar_num);
    ...
    snprintf(pin_output_high_name, sizeof(pin_output_high_name), "aw_int_output_high_sar%d", aw9610x->sar_num);
    ...
    snprintf(pin_output_low_name, sizeof(pin_output_low_name), "aw_int_output_low_sar%d", aw9610x->sar_num);
...
static int32_t aw9610x_interrupt_init(struct aw9610x *aw9610x)
{
    ...
    snprintf(irq_gpio_name, sizeof(irq_gpio_name), "aw9610x_irq_gpio%d", aw9610x->sar_num);

    if (gpio_is_valid(aw9610x->irq_gpio)) {
        aw9610x->to_irq = gpio_to_irq(aw9610x->irq_gpio);

        ret = devm_gpio_request_one(aw9610x->dev, aw9610x->irq_gpio, GPIOF_DIR_IN | GPIOF_INIT_HIGH, irq_gpio_name);
        if (ret) {
            AWLOGE(aw9610x->dev, "request irq gpio failed, ret = %d", ret);
            ret = -AW_IRQIO_FAILED;
        } else {
            /* register irq handler */
            irq_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;
            ret = devm_request_threaded_irq(&aw9610x->i2c->dev, aw9610x->to_irq, NULL, aw9610x_irq, irq_flags, "aw9610x_irq", aw9610x);
            if (ret != 0) {
                AWLOGE(aw9610x->dev, "failed to request IRQ %d: %d", aw9610x->to_irq, ret);
                ret = -AW_IRQ_REQUEST_FAILED;
            } else {
                AWLOGI(aw9610x->dev, "IRQ request successfully!");
                ret = AW_SAR_SUCCESS;
            }
        }
    } else {
 ...
 *
 *****************************************************/
static int32_t aw9610x_parse_dt(struct device *dev, struct aw9610x *aw9610x, struct device_node *np)
{
    uint32_t val = 0;
    ...
    aw9610x->firmware_flag = of_property_read_bool(np, "aw9610x,using-firmware");
    AWLOGI(aw9610x->dev, "firmware_flag = <%d>", aw9610x->firmware_flag);

    aw9610x->pwprox_dete = of_property_read_bool(np, "aw9610x,using-pwon-prox-dete");
    AWLOGI(aw9610x->dev, "pwprox_dete = <%d>", aw9610x->pwprox_dete);

    aw9610x->satu_release = of_property_read_bool(np, "aw9610x,using-satu");
...
static int32_t aw9610x_power_init(struct aw9610x *aw9610x)
{
    ...
    if (regulator_count_voltages(aw9610x->vcc) > 0) {
        rc = regulator_set_voltage(aw9610x->vcc, AW_VCC_MIN_UV, AW_VCC_MAX_UV);
        if (rc) {
            AWLOGE(aw9610x->dev, "regulator set vol failed rc = %d", rc);
            goto reg_vcc_put;
        }
    }
...
static void aw9610x_power_enable(struct aw9610x *aw9610x, bool on)
{
    ...
    if (on) {
        ...
        if (rc) {
            AWLOGE(aw9610x->dev, "regulator_enable vol failed rc = %d", rc);
        } else {
        ...
        if (rc) {
            AWLOGE(aw9610x->dev, "regulator_disable vol failed rc = %d", rc);
        } else {
...
static int32_t aw9610x_read_chipid(struct aw9610x *aw9610x)
{
    ...
    if (reg_val == AW9610X_CHIP_ID) {
        ...
        AWLOGE(aw9610x->dev, "unsupport dev, chipid is (0x%04x)", reg_val);
    }
...
static void aw9610x_ps_notify_callback_work(struct work_struct *work)
{
    ...
    aw9610x_i2c_read(aw9610x, REG_SCANCTRL0, &data_en);
    ret = aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8), (data_en & 0x3f) << 8);
    if (ret < 0) {
...
static int aw9610x_ps_get_state(struct aw9610x *aw9610x, struct power_supply *psy, bool *present)
{
    ...
    *present = (pval.intval) ? true : false;
    AWLOGI(aw9610x->dev, "%s is %s", psy->desc->name, (*present) ? "present" : "not present");
    return 0;
}

static int aw9610x_ps_notify_callback(struct notifier_block *self, unsigned long event, void *p)
{
...
static int aw9610x_ps_notify_init(struct aw9610x *aw9610x)
{
    ...
    if (psy) {
        ...
        if (ret) {
            AWLOGE(aw9610x->dev, "psy get property failed rc=%d", ret);
