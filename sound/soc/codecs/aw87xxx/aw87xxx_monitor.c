 ...
 *
 ****************************************************************************/
static int aw_monitor_check_header_v_1_0_0(struct device *dev, char *data, uint32_t data_len)
{
    int i = 0;
...
static int aw_monitor_check_data_v1_size(struct device *dev, char *data, int32_t data_len)
{
...
static int aw_monitor_check_data_size(struct device *dev, char *data, int32_t data_len)
{
    int ret = -1;
    struct aw_bin_header *header = (struct aw_bin_header *)data;
    ...
    switch (header->bin_data_ver) {
        case AW_MONITOR_DATA_VER:
            ...
            break;
        default:
            AW_DEV_LOGE(dev, "bin data_ver[0x%x] non support", header->bin_data_ver);
            return -EINVAL;
    }
...
static int aw_monitor_check_bin_header(struct device *dev, char *data, int32_t data_len)
{
    int ret = -1;
    ...
    switch (header->header_ver) {
        case HEADER_VERSION_1_0_0:
            ...
            break;
        default:
            AW_DEV_LOGE(dev, "bin version[0x%x] non support", header->header_ver);
            return -EINVAL;
    }
...
static int aw_monitor_bin_check_sum(struct device *dev, char *data, int32_t data_len)
{
    int i, data_sum = 0;
    ...
    if (*check_sum != data_sum) {
        AW_DEV_LOGE(dev, "check_sum[%d] is not equal to data_sum[%d]", *check_sum, data_sum);
        return -ENOMEM;
    }
...
static int aw_monitor_bin_check(struct device *dev, char *monitor_data, uint32_t data_len)
{
    int ret = -1;
 ...
 *
 *****************************************************************************/
static void aw_monitor_write_to_table_v1(struct device *dev, struct vmax_step_config *vmax_step, char *vmax_data, uint32_t step_count)
{
    int i = 0;
    int index = 0;
    ...
    for (i = 0; i < step_count; i++) {
        ...
        vmax_step[i].vbat_min =
            AW_GET_32_DATA(vmax_data[index + 3], vmax_data[index + 2], vmax_data[index + 1], vmax_data[index + 0]);
        vmax_step[i].vbat_max =
            AW_GET_32_DATA(vmax_data[index + 7], vmax_data[index + 6], vmax_data[index + 5], vmax_data[index + 4]);
        vmax_step[i].vmax_vol =
            AW_GET_32_DATA(vmax_data[index + 11], vmax_data[index + 10], vmax_data[index + 9], vmax_data[index + 8]);
    }

    for (i = 0; i < step_count; i++) {
        AW_DEV_LOGI(dev, "vbat_min:%d, vbat_max%d, vmax_vol:0x%x", vmax_step[i].vbat_min, vmax_step[i].vbat_max, vmax_step[i].vmax_vol);
    }
}

static int aw_monitor_parse_vol_data_v1(struct device *dev, struct aw_monitor *monitor, char *monitor_data)
{
    ...
    if (step_count) {
        vmax_step = devm_kzalloc(dev, sizeof(struct vmax_step_config) * step_count, GFP_KERNEL);
        if (vmax_step == NULL) {
            AW_DEV_LOGE(dev, "vmax_cfg vmalloc failed");
            return -ENOMEM;
        }
        memset(vmax_step, 0, sizeof(struct vmax_step_config) * step_count);
    }
...
static int aw_monitor_parse_data_v1(struct device *dev, struct aw_monitor *monitor, char *monitor_data)
{
    int ret = -1;
    int header_len = 0;
    ...
    memcpy(monitor_hdr, monitor_data + header_len, sizeof(struct aw_monitor_header));

    AW_DEV_LOGI(dev, "monitor_switch:%d, monitor_time:%d (ms), monitor_count:%d, step_count:%d", monitor_hdr->monitor_switch, monitor_hdr->monitor_time, monitor_hdr->monitor_count, monitor_hdr->step_count);
...
static int aw_monitor_parse_v_1_0_0(struct device *dev, struct aw_monitor *monitor, char *monitor_data)
{
    int ret = -1;
...
void aw87xxx_monitor_cfg_free(struct aw_monitor *monitor)
{
    ...
    memset(&monitor->monitor_hdr, 0, sizeof(struct aw_monitor_header));
    if (monitor->vmax_cfg) {
...
int aw87xxx_monitor_bin_parse(struct device *dev, char *monitor_data, uint32_t data_len)
{
    int ret = -1;
    ...
    AW_DEV_LOGI(dev, "monitor bin parse version: %s", AW_MONITOT_BIN_PARSE_VERSION);
    ...
    switch (bin_header->bin_data_ver) {
        case DATA_VERSION_V1:
            ret = aw_monitor_parse_v_1_0_0(dev, monitor, monitor_data);
            if (ret < 0) {
            ...
            break;
        default:
            AW_DEV_LOGE(dev, "Unrecognized this bin data version[0x%x]", bin_header->bin_data_ver);
    }
 ...
 *
 ***************************************************************************/
static int aw_monitor_get_battery_capacity(struct device *dev, struct aw_monitor *monitor, int *vbat_capacity)
{
    char name[] = "battery";
    int ret = -1;
    ...
    *vbat_capacity = prop.intval;
    AW_DEV_LOGI(dev, "The percentage is %d", *vbat_capacity);

    return 0;
}

static int aw_search_vmax_from_table(struct device *dev, struct aw_monitor *monitor, const int vbat_vol, int *vmax_vol)
{
    int i = 0;
    int vmax_set = 0;
    ...
    for (i = 0; i < monitor_hdr->step_count; i++) {
        if (vbat_vol == AW_VBAT_MAX) {
            ...
            vmax_flag = 1;
            AW_DEV_LOGD(dev, "vbat=%d, setting vmax=0x%x", vbat_vol, vmax_set);
            break;
        }
        ...
        if (vbat_vol >= vmax_cfg[i].vbat_min &&
            ...
            vmax_flag = 1;
            AW_DEV_LOGD(dev, "read setting vmax=0x%x, step[%d]: vbat_min=%d,vbat_max=%d", vmax_set, i, vmax_cfg[i].vbat_min, vmax_cfg[i].vbat_max);
            break;
        }
    }
...
static int aw_chip_status_recover(struct aw87xxx *aw87xxx)
{
    ...
    AW_DEV_LOGI(aw87xxx->dev, "current prof[%s], dev_index[%d] ", profile, aw87xxx->dev_index);
 ...
 *
 ***************************************************************************/
static int aw_monitor_update_vmax_to_dsp(struct device *dev, struct aw_monitor *monitor, int vmax_set)
{
    int ret = -1;
    uint32_t enable = 0;
    ...
    if (monitor->pre_vmax != vmax_set) {
        ...
        if (!enable || ret < 0) {
            AW_DEV_LOGE(dev, "get rx failed or rx disable, ret=%d, enable=%d", ret, enable);
            return -EPERM;
        }
...
static void aw_monitor_with_dsp_vmax_work(struct device *dev, struct aw_monitor *monitor)
{
    int ret = -1;
    int vmax_set = 0;
    int vbat_capacity = 0;
    ...
    if (monitor->timer_cnt < monitor_hdr->monitor_count) {
        ...
        AW_DEV_LOGI(dev, "timer_cnt = %d", monitor->timer_cnt);
    }
    if ((monitor->timer_cnt >= monitor_hdr->monitor_count) ||
        (monitor->first_entry == AW_FIRST_ENTRY)) {
        ...
        ret = aw_search_vmax_from_table(dev, monitor, ave_capacity, &vmax_set);
        if (ret < 0) {
...
{
    int ret = 0;
    struct aw87xxx *aw87xxx = container_of(work, struct aw87xxx, monitor.with_dsp_work.work);
    ...
    if (monitor->esd_enable || (monitor_hdr->monitor_switch &&
        ...
        schedule_delayed_work(&monitor->with_dsp_work, msecs_to_jiffies(monitor_hdr->monitor_time));
    }
}
...
void aw87xxx_monitor_start(struct aw_monitor *monitor)
{
    ...
    if (monitor->esd_enable || (monitor->monitor_hdr.monitor_switch &&
                                !(aw87xxx->aw_dev.is_rec_mode) && monitor->open_dsp_en && monitor->bin_status == AW_ACF_UPDATE)) {
        ...
        schedule_delayed_work(&monitor->with_dsp_work, msecs_to_jiffies(monitor->monitor_hdr.monitor_time));
    }
}
/***************************************************************************
 *
 ...
 *
 ***************************************************************************/
static ssize_t aw_attr_get_esd_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_monitor *monitor = &aw87xxx->monitor;
    ...
    if (monitor->esd_enable) {
        ...
        len += snprintf(buf + len, PAGE_SIZE - len, "esd-enable=true\n");
    } else {
        AW_DEV_LOGI(aw87xxx->dev, "esd-enable=false");
        len += snprintf(buf + len, PAGE_SIZE - len, "esd-enable=false\n");
    }

    return len;
}

static ssize_t aw_attr_set_esd_enable(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
    ...
    if (strlen(buf) > AW_ESD_ENABLE_STRLEN) {
        AW_DEV_LOGE(aw87xxx->dev, "input esd_enable_str_len is out of max[%d]", AW_ESD_ENABLE_STRLEN);
        return -EINVAL;
    }
    ...
    if (sscanf(buf, "%s", esd_enable) == 1) {
        ...
        AW_DEV_LOGI(dev, "set esd-enable=[%s]", monitor->esd_enable ? "true" : "false");
    } else {
...
static ssize_t aw_attr_get_vbat(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    int ret = -1;
    int vbat_capacity = 0;
...
static ssize_t aw_attr_set_vbat(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
    int ret = -1;
    int capacity = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    ...
    if (capacity >= AW_VBAT_CAPACITY_MIN &&
        ...
        AW_DEV_LOGE(aw87xxx->dev, "vbat_set=invalid,please input value [%d-%d]", AW_VBAT_CAPACITY_MIN, AW_VBAT_CAPACITY_MAX);
        return -EINVAL;
    }
...
static ssize_t aw_attr_get_vmax(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    int ret = -1;
    int vbat_capacity = 0;
    ...
    if (monitor->open_dsp_en) {
        ...
        if (ret < 0) {
            AW_DEV_LOGE(aw87xxx->dev, "get dsp vmax fail, ret=%d", ret);
            return ret;
        }
        ...
        AW_DEV_LOGI(aw87xxx->dev, "get_battery_capacity is [%d]", vbat_capacity);

        if (monitor->custom_capacity) {
            vbat_capacity = monitor->custom_capacity;
            AW_DEV_LOGI(aw87xxx->dev, "get custom_capacity is [%d]", vbat_capacity);
        }

        ret = aw_search_vmax_from_table(aw87xxx->dev, monitor, vbat_capacity, &vmax_get);
        if (ret < 0) {
            AW_DEV_LOGE(aw87xxx->dev, "not find vmax_vol");
            len += snprintf(buf + len, PAGE_SIZE - len, "not_find_vmax_vol\n");
            return len;
        }
...
static ssize_t aw_attr_set_vmax(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    uint32_t vmax_set = 0;
    int ret = -1;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    ...
    if (monitor->open_dsp_en) {
        ...
        if (ret < 0) {
            AW_DEV_LOGE(aw87xxx->dev, "send dsp_msg error, ret = %d", ret);
            return ret;
        }
        msleep(2);
    } else {
...
static ssize_t aw_attr_get_monitor_switch(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_monitor *monitor = &aw87xxx->monitor;
    ...
    len += snprintf(buf + len, PAGE_SIZE - len, "aw87xxx monitor switch: %d\n", monitor_hdr->monitor_switch);
    return len;
}
...
static ssize_t aw_attr_set_monitor_switch(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    uint32_t enable = 0;
    int ret = -1;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
...
static ssize_t aw_attr_get_monitor_time(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_monitor *monitor = &aw87xxx->monitor;
    ...
    len += snprintf(buf + len, PAGE_SIZE - len, "aw_monitor_timer = %d(ms)\n", monitor_hdr->monitor_time);
    return len;
}

static ssize_t aw_attr_set_monitor_time(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t aw_attr_get_monitor_count(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_monitor *monitor = &aw87xxx->monitor;
    ...
    len += snprintf(buf + len, PAGE_SIZE - len, "aw_monitor_count = %d\n", monitor_hdr->monitor_count);
    return len;
}

static ssize_t aw_attr_set_monitor_count(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t aw_attr_get_rx(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_monitor *monitor = &aw87xxx->monitor;
    ssize_t len = 0;
    ...
    if (monitor->open_dsp_en) {
        ...
        len += snprintf(buf + len, PAGE_SIZE - len, "command is invalid\n");
    }

    return len;
}

static ssize_t aw_attr_set_rx(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_monitor *monitor = &aw87xxx->monitor;
    int ret = -1;
...
static DEVICE_ATTR(esd_enable, S_IWUSR | S_IRUGO, aw_attr_get_esd_enable, aw_attr_set_esd_enable);
static DEVICE_ATTR(vbat, S_IWUSR | S_IRUGO, aw_attr_get_vbat, aw_attr_set_vbat);
static DEVICE_ATTR(vmax, S_IWUSR | S_IRUGO, aw_attr_get_vmax, aw_attr_set_vmax);

static DEVICE_ATTR(monitor_switch, S_IWUSR | S_IRUGO, aw_attr_get_monitor_switch, aw_attr_set_monitor_switch);
static DEVICE_ATTR(monitor_time, S_IWUSR | S_IRUGO, aw_attr_get_monitor_time, aw_attr_set_monitor_time);
static DEVICE_ATTR(monitor_count, S_IWUSR | S_IRUGO, aw_attr_get_monitor_count, aw_attr_set_monitor_count);
static DEVICE_ATTR(rx, S_IWUSR | S_IRUGO, aw_attr_get_rx, aw_attr_set_rx);
 ...
 *
 ***************************************************************************/
static void aw_monitor_dtsi_parse(struct device *dev, struct aw_monitor *monitor, struct device_node *dev_node)
{
    int ret = -1;
    ...
    if (ret < 0) {
        ...
        AW_DEV_LOGI(dev, "parse esd-enable=[%s]", monitor->esd_enable ? "true" : "false");
    }
}

void aw87xxx_monitor_init(struct device *dev, struct aw_monitor *monitor, struct device_node *dev_node)
{
    int ret = -1;
    struct aw87xxx *aw87xxx =
        container_of(monitor, struct aw87xxx, monitor);
...
void aw87xxx_monitor_exit(struct aw_monitor *monitor)
{
    struct aw87xxx *aw87xxx =
        container_of(monitor, struct aw87xxx, monitor);
    /*rm attr node*/
    sysfs_remove_group(&aw87xxx->dev->kobj, &aw_monitor_vol_adjust_group);

    aw87xxx_monitor_stop(monitor);

    if (monitor->open_dsp_en) {
        sysfs_remove_group(&aw87xxx->dev->kobj, &aw_monitor_control_group);
    }
}
