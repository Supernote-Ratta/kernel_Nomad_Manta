...
static const char *const aw87xxx_spin_switch[] = {"spin_0", "spin_90", "spin_180", "spin_270"};
...
static int aw87xxx_power_down(struct aw87xxx *aw87xxx, char *profile)
{
    ...
    AW_DEV_LOGD(aw87xxx->dev, "get profile[%s] data len [%d]", profile, data_container->len);
...
int aw87xxx_set_profile_by_id(int dev_index, int profile_id)
{
    ...
    if (profile == NULL) {
        AW_LOGE("aw87xxx, dev_index[%d] profile[%d] not support!", dev_index, profile_id);
        return -EINVAL;
    }

    AW_LOGI("aw87xxx, dev_index[%d] set profile[%s] by id[%d]", dev_index, profile, profile_id);
 ...
 *
 ****************************************************************************/
static int aw87xxx_profile_switch_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
    int count = 0;
    char *name = NULL;
    ...
    profile_name = aw87xxx_acf_get_prof_name_form_index(aw87xxx->dev, &aw87xxx->acf_info, count);
...
static int aw87xxx_profile_switch_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    int ret = -1;
    char *profile_name = NULL;
...
static int aw87xxx_profile_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    int index = 0;
    char *profile;
    struct aw87xxx *aw87xxx = (struct aw87xxx *)kcontrol->private_value;
    ...
    index = aw87xxx_acf_get_prof_index_form_name(aw87xxx->dev, &aw87xxx->acf_info, aw87xxx->current_profile);
    if (index < 0) {
...
static int aw87xxx_vmax_get_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
...
static int aw87xxx_vmax_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    int ret = -1;
    int vmax_val = 0;
    struct aw87xxx *aw87xxx = (struct aw87xxx *)kcontrol->private_value;
...
static int aw87xxx_monitor_switch_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
    int count;
    ...
    strlcpy(uinfo->value.enumerated.name, aw87xxx_monitor_switch[uinfo->value.enumerated.item], strlen(aw87xxx_monitor_switch[uinfo->value.enumerated.item]) + 1);

    return 0;
}

static int aw87xxx_monitor_switch_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int aw87xxx_monitor_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    struct aw87xxx *aw87xxx = (struct aw87xxx *)kcontrol->private_value;
    struct aw_monitor *aw_monitor = &aw87xxx->monitor;
...
static int aw87xxx_spin_switch_info(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_info *uinfo)
{
    int count;
    ...
    strlcpy(uinfo->value.enumerated.name, aw87xxx_spin_switch[uinfo->value.enumerated.item], strlen(aw87xxx_spin_switch[uinfo->value.enumerated.item]) + 1);

    return 0;
}

static int aw87xxx_spin_switch_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int aw87xxx_spin_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    struct aw87xxx *aw87xxx = (struct aw87xxx *)kcontrol->private_value;
...
static int aw87xxx_kcontrol_dynamic_create(struct aw87xxx *aw87xxx, void *codec)
{
    ...
    aw87xxx_kcontrol = devm_kzalloc(aw87xxx->dev, sizeof(struct snd_kcontrol_new) * kcontrol_num, GFP_KERNEL);
    ...
    snprintf(kctl_name[0], AW_NAME_BUF_MAX, "aw87xxx_profile_switch_%d", aw87xxx->dev_index);
    ...
    snprintf(kctl_name[1], AW_NAME_BUF_MAX, "aw87xxx_vmax_get_%d", aw87xxx->dev_index);
    ...
    kctl_name[2] = devm_kzalloc(aw87xxx->codec->dev, AW_NAME_BUF_MAX, GFP_KERNEL);
    if (kctl_name[2] == NULL) {
        return -ENOMEM;
    }

    snprintf(kctl_name[2], AW_NAME_BUF_MAX, "aw87xxx_monitor_switch_%d", aw87xxx->dev_index);
    ...
    ret = aw_componet_codec_ops.add_codec_controls(aw87xxx->codec, aw87xxx_kcontrol, kcontrol_num);
    if (ret < 0) {
        AW_DEV_LOGE(aw87xxx->dev, "add codec controls failed, ret = %d", ret);
        return ret;
    }

    AW_DEV_LOGI(aw87xxx->dev, "add codec controls[%s,%s,%s]", aw87xxx_kcontrol[0].name, aw87xxx_kcontrol[1].name, aw87xxx_kcontrol[2].name);

    return 0;
}

static int aw87xxx_public_kcontrol_create(struct aw87xxx *aw87xxx, void *codec)
{
    struct snd_kcontrol_new *aw87xxx_kcontrol = NULL;
    aw_snd_soc_codec_t *soc_codec = (aw_snd_soc_codec_t *)codec;
    ...
    AW_DEV_LOGI(aw87xxx->dev, "add public codec controls[%s]", aw87xxx_kcontrol[0].name);
...
static int aw87xxx_init_default_prof(struct aw87xxx *aw87xxx)
{
    ...
    AW_DEV_LOGI(aw87xxx->dev, "init profile name [%s]", aw87xxx->current_profile);
...
static void aw87xxx_fw_load_retry(struct aw87xxx *aw87xxx)
{
    ...
    if (acf_info->load_count < AW_LOAD_FW_RETRIES) {
        ...
        schedule_delayed_work(&aw87xxx->fw_load_work, msecs_to_jiffies(ram_timer_val));
    } else {
...
static void aw87xxx_fw_load(const struct firmware *fw, void *context)
{
    ...
    AW_DEV_LOGD(aw87xxx->dev, "loaded %s - size: %ld", aw87xxx->fw_name, (u_long)(fw ? fw->size : 0));
...
static void aw87xxx_fw_load_work_routine(struct work_struct *work)
{
    ...
    if (prof_info->status == AW_ACF_WAIT) {
        request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG, aw87xxx->fw_name, aw87xxx->dev, GFP_KERNEL, aw87xxx, aw87xxx_fw_load);
    }
}
 ...
 *
 ****************************************************************************/
static ssize_t aw87xxx_attr_get_reg(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    int ret = 0;
    unsigned int i = 0;
    ...
    for (i = 0; i < aw_dev->reg_max_addr; i++) {
        ...
        if (ret < 0) {
            len += snprintf(buf + len, PAGE_SIZE - len, "read reg [0x%x] failed\n", i);
...
static ssize_t aw87xxx_attr_set_reg(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
    ...
    if (sscanf(buf, "0x%x 0x%x", &databuf[0], &databuf[1]) == 2) {
        if (databuf[0] >= aw87xxx->aw_dev.reg_max_addr) {
            AW_DEV_LOGE(aw87xxx->dev, "set reg[0x%x] error,is out of reg_addr_max[0x%x]", databuf[0], aw87xxx->aw_dev.reg_max_addr);
            mutex_unlock(&aw87xxx->reg_lock);
            return -EINVAL;
        }

        ret = aw87xxx_dev_i2c_write_byte(&aw87xxx->aw_dev, databuf[0], databuf[1]);
        if (ret < 0) {
            AW_DEV_LOGE(aw87xxx->dev, "set [0x%x]=0x%x failed", databuf[0], databuf[1]);
        } else {
            AW_DEV_LOGD(aw87xxx->dev, "set [0x%x]=0x%x succeed", databuf[0], databuf[1]);
        }
    } else {
...
static ssize_t aw87xxx_attr_get_profile(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    unsigned int i = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    ...
    if (!prof_info->status) {
        len += snprintf(buf + len, PAGE_SIZE - len, "profile_cfg not load\n");
        return len;
    }
    ...
    for (i = 0; i < prof_info->count; i++) {
        if (!strncmp(aw87xxx->current_profile, prof_info->prof_name_list[i], AW_PROFILE_STR_MAX)) {
...
static ssize_t aw87xxx_attr_set_profile(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
    ...
    if (strlen(buf) > AW_PROFILE_STR_MAX) {
        AW_DEV_LOGE(aw87xxx->dev, "input profile_str_len is out of max[%d]", AW_PROFILE_STR_MAX);
        return -EINVAL;
    }
...
static ssize_t aw87xxx_attr_get_hwen(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
...
static ssize_t aw87xxx_attr_set_hwen(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
    int ret = -1;
    unsigned int state;
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    ...
    if (state == AW_DEV_HWEN_OFF) {
        ...
        AW_DEV_LOGE(aw87xxx->dev, "input [%d] error, hwen_on=[%d],hwen_off=[%d]", state, AW_DEV_HWEN_ON, AW_DEV_HWEN_OFF);
    }
    mutex_unlock(&aw87xxx->reg_lock);
    return len;
}
...
int aw87xxx_awrw_write(struct aw87xxx *aw87xxx, const char *buf, size_t count)
{
    ...
    for (i = 0; i < packet->reg_num; i++) {
        ret = sscanf(buf + AWRW_HDR_LEN + 1 + i * (data_str_size + 1), "0x%x", &temp_data);
        if (ret != 1) {
...
static int aw87xxx_awrw_data_check(struct aw87xxx *aw87xxx, int *data, size_t count)
{
    struct aw_i2c_packet *packet = &aw87xxx->i2c_packet;
    int req_data_len = 0;
    int act_data_len = 0;
    ...
    if ((data[AWRW_HDR_ADDR_BYTES] != AWRW_ADDR_BYTES) || (data[AWRW_HDR_DATA_BYTES] != AWRW_DATA_BYTES)) {
        AW_DEV_LOGE(aw87xxx->dev, "addr_bytes [%d] or data_bytes [%d] unsupport", data[AWRW_HDR_ADDR_BYTES], data[AWRW_HDR_DATA_BYTES]);
        return -EINVAL;
    }
    ...
    if (data[AWRW_HDR_WR_FLAG] == AWRW_FLAG_WRITE) {
        ...
        if (req_data_len > act_data_len) {
            AW_DEV_LOGE(aw87xxx->dev, "data_len checkfailed,requeset data_len [%d],actaul data_len [%d]", req_data_len, act_data_len);
            return -EINVAL;
        }
    }
...
static int aw87xxx_awrw_parse_buf(struct aw87xxx *aw87xxx, const char *buf, size_t count, int *wr_status)
{
    int data[AWRW_HDR_MAX] = {0};
    struct aw_i2c_packet *packet = &aw87xxx->i2c_packet;
    int ret = -1;

    if (sscanf(buf, "0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", &data[AWRW_HDR_WR_FLAG], &data[AWRW_HDR_ADDR_BYTES], &data[AWRW_HDR_DATA_BYTES], &data[AWRW_HDR_REG_NUM], &data[AWRW_HDR_REG_ADDR]) == 5) {
...
static ssize_t aw87xxx_attr_awrw_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_i2c_packet *packet = &aw87xxx->i2c_packet;
    int wr_status = 0;
    ...
    if (wr_status == AWRW_FLAG_WRITE) {
        ...
        AW_DEV_LOGE(aw87xxx->dev, "please check str format, unsupport read_write_status: %d", wr_status);
        return -EINVAL;
    }
...
static ssize_t aw87xxx_attr_awrw_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct aw87xxx *aw87xxx = dev_get_drvdata(dev);
    struct aw_i2c_packet *packet = &aw87xxx->i2c_packet;
    int data_len = 0;
    ...
    mutex_lock(&aw87xxx->reg_lock);
    ret = aw87xxx_dev_i2c_read_msg(&aw87xxx->aw_dev, packet->reg_addr, (char *)reg_data, data_len);
    if (ret < 0) {
        ret = -EFAULT;
        mutex_unlock(&aw87xxx->reg_lock);
        goto exit;
    ...
    AW_DEV_LOGI(aw87xxx->dev, "reg_addr 0x%02x, reg_num %d", packet->reg_addr, packet->reg_num);
...
static ssize_t aw87xxx_drv_ver_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t len = 0;
...
static DEVICE_ATTR(reg, S_IWUSR | S_IRUGO, aw87xxx_attr_get_reg, aw87xxx_attr_set_reg);
static DEVICE_ATTR(profile, S_IWUSR | S_IRUGO, aw87xxx_attr_get_profile, aw87xxx_attr_set_profile);
static DEVICE_ATTR(hwen, S_IWUSR | S_IRUGO, aw87xxx_attr_get_hwen, aw87xxx_attr_set_hwen);
static DEVICE_ATTR(awrw, S_IWUSR | S_IRUGO, aw87xxx_attr_awrw_show, aw87xxx_attr_awrw_store);
...
static int aw87xxx_dtsi_parse(struct aw87xxx *aw87xxx, struct device_node *dev_node)
{
    int ret = -1;
    ...
    if (ret < 0) {
        AW_DEV_LOGI(aw87xxx->dev, "dev_index parse failed, user default[%d], ret=%d", g_aw87xxx_dev_cnt, ret);
        aw87xxx->dev_index = g_aw87xxx_dev_cnt;
    } else {
        aw87xxx->dev_index = dev_index;
        AW_DEV_LOGI(aw87xxx->dev, "parse dev_index=[%d]", aw87xxx->dev_index);
    }
...
static int aw87xxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    AW_DEV_LOGI(aw87xxx->dev, "succeed, dev_index=[%d], g_aw87xxx_dev_cnt= [%d]", aw87xxx->dev_index, g_aw87xxx_dev_cnt);
