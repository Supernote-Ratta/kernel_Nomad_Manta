...
static char *g_profile_name[] = {"Music", "Voice", "Voip", "Ringtone", "Ringtone_hs", "Lowpower", "Bypass", "Mmi", "Fm", "Notification", "Receiver", "Off"};
...
static int aw_check_file_id(struct device *dev, char *fw_data, int32_t file_id)
{
...
static int aw_check_header_size(struct device *dev, char *fw_data, size_t fw_size)
{
...
static int aw_check_data_size_v_0_0_0_1(struct device *dev, char *fw_data, size_t fw_size)
{
    int i = 0;
    size_t data_size = 0;
    ...
    for (i = 0; i < acf_hdr->dde_num; ++i) {
        if (acf_dde[i].data_size % 2) {
            AW_DEV_LOGE(dev, "acf dde[%d].data_size[%d],dev_name[%s],data_type[%d], data_size check failed", i, acf_dde[i].data_size, acf_dde[i].dev_name, acf_dde[i].data_type);
            return -EINVAL;
        }
    ...
    if (fw_size != data_size + sizeof(struct aw_acf_hdr) + acf_hdr->ddt_size) {
        ...
        AW_DEV_LOGE(dev, "fw_size=%ld,hdr_size and ddt size and data size =%ld", (u_long)fw_size, (u_long)(data_size + sizeof(struct aw_acf_hdr) + acf_hdr->ddt_size));
        return -EINVAL;
    }
...
static int aw_check_data_v_0_0_0_1(struct device *dev, char *fw_data, size_t size)
{
    int ret = -1;
...
static int aw_check_data_size_v_1_0_0_0(struct device *dev, char *fw_data, size_t fw_size)
{
    int i = 0;
    size_t data_size = 0;
    struct aw_acf_hdr *acf_hdr = NULL;
...
static int aw_check_data_v_1_0_0_0(struct device *dev, char *fw_data, size_t size)
{
    int ret = -1;
 ...
 * acf chack API
 **************************************************************************/
static int aw_check_acf_firmware(struct device *dev, char *fw_data, size_t size)
{
    int ret = -1;
    struct aw_acf_hdr *acf_hdr = NULL;
    ...
    switch (acf_hdr->hdr_version) {
        ...
        default:
            AW_DEV_LOGE(dev, "unsupported hdr_version [0x%x]", acf_hdr->hdr_version);
            return -EINVAL;
    }
 ...
 *
 *************************************************************************/
static int aw_parse_raw_reg(struct device *dev, uint8_t *data, uint32_t data_len, struct aw_prof_desc *prof_desc)
{
...
static int aw_parse_reg_with_hdr(struct device *dev, uint8_t *data, uint32_t data_len, struct aw_prof_desc *prof_desc)
{
...
static int aw_parse_monitor_config(struct device *dev, char *monitor_data, uint32_t data_len)
{
    int ret = -1;
...
static int aw_check_prof_str_is_off(char *profile_name)
{
    ...
    for (i = 0; i < AW_POWER_OFF_NAME_SUPPORT_COUNT; i++) {
        if (strnstr(profile_name, g_power_off_name[i], strlen(profile_name) + 1)) {
            return 0;
        }
    }
 ...
 **************************************************************************/
static int aw_check_product_name_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_acf_dde *prof_hdr)
{
    int i = 0;
...
static int aw_check_data_type_is_monitor_v_0_0_0_1(struct device *dev, struct aw_acf_dde *prof_hdr)
{
...
static int aw_parse_data_by_sec_type_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_acf_dde *prof_hdr, struct aw_prof_desc *profile_prof_desc)
{
    int ret = -1;
    ...
    switch (prof_hdr->data_type) {
        case AW_BIN_TYPE_REG:
            snprintf(profile_prof_desc->dev_name, sizeof(prof_hdr->dev_name), "%s", prof_hdr->dev_name);
            ...
            ret = aw_parse_raw_reg(dev, cfg_data, prof_hdr->data_size, profile_prof_desc);
            break;
        case AW_BIN_TYPE_HDR_REG:
            snprintf(profile_prof_desc->dev_name, sizeof(prof_hdr->dev_name), "%s", prof_hdr->dev_name);
            profile_prof_desc->prof_name = aw_get_prof_name(prof_hdr->dev_profile);
            AW_DEV_LOGD(dev, "parse hdr_reg type data enter,profile=%s", aw_get_prof_name(prof_hdr->dev_profile));
            ret = aw_parse_reg_with_hdr(dev, cfg_data, prof_hdr->data_size, profile_prof_desc);
            break;
    }
...
static int aw_parse_dev_type_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_all_prof_info *all_prof_info)
{
    int i = 0;
    int ret = -1;
    int sec_num = 0;
    ...
    for (i = 0; i < acf_info->acf_hdr.dde_num; i++) {
        if ((acf_info->aw_dev->i2c_bus == acf_dde[i].dev_bus) &&
            ...
            ret = aw_parse_data_by_sec_type_v_0_0_0_1(dev, acf_info, &acf_dde[i], prof_desc);
            if (ret < 0) {
    ...
    if (sec_num == 0) {
        AW_DEV_LOGD(dev, "get dev type num is %d, please use default", sec_num);
...
static int aw_parse_default_type_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_all_prof_info *all_prof_info)
{
    int i = 0;
    int ret = -1;
    int sec_num = 0;
    ...
    if (sec_num == 0) {
        AW_DEV_LOGE(dev, "get dev default type failed, get num[%d]", sec_num);
        return -EINVAL;
    }
...
static int aw_get_prof_count_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_all_prof_info *all_prof_info)
{
    int i = 0;
    int prof_count = 0;
...
static int aw_set_prof_off_info_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_all_prof_info *all_prof_info, int index)
{
    struct aw_prof_desc *prof_desc = all_prof_info->prof_desc;
    struct aw_prof_info *prof_info = &acf_info->prof_info;

    if (index >= prof_info->count) {
        AW_DEV_LOGE(dev, "index[%d] is out of table,profile count[%d]", index, prof_info->count);
        return -EINVAL;
    }

    if (prof_desc[AW_PROFILE_OFF].prof_st == AW_PROFILE_OK) {
        prof_info->prof_desc[index] = prof_desc[AW_PROFILE_OFF];
        AW_DEV_LOGI(dev, "product=[%s]----profile=[%s]", prof_info->prof_desc[index].dev_name, aw_get_prof_name(AW_PROFILE_OFF));
    } else {
        memset(&prof_info->prof_desc[index].data_container, 0, sizeof(struct aw_data_container));
...
static int aw_get_vaild_prof_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info, struct aw_all_prof_info *all_prof_info)
{
    int i = 0;
    int ret = 0;
    int index = 0;
    ...
    prof_info->count = ret;
    prof_info->prof_desc = devm_kzalloc(dev, prof_info->count * sizeof(struct aw_prof_desc), GFP_KERNEL);
    ...
    for (i = 0; i < AW_PROFILE_MAX; i++) {
        if (i != AW_PROFILE_OFF && prof_desc[i].prof_st == AW_PROFILE_OK) {
            if (index >= prof_info->count) {
                AW_DEV_LOGE(dev, "get profile index[%d] overflow count[%d]", index, prof_info->count);
                return -ENOMEM;
            }
            prof_info->prof_desc[index] = prof_desc[i];
            AW_DEV_LOGI(dev, "product=[%s]----profile=[%s]", prof_info->prof_desc[index].dev_name, aw_get_prof_name(i));
            index++;
        }
    }
...
static int aw_set_prof_name_list_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info)
{
    int i = 0;
    int count = acf_info->prof_info.count;
    struct aw_prof_info *prof_info = &acf_info->prof_info;

    prof_info->prof_name_list = (char(*)[AW_PROFILE_STR_MAX])devm_kzalloc(dev, count * (AW_PROFILE_STR_MAX), GFP_KERNEL);
    ...
    for (i = 0; i < count; ++i) {
        snprintf(prof_info->prof_name_list[i], AW_PROFILE_STR_MAX, "%s", prof_info->prof_desc[i].prof_name);
        AW_DEV_LOGI(dev, "index=[%d], profile_name=[%s]", i, prof_info->prof_name_list[i]);
    }

    return 0;
}

static int aw_parse_acf_v_0_0_0_1(struct device *dev, struct acf_bin_info *acf_info)

{
    ...
    if (ret < 0) {
        ...
        AW_DEV_LOGE(dev, "hdr_cersion[0x%x] parse failed", acf_info->acf_hdr.hdr_version);
        return ret;
    }
 ...
 **************************************************************************/
static int aw_check_product_name_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info, struct aw_acf_dde_v_1_0_0_0 *prof_hdr)
{
    int i = 0;
...
static int aw_get_dde_type_info_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    int i;
    int dev_num = 0;
    int default_num = 0;
...
static int aw_parse_get_dev_type_prof_count_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    struct aw_acf_hdr *acf_hdr = (struct aw_acf_hdr *)acf_info->fw_data;
    struct aw_acf_dde_v_1_0_0_0 *acf_dde =
        (struct aw_acf_dde_v_1_0_0_0 *)(acf_info->fw_data + acf_hdr->ddt_offset);
...
static int aw_parse_get_default_type_prof_count_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    struct aw_acf_hdr *acf_hdr = (struct aw_acf_hdr *)acf_info->fw_data;
    struct aw_acf_dde_v_1_0_0_0 *acf_dde =
        (struct aw_acf_dde_v_1_0_0_0 *)(acf_info->fw_data + acf_hdr->ddt_offset);
...
static int aw_parse_get_profile_count_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    int ret = 0;
    ...
    if (acf_info->prof_info.prof_type == AW_DDE_DEV_TYPE_ID) {
        ...
        AW_DEV_LOGE(dev, "unsupport prof_type[0x%x]", acf_info->prof_info.prof_type);
        return -EINVAL;
    }
...
static int aw_parse_dev_type_prof_name_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    struct aw_acf_hdr *acf_hdr = (struct aw_acf_hdr *)acf_info->fw_data;
    struct aw_acf_dde_v_1_0_0_0 *acf_dde =
        (struct aw_acf_dde_v_1_0_0_0 *)(acf_info->fw_data + acf_hdr->ddt_offset);
    ...
    for (i = 0; i < acf_hdr->dde_num; ++i) {
        if (((acf_dde[i].data_type == AW_BIN_TYPE_REG) ||
            ...
            snprintf(prof_info->prof_name_list[list_index], AW_PROFILE_STR_MAX, "%s", acf_dde[i].dev_profile_str);
            AW_DEV_LOGI(dev, "profile_name=[%s]", prof_info->prof_name_list[list_index]);
            list_index++;
        }
    }
...
static int aw_parse_default_type_prof_name_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    struct aw_acf_hdr *acf_hdr = (struct aw_acf_hdr *)acf_info->fw_data;
    struct aw_acf_dde_v_1_0_0_0 *acf_dde =
        (struct aw_acf_dde_v_1_0_0_0 *)(acf_info->fw_data + acf_hdr->ddt_offset);
...
static int aw_parse_prof_name_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    int ret = 0;
    int count = acf_info->prof_info.count;
    struct aw_prof_info *prof_info = &acf_info->prof_info;
...
static int aw_search_prof_index_from_list_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info, struct aw_prof_desc **prof_desc, struct aw_acf_dde_v_1_0_0_0 *prof_hdr)
{
    int i = 0;
    int count = acf_info->prof_info.count;
...
static int aw_parse_data_by_sec_type_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info, struct aw_acf_dde_v_1_0_0_0 *prof_hdr)
{
    int ret = -1;
    char *cfg_data = acf_info->fw_data + prof_hdr->data_offset;
    struct aw_prof_desc *prof_desc = NULL;
    ...
    switch (prof_hdr->data_type) {
        case AW_BIN_TYPE_REG:
            snprintf(prof_desc->dev_name, sizeof(prof_hdr->dev_name), "%s", prof_hdr->dev_name);
            AW_DEV_LOGI(dev, "parse reg type data enter,product=[%s],prof_id=[%d],prof_name=[%s]", prof_hdr->dev_name, prof_hdr->dev_profile, prof_hdr->dev_profile_str);
            prof_desc->prof_name = prof_hdr->dev_profile_str;
            ret = aw_parse_raw_reg(dev, cfg_data, prof_hdr->data_size, prof_desc);
            break;
        case AW_BIN_TYPE_HDR_REG:
            snprintf(prof_desc->dev_name, sizeof(prof_hdr->dev_name), "%s", prof_hdr->dev_name);
            AW_DEV_LOGI(dev, "parse hdr_reg type data enter,product=[%s],prof_id=[%d],prof_name=[%s]", prof_hdr->dev_name, prof_hdr->dev_profile, prof_hdr->dev_profile_str);
            prof_desc->prof_name = prof_hdr->dev_profile_str;
            ret = aw_parse_reg_with_hdr(dev, cfg_data, prof_hdr->data_size, prof_desc);
            break;
    }
...
static int aw_parse_dev_type_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    int i = 0;
    int ret;
    ...
    for (i = 0; i < acf_info->acf_hdr.dde_num; i++) {
        if ((acf_dde[i].type == AW_DDE_DEV_TYPE_ID) &&
            ...
            if (acf_dde[i].data_type == AW_MONITOR) {
                ...
                ret = aw_parse_data_by_sec_type_v_1_0_0_0(dev, acf_info, &acf_dde[i]);
                if (ret < 0) {
                    AW_DEV_LOGE(dev, "parse dev type data failed");
                } else {
                    parse_prof_count++;
...
static int aw_parse_default_type_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    int i = 0;
    int ret;
    int parse_prof_count = 0;
...
static int aw_parse_by_hdr_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    int ret;
...
static int aw_set_prof_off_info_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)
{
    struct aw_prof_info *prof_info = &acf_info->prof_info;
    int i = 0;
    int ret = 0;
    ...
    for (i = 0; i < prof_info->count; ++i) {
        if (!(prof_info->prof_desc[i].prof_st)) {
            snprintf(prof_info->prof_name_list[i], AW_PROFILE_STR_MAX, "%s", g_power_off_name[0]);
            prof_info->prof_desc[i].prof_name = prof_info->prof_name_list[i];
            prof_info->prof_desc[i].prof_st = AW_PROFILE_WAIT;
            memset(&prof_info->prof_desc[i].data_container, 0, sizeof(struct aw_data_container));
            return 0;
        }

        ret = aw_check_prof_str_is_off(prof_info->prof_name_list[i]);
        if (ret == 0) {
            AW_DEV_LOGD(dev, "found profile off,data_len=[%d]", prof_info->prof_desc[i].data_container.len);
            return 0;
        }
    }

    AW_DEV_LOGE(dev, "index[%d] is out of table,profile count[%d]", i, prof_info->count);
    return -EINVAL;
}

static int aw_parse_acf_v_1_0_0_0(struct device *dev, struct acf_bin_info *acf_info)

{
    ...
    acf_info->prof_info.prof_desc = devm_kzalloc(dev, prof_info->count * sizeof(struct aw_prof_desc), GFP_KERNEL);
...
int aw87xxx_acf_parse(struct device *dev, struct acf_bin_info *acf_info)
{
    ...
    memcpy(&acf_info->acf_hdr, acf_info->fw_data, sizeof(struct aw_acf_hdr));
    ...
    switch (acf_info->acf_hdr.hdr_version) {
        ...
        default:
            AW_DEV_LOGE(dev, "unsupported hdr_version [0x%x]", acf_info->acf_hdr.hdr_version);
            return -EINVAL;
    }
...
struct aw_prof_desc *aw87xxx_acf_get_prof_desc_form_name(struct device *dev, struct acf_bin_info *acf_info, char *profile_name)
{
    int i = 0;
    struct aw_prof_desc *prof_desc = NULL;
    struct aw_prof_info *prof_info = &acf_info->prof_info;
    ...
    for (i = 0; i < prof_info->count; i++) {
        if (!strncmp(profile_name, prof_info->prof_desc[i].prof_name, AW_PROFILE_STR_MAX)) {
...
int aw87xxx_acf_get_prof_index_form_name(struct device *dev, struct acf_bin_info *acf_info, char *profile_name)
{
    int i = 0;
    struct aw_prof_info *prof_info = &acf_info->prof_info;
    ...
    for (i = 0; i < prof_info->count; i++) {
        if (!strncmp(profile_name, prof_info->prof_name_list[i], AW_PROFILE_STR_MAX)) {
            return i;
        }
    }
...
char *aw87xxx_acf_get_prof_name_form_index(struct device *dev, struct acf_bin_info *acf_info, int index)
{
    struct aw_prof_info *prof_info = &acf_info->prof_info;
...
int aw87xxx_acf_get_profile_count(struct device *dev, struct acf_bin_info *acf_info)
{
    struct aw_prof_info *prof_info = &acf_info->prof_info;
...
char *aw87xxx_acf_get_prof_off_name(struct device *dev, struct acf_bin_info *acf_info)
{
    int i = 0;
    int ret = 0;
    struct aw_prof_info *prof_info = &acf_info->prof_info;
