...
static int mtk_spk_recv_ipi_buf_from_dsp(int8_t *buffer, int16_t size, uint32_t *buf_len)
{
    AW_LOGI("enter");
    return 0;
}
...
#ifdef AW_MTK_OPEN_DSP_PLATFORM
static int aw_mtk_write_data_to_dsp(int32_t param_id, void *data, int size)
{
    ...
    memcpy(((char *)dsp_data) + sizeof(mtk_dsp_hdr_t), data, size);

    ret = mtk_spk_send_ipi_buf_to_dsp(dsp_data, sizeof(mtk_dsp_hdr_t) + size);
    if (ret < 0) {
...
static int aw_mtk_read_data_from_dsp(int32_t param_id, void *data, int data_size)
{
    int ret;
    mtk_dsp_hdr_t hdr;
...
static int aw_qcom_write_data_to_dsp(int32_t param_id, void *data, int data_size)
{
    int ret = 0;
...
static int aw_qcom_read_data_from_dsp(int32_t param_id, void *data, int data_size)
{
    int ret = 0;
...
int aw87xxx_dsp_get_rx_module_enable(int *enable)
{
    ...
    return aw_read_data_from_dsp(AWDSP_RX_SET_ENABLE, (void *)enable, sizeof(uint32_t));
}
...
int aw87xxx_dsp_get_vmax(uint32_t *vmax, int dev_index)
{
    ...
    switch (dev_index % AW_DSP_CHANNEL_MAX) {
        ...
        case AW_DSP_CHANNEL_1:
            ...
            break;
        default:
            AW_LOGE("algo only support double PA channel:%d unsupport", dev_index);
            return -EINVAL;
    }

    return aw_read_data_from_dsp(param_id, (void *)vmax, sizeof(uint32_t));
}
...
int aw87xxx_dsp_set_spin(uint32_t ctrl_value)
{
    ...
    ret = aw_write_data_to_dsp(AW_MSG_ID_SPIN, &ctrl_value, sizeof(uint32_t));
    if (ret) {
