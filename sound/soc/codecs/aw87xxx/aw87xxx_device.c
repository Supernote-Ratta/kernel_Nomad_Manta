 ...
 *
 ***************************************************************************/
int aw87xxx_dev_i2c_write_byte(struct aw_device *aw_dev, uint8_t reg_addr, uint8_t reg_data)
{
    int ret = -1;
    unsigned char cnt = 0;
...
int aw87xxx_dev_i2c_read_byte(struct aw_device *aw_dev, uint8_t reg_addr, uint8_t *reg_data)
{
    int ret = -1;
    unsigned char cnt = 0;
...
int aw87xxx_dev_i2c_read_msg(struct aw_device *aw_dev, uint8_t reg_addr, uint8_t *data_buf, uint32_t data_len)
{
    int ret = -1;
...
int aw87xxx_dev_i2c_write_bits(struct aw_device *aw_dev, uint8_t reg_addr, uint8_t mask, uint8_t reg_data)
{
    int ret = -1;
 ...
 *
 ************************************************************************/
static int aw87xxx_dev_reg_update(struct aw_device *aw_dev, struct aw_data_container *profile_data)
{
    int i = 0;
    int ret = -1;
    ...
    for (i = 0; i < profile_data->len; i = i + 2) {
        AW_DEV_LOGI(aw_dev->dev, "reg=0x%02x, val = 0x%02x", profile_data->data[i], profile_data->data[i + 1]);

        ret = aw87xxx_dev_i2c_write_byte(aw_dev, profile_data->data[i], profile_data->data[i + 1]);
        if (ret < 0) {
            return ret;
        }
    }
...
static void aw87xxx_dev_reg_mute_bits_set(struct aw_device *aw_dev, uint8_t *reg_val, bool enable)
{
    if (enable) {
...
static int aw87xxx_dev_reg_update_mute(struct aw_device *aw_dev, struct aw_data_container *profile_data)
{
    int i = 0;
    int ret = -1;
    uint8_t reg_val = 0;
...
static int aw87xxx_dev_mute_ctrl(struct aw_device *aw_dev, bool enable)
{
    ...
    if (enable) {
        ret = aw87xxx_dev_i2c_write_bits(aw_dev, aw_dev->mute_desc.addr, aw_dev->mute_desc.mask, aw_dev->mute_desc.enable);
        if (ret < 0) {
            return ret;
        }
        AW_DEV_LOGI(aw_dev->dev, "set mute down");
    } else {
        ret = aw87xxx_dev_i2c_write_bits(aw_dev, aw_dev->mute_desc.addr, aw_dev->mute_desc.mask, aw_dev->mute_desc.disable);
        if (ret < 0) {
            return ret;
        }
...
void aw87xxx_dev_soft_reset(struct aw_device *aw_dev)
{
    ...
    if (soft_rst->len % 2) {
        AW_DEV_LOGE(aw_dev->dev, "softrst data_len[%d] is odd number,data not available", aw_dev->soft_rst_desc.len);
        return;
    }

    for (i = 0; i < soft_rst->len; i += 2) {
        AW_DEV_LOGD(aw_dev->dev, "softrst_reg=0x%02x, val = 0x%02x", soft_rst->access[i], soft_rst->access[i + 1]);

        ret = aw87xxx_dev_i2c_write_byte(aw_dev, soft_rst->access[i], soft_rst->access[i + 1]);
        if (ret < 0) {
            AW_DEV_LOGE(aw_dev->dev, "write failed,ret = %d,cnt=%d", ret, i);
            return;
        }
    }
...
int aw87xxx_dev_default_pwr_off(struct aw_device *aw_dev, struct aw_data_container *profile_data)
{
    int ret = 0;
...
int aw87xxx_dev_default_pwr_on(struct aw_device *aw_dev, struct aw_data_container *profile_data)
{
    int ret = 0;
...
int aw87xxx_dev_esd_reg_status_check(struct aw_device *aw_dev)
{
    ...
    ret = aw87xxx_dev_i2c_read_byte(aw_dev, esd_desc->first_update_reg_addr, &reg_val);
    if (ret < 0) {
        AW_DEV_LOGE(aw_dev->dev, "read reg 0x%02x failed", esd_desc->first_update_reg_addr);
        return ret;
    }

    AW_DEV_LOGD(aw_dev->dev, "0x%02x:default val=0x%02x real val=0x%02x", esd_desc->first_update_reg_addr, esd_desc->first_update_reg_val, reg_val);
...
int aw87xxx_dev_check_reg_is_rec_mode(struct aw_device *aw_dev)
{
    ...
    if (ret < 0) {
        AW_DEV_LOGE(aw_dev->dev, "read reg 0x%02x failed", rec_desc->addr);
        return ret;
    }
...
static int aw_dev_pid_9b_reg_update(struct aw_device *aw_dev, struct aw_data_container *profile_data)
{
    int i = 0;
    int ret = -1;
    uint8_t reg_val = 0;
    ...
    ret = aw87xxx_dev_i2c_write_byte(aw_dev, AW87XXX_PID_9B_ENCRYPTION_REG, AW87XXX_PID_9B_ENCRYPTION_BOOST_OUTPUT_SET);
    if (ret < 0) {
        return ret;
    }

    for (i = 1; i < AW_PID_9B_BIN_REG_CFG_COUNT; i++) {
        AW_DEV_LOGI(aw_dev->dev, "reg=0x%02x, val = 0x%02x", i, profile_data->data[i]);
...
static int aw87xxx_dev_get_chipid(struct aw_device *aw_dev)
{
    ...
    for (cnt = 0; cnt < AW_READ_CHIPID_RETRIES; cnt++) {
        ...
        if (ret < 0) {
            AW_DEV_LOGE(aw_dev->dev, "[%d] read chip is failed, ret=%d", cnt, ret);
            continue;
        }
        break;
    }
