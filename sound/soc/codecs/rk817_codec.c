...
static int rk817_codec_ctl_gpio(struct rk817_codec_priv *rk817, int gpio, int level)
{
    if ((gpio & CODEC_SET_SPK) && rk817->spk_ctl_gpio) {
        gpiod_set_value(rk817->spk_ctl_gpio, level);
        DBG("%s set spk clt %d\n", __func__, level);
        msleep(rk817->spk_mute_delay);
    }

    if ((gpio & CODEC_SET_HP) && rk817->hp_ctl_gpio) {
...
#define RK817_CODEC_PLAYBACK_POWER_DOWN_LIST_LEN ARRAY_SIZE(playback_power_down_list)
#ifndef CONFIG_SND_SOC_ES7210
...
#define RK817_CODEC_CAPTURE_POWER_UP_LIST_LEN ARRAY_SIZE(capture_power_up_list)
...
#define RK817_CODEC_CAPTURE_POWER_DOWN_LIST_LEN ARRAY_SIZE(capture_power_down_list)
...
static int rk817_codec_power_up(struct snd_soc_component *component, int type)
{
    ...
    DBG("%s : power up %s %s %s\n", __func__, type & RK817_CODEC_PLAYBACK ? "playback" : "", type & RK817_CODEC_CAPTURE ? "capture" : "", type & RK817_CODEC_INCALL ? "incall" : "");

    if (type & RK817_CODEC_PLAYBACK) {
        snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, DAC_DIG_CLK_MASK, DAC_DIG_CLK_EN);
        for (i = 0; i < RK817_CODEC_PLAYBACK_POWER_UP_LIST_LEN; i++) {
            snd_soc_component_write(component, playback_power_up_list[i].reg, playback_power_up_list[i].value);
        }
    }

#ifndef CONFIG_SND_SOC_ES7210
    if (type & RK817_CODEC_CAPTURE) {
        snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, ADC_DIG_CLK_MASK, ADC_DIG_CLK_EN);
        for (i = 0; i < RK817_CODEC_CAPTURE_POWER_UP_LIST_LEN; i++) {
            snd_soc_component_write(component, capture_power_up_list[i].reg, capture_power_up_list[i].value);
        }

        if (rk817->mic_in_differential) {
            snd_soc_component_update_bits(component, RK817_CODEC_AMIC_CFG0, MIC_DIFF_MASK, MIC_DIFF_EN);
        } else {
            snd_soc_component_update_bits(component, RK817_CODEC_AMIC_CFG0, MIC_DIFF_MASK, MIC_DIFF_DIS);
        }

        if (rk817->pdmdata_out_enable) {
            snd_soc_component_update_bits(component, RK817_CODEC_DI2S_CKM, PDM_EN_MASK, PDM_EN_ENABLE);
        }

        snd_soc_component_write(component, RK817_CODEC_DADC_VOLL, rk817->capture_volume);
        snd_soc_component_write(component, RK817_CODEC_DADC_VOLR, rk817->capture_volume);
    }
#endif
...
static int rk817_codec_power_down(struct snd_soc_component *component, int type)
{
    ...
    if ((type & RK817_CODEC_PLAYBACK) || (type & RK817_CODEC_INCALL)) {
        snd_soc_component_update_bits(component, RK817_CODEC_DDAC_MUTE_MIXCTL, DACMT_ENABLE, DACMT_ENABLE);
    }

    if (type & RK817_CODEC_CAPTURE) {
        for (i = 0; i < RK817_CODEC_CAPTURE_POWER_DOWN_LIST_LEN; i++) {
            snd_soc_component_write(component, capture_power_down_list[i].reg, capture_power_down_list[i].value);
        }
        snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, ADC_DIG_CLK_MASK, ADC_DIG_CLK_DIS);
    }

    if (type & RK817_CODEC_PLAYBACK) {
        for (i = 0; i < RK817_CODEC_PLAYBACK_POWER_DOWN_LIST_LEN; i++) {
            snd_soc_component_write(component, playback_power_down_list[i].reg, playback_power_down_list[i].value);
        }
        snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, DAC_DIG_CLK_MASK, DAC_DIG_CLK_DIS);
    }
...
static const char *const rk817_playback_path_mode[] = {
    ...
    "RING_SPK", "RING_HP", "RING_HP_NO_MIC", "RING_SPK_HP"
}; /* 7-10 */
...
static SOC_ENUM_SINGLE_DECL(rk817_playback_path_type, 0, 0, rk817_playback_path_mode);
static SOC_ENUM_SINGLE_DECL(rk817_capture_path_type, 0, 0, rk817_capture_path_mode);
static SOC_ENUM_SINGLE_DECL(rk817_call_path_type, 0, 0, rk817_call_path_mode);
static SOC_ENUM_SINGLE_DECL(rk817_modem_input_type, 0, 0, rk817_modem_input_mode);

static int rk817_playback_path_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int rk817_playback_path_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
    struct rk817_codec_priv *rk817 = snd_soc_component_get_drvdata(component);
    long int pre_path;
    ...
    switch (rk817->playback_path) {
        case OFF:
            if (pre_path != OFF && (pre_path != HP_PATH && pre_path != HP_NO_MIC && pre_path != RING_HP && pre_path != RING_HP_NO_MIC)) {
        ...
        case RING_SPK:
            ...
            if (rk817->out_l2spk_r2hp) {
                ...
                snd_soc_component_write(component, RK817_CODEC_ADAC_CFG1, PWD_DACBIAS_ON | PWD_DACD_ON | PWD_DACL_ON | PWD_DACR_ON);
                /* CLASS D mode */
                snd_soc_component_write(component, RK817_CODEC_DDAC_MUTE_MIXCTL, 0x18);
                /* CLASS D enable */
                snd_soc_component_write(component, RK817_CODEC_ACLASSD_CFG1, 0xa5);
                /* restart CLASS D, OCPP/N */
                snd_soc_component_write(component, RK817_CODEC_ACLASSD_CFG2, 0xf7);
            } else if (!rk817->use_ext_amplifier) {
                /* power on dac ibias/l/r */
                snd_soc_component_write(component, RK817_CODEC_ADAC_CFG1, PWD_DACBIAS_ON | PWD_DACD_ON | PWD_DACL_DOWN | PWD_DACR_DOWN);
                /* CLASS D mode */
                ...
                snd_soc_component_write(component, RK817_CODEC_AHP_CFG0, 0x80);
                /* power on dac ibias/l/r */
                snd_soc_component_write(component, RK817_CODEC_ADAC_CFG1, PWD_DACBIAS_ON | PWD_DACD_DOWN | PWD_DACL_ON | PWD_DACR_ON);
                snd_soc_component_update_bits(component, RK817_CODEC_DDAC_MUTE_MIXCTL, DACMT_ENABLE, DACMT_DISABLE);
            }
...
static int rk817_capture_path_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
    struct rk817_codec_priv *rk817 = snd_soc_component_get_drvdata(component);
    ...
    switch (rk817->capture_path) {
        ...
        case MAIN_MIC:
            ...
            if (rk817->adc_for_loopback) {
                ...
                snd_soc_component_update_bits(component, RK817_CODEC_AMIC_CFG0, 0xf, 0x0);
                snd_soc_component_write(component, RK817_CODEC_DMIC_PGA_GAIN, 0x66);
                snd_soc_component_write(component, RK817_CODEC_DADC_VOLL, 0x00);
                snd_soc_component_write(component, RK817_CODEC_DADC_VOLR, 0x00);
                break;
            }
            if (!rk817->mic_in_differential) {
                snd_soc_component_write(component, RK817_CODEC_DADC_VOLR, 0xff);
                snd_soc_component_update_bits(component, RK817_CODEC_AADC_CFG0, ADC_R_PWD_MASK, ADC_R_PWD_EN);
                snd_soc_component_update_bits(component, RK817_CODEC_AMIC_CFG0, PWD_PGA_R_MASK, PWD_PGA_R_EN);
            }
#endif
            break;
        case HANDS_FREE_MIC:
            ...
            if (!rk817->mic_in_differential) {
                snd_soc_component_write(component, RK817_CODEC_DADC_VOLL, 0xff);
                snd_soc_component_update_bits(component, RK817_CODEC_AADC_CFG0, ADC_L_PWD_MASK, ADC_L_PWD_EN);
                snd_soc_component_update_bits(component, RK817_CODEC_AMIC_CFG0, PWD_PGA_L_MASK, PWD_PGA_L_EN);
            }
#endif
            break;
        case BT_SCO_MIC:
...
static struct snd_kcontrol_new rk817_snd_path_controls[] = {
    SOC_ENUM_EXT("Playback Path", rk817_playback_path_type, rk817_playback_path_get, rk817_playback_path_put),
    SOC_ENUM_EXT("Capture MIC Path", rk817_capture_path_type, rk817_capture_path_get, rk817_capture_path_put),
};

static int rk817_set_dai_sysclk(struct snd_soc_dai *codec_dai, int clk_id, unsigned int freq, int dir)
{
...
static int rk817_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
    struct snd_soc_component *component = codec_dai->component;
...
static int rk817_hw_params(struct snd_pcm_substream *substream, struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
    ...
    snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, dtop_digen_clke, 0x00);
    snd_soc_component_update_bits(component, RK817_CODEC_DDAC_SR_LMT0, DACSRT_MASK, dtop_digen_sr_lmt0);
    snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, dtop_digen_clke, dtop_digen_clke);
...
static int rk817_digital_mute(struct snd_soc_dai *dai, int mute)
{
    ...
    if (mute) {
        ...
        snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, DAC_DIG_CLK_EN, DAC_DIG_CLK_DIS);
        snd_soc_component_update_bits(component, RK817_CODEC_DTOP_DIGEN_CLKE, DAC_DIG_CLK_EN, DAC_DIG_CLK_EN);
    } else {
        snd_soc_component_update_bits(component, RK817_CODEC_DDAC_MUTE_MIXCTL, DACMT_ENABLE, DACMT_DISABLE);
    }
...
static int rk817_codec_parse_dt_property(struct device *dev, struct rk817_codec_priv *rk817)
{
    ...
    rk817->hp_ctl_gpio = devm_gpiod_get_optional(dev, "hp-ctl", GPIOD_OUT_LOW);
    if (!IS_ERR_OR_NULL(rk817->hp_ctl_gpio)) {
        DBG("%s : hp-ctl-gpio %d\n", __func__, desc_to_gpio(rk817->hp_ctl_gpio));
    }
    ...
    ret = of_property_read_u32(node, "spk-mute-delay-ms", &rk817->spk_mute_delay);
    if (ret < 0) {
        DBG("%s() Can not read property spk-mute-delay-ms\n", __func__);
        rk817->spk_mute_delay = 0;
    }

    ret = of_property_read_u32(node, "hp-mute-delay-ms", &rk817->hp_mute_delay);
    if (ret < 0) {
    ...
    ret = of_property_read_u32(node, "capture-volume", &rk817->capture_volume);
    if (ret < 0) {
        DBG("%s() Can not read property capture-volume\n", __func__);
        rk817->capture_volume = CAPTURE_VOLUME;
    }

    rk817->mic_in_differential = of_property_read_bool(node, "mic-in-differential");
    rk817->pdmdata_out_enable = of_property_read_bool(node, "pdmdata-out-enable");
    rk817->use_ext_amplifier = of_property_read_bool(node, "use-ext-amplifier");
    rk817->out_l2spk_r2hp = of_property_read_bool(node, "out-l2spk-r2hp");
    rk817->adc_for_loopback = of_property_read_bool(node, "adc-for-loopback");
...
static int rk817_platform_probe(struct platform_device *pdev)
{
    ...
    ret = devm_snd_soc_register_component(&pdev->dev, &soc_codec_dev_rk817, rk817_dai, ARRAY_SIZE(rk817_dai));
    if (ret < 0) {
